...
describe("formatEditInputForPrompt", () => {
   test("returns path-only description when path provided", () => {
      const result = formatEditInputForPrompt({ path: "/foo.ts" });
      expect(result).toBe("for '/foo.ts'");
   });

   test("returns path-only description with edits present", () => {
      const result = formatEditInputForPrompt({
         path: "/foo.ts",
         edits: [{ oldText: "line1\nline2", newText: "replaced" }],
      });
      expect(result).toBe("for '/foo.ts'");
      // Summary is intentionally omitted — the diff is rendered separately.
      expect(result).not.toContain("replacement");
   });

   test("falls back to 'with edit input' when no path", () => {
      const result = formatEditInputForPrompt({
         edits: [{ oldText: "x", newText: "y" }],
      });
      expect(result).toBe("with edit input");
   });
});
