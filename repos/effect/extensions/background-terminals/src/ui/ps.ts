...
export function wrapCommandLines(sanitizedCommand: string, width: number): string[] {
   // First line has '$ ' (2 cols), continuation lines have '  ' (2 cols).
   // Total line width available for text content is max(1, width - 2).
   const textWidth = Math.max(1, width - 2);
   const wrapped = wrapTextWithAnsi(sanitizedCommand, textWidth);
   return wrapped.map((line, index) => (index === 0 ? "$ " : "  ") + line);
}

export function wrapDetailHeaderLines(header: string, width: number): string[] {
   return wrapTextWithAnsi(header, Math.max(1, width));
}

export function wrapHintLines(hintText: string, width: number): string[] {
   return wrapTextWithAnsi(hintText, Math.max(1, width));
}

export function calculateViewportHeight(
   terminalRows: number,
   commandLineCount: number = 1,
   hintLineCount: number = 1,
   headerLineCount: number = 1
): number {
   // Base chrome: 4 borders + 1 header + 1 tab = 6 fixed lines.
   // Plus dynamic command lines, hint lines, and header lines.
   const extraChrome = commandLineCount - 1 + (hintLineCount - 1) + (headerLineCount - 1);
   const totalChrome = 8 + extraChrome;
   return Math.max(6, terminalRows - 1 - totalChrome);
}
...
export function reconcileDashboardSelection(
   ...
   selection.index =
      stableIndex >= 0 ? stableIndex : Math.min(Math.max(0, selection.index), Math.max(0, terminals.length - 1));
...
class TerminalDetailView implements Component {
   ...
   constructor(
      ...
      this.view = view;
      this.done = done;
      this.lastViewportHeight = calculateViewportHeight(tui.terminal.rows || 30, 1, 1);
   ...
   private viewportHeight(width: number, snap?: TerminalSnapshot): number {
      const rows = this.tui.terminal.rows || 30;
      const headerLines = snap
         ? wrapDetailHeaderLines(
              `${statusGlyph(snap, this.theme)} ` +
                 this.theme.fg("accent", this.theme.bold(`${snap.id} · ${oneLine(snap.title)}`)) +
                 this.theme.fg("muted", ` · ${snap.status} · ${formatElapsed(snap)} · pid ${snap.pid ?? "?"}`) +
                 (snap.status !== "running" ? this.theme.fg("muted", ` · ${formatExit(snap)}`) : "") +
                 this.theme.fg("dim", ` · ${snap.cwd}`),
              width
           ).length
         : 1;
      const sanitizedCmd = snap ? oneLine(snap.command) : "";
      const cmdLines = snap ? wrapCommandLines(sanitizedCmd, width).length : 1;
      const hintText = `${configuredKeys(this.keybindings, "tui.select.cancel")} back · t stdout/stderr · x kill · ${configuredKeys(this.keybindings, "tui.editor.cursorUp")}/${configuredKeys(this.keybindings, "tui.editor.cursorDown")}/jk scroll · ${configuredKeys(this.keybindings, "tui.editor.pageUp")}/${configuredKeys(this.keybindings, "tui.editor.pageDown")} page · g/G top/bottom`;
      const hintLines = wrapHintLines(hintText, width).length;

      return calculateViewportHeight(rows, cmdLines, hintLines, headerLines);
   }
   ...
   render(width: number): string[] {
      ...
      lines.push(...wrapDetailHeaderLines(header, width));

      // Command row: wrap across multiple lines if needed without ellipsis.
      const sanitizedCmd = oneLine(snap.command);
      const wrappedCmdLines = wrapCommandLines(sanitizedCmd, width);
      for (const cmdLine of wrappedCmdLines) {
         lines.push(theme.fg("dim", cmdLine.slice(0, 2)) + theme.fg("text", cmdLine.slice(2)));
      }
      lines.push(border);
      ...
      const viewport = this.viewportHeight(width, snap);
      ...
      // Bottom hint: wrap without ellipsis
      const rawHint = `${configuredKeys(this.keybindings, "tui.select.cancel")} back · t stdout/stderr · x kill · ${configuredKeys(this.keybindings, "tui.editor.cursorUp")}/${configuredKeys(this.keybindings, "tui.editor.cursorDown")}/jk scroll · ${configuredKeys(this.keybindings, "tui.editor.pageUp")}/${configuredKeys(this.keybindings, "tui.editor.pageDown")} page · g/G top/bottom`;
      const wrappedHint = wrapHintLines(rawHint, width);
      for (const hintLine of wrappedHint) {
