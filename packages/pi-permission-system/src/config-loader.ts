...
export function normalizeUnifiedConfig(raw: unknown): {
   ...
   if (record.permissionMode === "default" || record.permissionMode === "auto" || record.permissionMode === "yolo") {
      config.permissionMode = record.permissionMode;
   } else if (typeof record.yoloMode === "boolean") {
      config.permissionMode = record.yoloMode ? "yolo" : "default";
   }
 ...
 * - Scalar fields (debugLog, permissionReviewLog, permissionMode, yoloMode) are replaced when
...
export function mergeUnifiedConfigs(
   ...
   for (const key of ["debugLog", "permissionReviewLog", "permissionMode", "yoloMode"] as const) {
      const value = override[key] ?? base[key];
      if (value !== undefined) {
         merged[key] = value as never;
      }
   }
   if (override.permissionMode === undefined && override.yoloMode !== undefined) {
      merged.permissionMode = override.yoloMode ? "yolo" : "default";
   }
