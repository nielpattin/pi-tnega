...
const TestRegistryLive = Layer.sync(BackendRegistry, () => {
  const backends: SubagentBackend[] = [
    ...
    makeStubBackend({
      backend: "pi",
      defaultModelLabel: "anthropic/claude-sonnet-4-5",
...
test("pi spawn fails fast without the parent model registry", async () => {
  // Directly instantiate real piBackend spawn without registry
  const realPi = piBackend;
  const effect = Effect.scoped(
    realPi.spawn({
      prompt: "needs a registry",
      title: "test",
      cwd: process.cwd(),
...
test("requestPopQueued pops last queued steer message", async () => {
  await withManager(async (manager, runtime) => {
    const snap = await runTool(
      runtime,
      manager.spawn("pi", task("Initial turn")),
    );
    // Wait for stub driver to dequeue the initial prompt
    await new Promise((resolve) => setTimeout(resolve, 60));
    // Send two steer messages while it's running
    await runTool(runtime, manager.send(snap.id, "steer message 1"));
    await runTool(runtime, manager.send(snap.id, "steer message 2"));

    const popped = manager.view.requestPopQueued(snap.id);
    assert.equal(popped, "steer message 2");

    const popped2 = manager.view.requestPopQueued(snap.id);
    assert.equal(popped2, "steer message 1");

    const popped3 = manager.view.requestPopQueued(snap.id);
    assert.equal(popped3, undefined);
