...
export async function openSubagentTakeover(
   ...
   await ctx.ui.custom<null>(
      (tui, theme, keybindings, done) => new TakeoverView(tui, theme, keybindings, id, view, done, options),
      {
         overlay: true,
...
class TakeoverView implements Component, Focusable {
   ...
   handleInput(data: string): void {
      if (matchesKey(data, "alt+up")) {
         const text = this.view.requestPopQueued(this.id);
         if (text != null && text !== "") {
