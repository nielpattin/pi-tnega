...
export const BACKEND_NAMES = ["pi", "agy"] as const;
 ...
 * Omitted = backend default (pi inherits the parent level).
 */
...
export interface SpawnTask {
   ...
   /**
    * Generic model hint:
    * pi: "provider/model-id" or bare model id.
    * agy: model slug (e.g. "gemini-3.6-flash-low"). Omitted = backend default / inherit.
    */
...
export interface SubagentMeta {
   ...
   /** pi session file path (agy has none). */
...
/**
 * Rebuild steering and followUp queue arrays after popping the most recent item.
 * Prefers the last steering message; if none, the last follow-up message.
 */
export function rebuildQueuesAfterPop(
   steering: readonly string[],
   followUp: readonly string[]
): { popped: string | undefined; steering: string[]; followUp: string[] } {
   if (steering.length > 0) {
      return {
         popped: steering[steering.length - 1],
         steering: steering.slice(0, -1),
         followUp: [...followUp]
      };
   }
   if (followUp.length > 0) {
      return {
         popped: followUp[followUp.length - 1],
         steering: [],
...
/**
 * Pop the most recently queued pending message.
 */
export function popLastFromPending(pending: readonly string[]): {
   popped: string | undefined;
   remaining: string[];
} {
   if (pending.length > 0) {
      return {
         popped: pending[pending.length - 1],
         remaining: pending.slice(0, -1)
      };
   }
   return {
      popped: undefined,
