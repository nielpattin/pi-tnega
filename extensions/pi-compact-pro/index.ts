...
function writeSettingsCompaction(
...
function persistSettings(
  ...
  return {
    settingsChanged: writeSettingsCompaction(
      reserveTokens,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const { settingsChanged, reserveTokens } = persistSettings(
      config,
      ctx.model,
  ...
  pi.registerCommand("compaction", {
    ...
    handler: async (args, ctx) => {
      ...
      if (subcommand && parts.length >= 2 && subcommand !== "") {
        ...
        ctx.ui.notify(
          `Updated: ${key}=${valueStr}\n` +
            (settingsChanged
              ? "Run /reload to apply settings.json changes."
              : "Applied immediately."),
          "info",
        );
        return;
      ...
      if (ctx.mode !== "tui") {
        ctx.ui.notify(
          "Interactive /compaction requires TUI mode. Use: /compaction <key> <value>",
          "error",
        );
        return;
      }
      ...
      await ctx.ui.custom<void>((tui, theme, _kb, done) => {
        ...
        const withCurrent = (presets: Option[], current: string, label: string): Option[] =>
          presets.some((p) => p.value === current)
            ? presets
            : [{ value: current, label, description: "Current value (custom)" }, ...presets];
        ...
        const maxContextChoices = (): Option[] =>
          withCurrent(
            ...
            config.maxContext === NO_CAP
              ? "No cap (native windows)"
              : formatTokens(config.maxContext),
          );
            ...
            [
              {
                ...
                description:
                  "Do not enforce an early compaction target; let context flow to native limit",
              },
        ...
        const buildItems = () => {
          ...
          const reserveTokens = model
            ...
            : isNoTarget
              ...
              : Math.max(
                  DEFAULT_NATIVE_RESERVE_TOKENS,
                  (config.maxContext === NO_CAP ? 1_000_000 : config.maxContext) -
                    config.compactionTarget,
                );
          const effectiveTarget = Math.max(1, effectiveWindow - reserveTokens);
          const effLabel = Number.isFinite(effectiveWindow)
          ...
          const maxContextDisplay =
            config.maxContext === NO_CAP
              ? "No cap (native windows)"
              : formatTokens(config.maxContext);
          const targetDisplay = isNoTarget
            ? "Native (flow normally)"
            : formatTokens(config.compactionTarget);
...
function overrideSizeChoices(current: number | undefined): Option[] {
  ...
  return [
    {
      value: currentString,
      label: formatTokens(current),
      description: "Current override (custom)",
    },
    ...presets,
  ];
}
...
function filterOptions(selectList: SelectList, rawOptions: Option[], raw: string): void {
  const internals = selectList as unknown as {
    items: Option[];
...
class OverrideSubmenu implements Component {
  ...
  private pushMain(): void {
    ...
    const options: Option[] = keys.map((key) => {
      ...
      return {
        value: `edit:${key}`,
        label: key,
        description: `${size} max context for this model`,
      };
    });
    if (keys.length > 0) {
      options.push({
        ...
        description: "Clear every per-model override",
      });
    }
    options.push({
      value: "add",
      label: "+ Add override",
      description: "Cap a specific model's native window",
    });
    this.levels = [
      {
    ...
    if (value.startsWith("edit:")) {
      ...
      this.pushLevel({
        ...
        options: [
          {
            value: "edit-size",
            label: "Edit max context",
            description: "Change the override size",
          },
          {
            value: "remove",
  ...
  private renderTop(): void {
    ...
    this.selectList = new SelectList(
      level.options,
      Math.min(level.options.length, 10),
      getSelectListTheme(),
...
class CompactionSettingsPanel extends Container {
  ...
  handleInput(data: string): void {
    const hasSubmenu = Boolean(
      (this.settingsList as unknown as { submenuComponent?: unknown }).submenuComponent,
    );
    if (
      !hasSubmenu &&
      (matchesKey(data, Key.escape) || data === "\x1b" || data === "escape" || data === "q")
    ) {
      this.onDone();
      return;
    }
