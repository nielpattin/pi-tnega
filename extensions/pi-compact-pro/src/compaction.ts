...
export function normalizeConfig(value: unknown): CompactionConfig {
  ...
  if (input.modelOverrides && typeof input.modelOverrides === "object") {
    for (const [key, rawOverride] of Object.entries(
      input.modelOverrides as Record<string, unknown>,
    )) {
...
export interface CompactionPreparationLike {
  ...
  tokensBefore: number;
  previousSummary?: string;
  fileOps: { read: ReadonlySet<string>; written: ReadonlySet<string>; edited: ReadonlySet<string> };
  settings: { reserveTokens: number; keepRecentTokens: number; enabled: boolean };
}
