...
function loadDescribeImageConfig(): DescribeImageConfig {
  ...
  try {
    ...
    const c = cfg as { model?: unknown; fallbackModel?: unknown; timeout?: unknown };
...
function resolveFromFilesystem(name: string, cwd: string): ResolvedImage | null {
  ...
  return { data: buf.toString("base64"), mimeType, source: absPath };
}
...
function resolveImage(name: string, ctx: ExtensionContext): ResolvedImage {
  ...
  if (!name || name === "clipboard") {
    ...
    throw new Error(
      `describe_image: no image found in session. Paste an image first, or provide a file path.`,
    );
  }
  ...
  throw new Error(
    `describe_image: could not find image "${name}". Pass an absolute path or paste an image first.`,
  );
}
...
async function analyzeWithModel(
  ...
  const result = await completeSimple(
    ...
    { apiKey: auth.apiKey, headers: auth.headers, signal, timeoutMs, maxTokens: 2048 },
  );
...
export default function describeImageExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (fallbackCheck.ok)
        attempts.push({ label: config.fallbackModel, model: fallbackCheck.model });
      ...
      async function tryAttempts(
        remaining: typeof attempts,
      ): Promise<ReturnType<typeof analyzeWithModel>> {
