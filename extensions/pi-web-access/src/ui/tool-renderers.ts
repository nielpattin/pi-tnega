...
export function renderSearchCall(args: unknown, theme: Theme): Component {
  ...
  const queryText = params.query
    ...
    : params.queries && params.queries.length > 0
      ...
      : "";
  const providerText =
    params.provider && params.provider !== "auto" ? ` [p:${params.provider}]` : "";
  ...
  return new Text(
    theme.fg("toolTitle", theme.bold("web_search ")) +
      theme.fg(
        "muted",
        `${queryText}${providerText}${modeText}${categoryText}${freshnessText}${limitText}`,
      ),
    0,
    0,
  );
...
export function renderSearchResult(
  ...
  if (!options.expanded) {
    if (count === 0 && !details?.answer) {
      return new Text(
        theme.fg("muted", `No results found for "${details?.query ?? ""}" via ${provider}`),
        0,
        0,
      );
    }
    ...
    if (details?.answer) {
      const answerSnippet =
        details.answer.length > 90 ? `${details.answer.slice(0, 87)}...` : details.answer;
...
export function renderFetchCall(args: unknown, theme: Theme): Component {
  ...
  const providerText =
    params.provider && params.provider !== "auto" && params.provider !== "local"
      ? ` [${params.provider}]`
      : "";
  const formatText =
    params.format && params.format !== "markdown" && params.format !== "text"
      ? ` [${params.format}]`
      : "";
...
export function renderResearchCall(args: unknown, theme: Theme): Component {
  ...
  return new Text(
    theme.fg("toolTitle", theme.bold("web_research ")) +
      theme.fg(
        "muted",
        `${queryText}${scopeText}${depthText}${providerText}${authorsText}${categoriesText}`,
      ),
    0,
    0,
  );
...
export function renderResearchResult(
  ...
  if (options.isPartial) {
    ...
    if (activities.length === 0) {
      return new Text(
        theme.fg("warning", `${spinner} Initializing deep web research...`) +
          theme.fg("dim", durationText),
        0,
        0,
      );
    }
    ...
    const lines: string[] = [
      theme.fg("warning", `${spinner} Deep Web Research in progress...`) +
        theme.fg("dim", durationText),
    ];
...
export function renderOutlineCall(args: unknown, theme: Theme): Component {
  ...
  return new Text(
    theme.fg("toolTitle", theme.bold("outline_site ")) +
      theme.fg("muted", `${targetText}${searchText}${limitText}`),
    0,
    0,
  );
}
