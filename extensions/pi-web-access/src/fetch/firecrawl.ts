...
export async function fetchWithFirecrawl(url: string, options: FetchOptions): Promise<FetchResult> {
  ...
  const formatsPayload: Array<{ type: string }> = [
    { type: format === "html" ? "html" : "markdown" },
  ];
...
export async function parseLocalFileWithFirecrawl(
  ...
  if (!apiKey) {
    return {
      ...
      provider: "firecrawl",
      error:
        "FIRECRAWL_API_KEY is required to parse Office/eBook documents (.docx, .xlsx, .pptx, .epub, .odt, .rtf)",
    };
  }
  ...
  try {
    ...
    formData.append(
      "options",
      JSON.stringify({ formats: [{ type: "markdown" }], onlyMainContent: true }),
    );
