...
async function handleLocalFileFetch(
  inputPath: string,
...
async function executeRemoteHttpFetch(
...
export async function fetchWebContent(options: FetchOptions): Promise<FetchResult> {
  ...
  if (isLocalFilePath(inputUrl)) {
    ...
    return { ...localRes, durationMs: Date.now() - startTime };
  }
  ...
  const finalResult: FetchResult = { ...result, durationMs: Date.now() - startTime };
