...
interface ExaAgentRunResponse {
  ...
  output?: { text?: string; structured?: unknown; grounding?: ExaAgentRunGroundingItem[] };
...
export async function researchExa(options: ResearchOptions): Promise<ResearchResponse> {
  ...
  try {
    ...
    const startRes = await fetchWithTimeout("https://api.exa.ai/agent/runs", {
      ...
      body: JSON.stringify({ query: queryWithInstructions, effort }),
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      if (run.status === "completed") {
        ...
        return { query, provider: "exa", synthesis: text, sources, activities };
      }
      ...
      if (run.status === "failed" || run.status === "cancelled") {
        return {
          ...
          error:
            run.error?.message ||
            `Exa Agent run ${run.status} (reason: ${run.stopReason || "unknown"})`,
        };
      }
    }
