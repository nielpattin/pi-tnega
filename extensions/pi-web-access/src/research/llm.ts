...
export async function resolveCandidateModels(
  ctx: ExtensionContext,
): Promise<AuthenticatedModelCandidate[]> {
...
async function invokeLLMSynthesis(
  ...
  const prompt = buildPromptForLLM(
    mainQuery,
    systemPrompt,
    depth,
    queryResults,
...
export async function researchLLM(
  ...
  try {
    ...
    const searchPromises = queryList.map(async (queryAngle) => {
      ...
      return { query: queryAngle, response };
    });
    ...
    for (const res of settled) {
      ...
      queryResults.push({ query: queryAngle, items: validItems, answer: response.answer });
    }
    ...
    if (llmResult) {
      ...
      providerLabel =
        searchEnginesUsed.length > 0 ? `llm (${searchEnginesUsed.join(", ")})` : "llm";
    }
