...
function buildAcademicDeterministicSynthesis(
  ...
  if (candidates.length === 0) {
    lines.push(
      "No academic papers matching the query criteria were discovered in the research index.",
    );
    return lines.join("\n");
  }
...
function buildAcademicPromptForLLM(
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    const cand = candidates.find(
      (c) =>
        (c.paper.primaryId || c.paper.paperId) === src.primaryId || c.paper.paperId === src.paperId,
    );
  ...
  for (const { model, apiKey, headers } of candidateModels) {
    try {
      ...
      if (synthesisText.length > 0) {
        return { text: synthesisText, modelName: `${model.provider}/${model.id}` };
      }
    } catch (err) {
...
export async function researchAcademic(
  ...
  try {
    ...
    emitUpdate(
      "plan",
      `Searching academic paper index across ${searchQueries.length} query angles...`,
    );
    ...
    for (const query of searchQueries) {
      ...
      for (const p of searchRes.results) {
        candidates.push({ paper: p, sourceKind: "search" });
      }
      ...
      for (const seed of topSeeds) {
        ...
        if (relatedRes.success && relatedRes.results.length > 0) {
          for (const rel of relatedRes.results) {
            candidates.push({ paper: rel, sourceKind: "citation-expansion" });
          }
          candidates = deduplicatePapers(candidates);
          emitUpdate(
            "expand",
    ...
    if (passageCandidates.length > 0 && !options.signal?.aborted) {
      emitUpdate(
        "passages",
        `Reading relevant full-text passages across ${passageCandidates.length} papers...`,
      );
