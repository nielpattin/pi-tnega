...
export async function searchExa(options: SearchOptions): Promise<SearchResponse> {
  ...
  if (!apiKey) {
    return { query, provider: "exa", results: [], error: "EXA_API_KEY is not configured" };
  }

  const limit = options.limit ?? 5;
  const isRestrictedCategory = options.category === "company" || options.category === "people";
  const startPublishedDate = isRestrictedCategory
    ? undefined
    : getPublishedDateCutoff(options.freshness);
  ...
  try {
    if (isAnswerMode) {
      ...
      if (answerRes.ok) {
        ...
        const results: SearchItem[] = citations
          .slice(0, limit)
          .map((c) => ({
    ...
    const response = await fetchWithTimeout("https://api.exa.ai/search", {
      ...
      body: JSON.stringify({
        ...
        userLocation,
        systemPrompt: options.systemPrompt,
        contents: { text: { maxCharacters: 1500 }, summary: true, highlights: true },
      }),
      signal: options.signal,
    });
