...
export async function searchTavily(options: SearchOptions): Promise<SearchResponse> {
  ...
  if (!apiKey) {
    return { query, provider: "tavily", results: [], error: "TAVILY_API_KEY is not configured" };
  }
  ...
  try {
    const response = await fetchWithTimeout("https://api.tavily.com/search", {
      ...
      body: JSON.stringify({ api_key: apiKey, query, max_results: limit, include_answer: true }),
    ...
    return { query, provider: "tavily", results, answer: data.answer };
  } catch (error) {
    return {
      query,
      provider: "tavily",
