...
export function resolvePaperUrl(idOrUrl: string, fallback?: string): string {
  ...
  return (
    fallback ?? `https://api.firecrawl.dev/v2/search/research/papers/${encodeURIComponent(trimmed)}`
  );
}
...
export async function searchResearchPapers(
  ...
  if (!apiKey) {
    return { success: false, results: [], error: "FIRECRAWL_API_KEY is not configured" };
  }
  ...
  try {
    ...
    return { success: false, results: [], error: `Firecrawl Paper Search failed: ${errorMsg}` };
  }
}
...
export async function getPaperOrPassages(
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    return { success: false, error: `Firecrawl Get Paper failed: ${errorMsg}` };
  }
}
...
export async function findRelatedPapers(
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    return { success: false, results: [], error: `Firecrawl Related Papers failed: ${errorMsg}` };
  }
}
