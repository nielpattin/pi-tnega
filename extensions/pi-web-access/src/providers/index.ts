...
export const PROVIDER_PRIORITY_ORDER: ReadonlyArray<SearchProviderId> = [
  "firecrawl",
  "exa",
  "tavily",
];
...
export function getAvailableProviders(): SearchProviderId[] {
  return Object.keys(PROVIDERS).filter((id) =>
    isProviderAvailable(id as SearchProviderId),
  ) as SearchProviderId[];
}
...
export async function executeSearch(options: SearchOptions): Promise<SearchResponse> {
  ...
  for (const providerId of candidates) {
    ...
    try {
      ...
      errors.push({
        provider: providerId,
        error: response.error || "Zero search results returned",
      });
    } catch (err) {
