...
export interface WebAccessConfigFile {
  ...
  search?: { defaultProvider?: SearchProviderId; userLocation?: string; limit?: number };
  research?: {
    provider?: "llm" | "exa";
    model?: string;
  ...
  fetch?: { provider?: FetchProviderId; maxBytes?: number; timeoutMs?: number; userAgent?: string };
  keys?: { firecrawl?: string; exa?: string; tavily?: string };
...
export function getWebAccessConfig(): WebAccessConfig {
  ...
  const parsedMaxBytes = Number.parseInt(
    env.PI_WEB_ACCESS_DEFAULT_MAX_BYTES ??
      String(fileConfig.fetch?.maxBytes ?? fileConfig.maxBytes ?? ""),
    10,
  );
  const parsedTimeout = Number.parseInt(
    env.PI_WEB_ACCESS_TIMEOUT_MS ??
      String(fileConfig.fetch?.timeoutMs ?? fileConfig.timeoutMs ?? ""),
    10,
  );
  ...
  const exaKey =
    env.EXA_API_KEY ||
    fileConfig.keys?.exa ||
    fileConfig.exaApiKey ||
    extractApiKey(authData, "exa");

  const tavilyKey =
    env.TAVILY_API_KEY ||
    fileConfig.keys?.tavily ||
    fileConfig.tavilyApiKey ||
    extractApiKey(authData, "tavily");
  ...
  const userLocation =
    env.PI_WEB_ACCESS_USER_LOCATION || fileConfig.search?.userLocation || fileConfig.userLocation;
  ...
  const researchModel =
    env.PI_WEB_RESEARCH_MODEL || fileConfig.research?.model || fileConfig.researchModel;
  const researchModelFallbacks =
    fileConfig.research?.modelFallbacks || fileConfig.researchModelFallbacks;
  ...
  const maxBytes =
    Number.isFinite(parsedMaxBytes) && parsedMaxBytes > 0 ? parsedMaxBytes : DEFAULT_MAX_BYTES;
  const timeoutMs =
    Number.isFinite(parsedTimeout) && parsedTimeout > 0 ? parsedTimeout : DEFAULT_TIMEOUT_MS;
  const userAgent =
    env.PI_WEB_ACCESS_USER_AGENT ||
    fileConfig.fetch?.userAgent ||
  ...
  const fetch: FetchConfig = { provider: fetchProvider, maxBytes, timeoutMs, userAgent };

  const keys: KeysConfig = { firecrawl: firecrawlKey, exa: exaKey, tavily: tavilyKey };
