...
export default async function codexUsageExtension(pi: ExtensionAPI): Promise<void> {
  ...
  const setVerbosity = async (
    nextVerbosity: CodexVerbosity,
  ...
  pi.registerCommand("codex-usage", {
    ...
    getArgumentCompletions: (prefix) => {
      ...
      const query = (
        verbosityPrefix ? normalizedPrefix.slice("verbosity ".length) : normalizedPrefix
      ).trim();
    ...
    handler: async (args, ctx) => {
      ...
      if (tokens.length > 0) {
        ...
        if (isVerbosityCommand && !requestedVerbosity) {
          if (!ctx.hasUI) {
            ctx.ui.notify(
              `Current Codex verbosity: ${config.verbosity}. ${CODEX_USAGE_COMMAND_USAGE}`,
              "info",
            );
            return;
          }
          const selectedVerbosity = await ctx.ui.select("Codex response verbosity", [
            ...CODEX_VERBOSITY_LEVELS,
          ]);
