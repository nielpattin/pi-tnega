  ...
  const visit = (current: unknown, depth: number): unknown => {
    ...
    if (Array.isArray(current)) {
      ...
      for (const key of Object.keys(current).toSorted())
        sorted[key] = visit(current[key], depth + 1);
      result = sorted;
    } else {
...
export function durableBlockId(
  ...
  if (kind === "tool_result") {
    ...
    return typeof callId === "string" && callId.length > 0
      ? `r:${callId}`
      : `r:h${messageHash(message)}`;
  }
  if (kind === "tool_call") {
    const block =
      Array.isArray(message.content) && contentIndex !== undefined
...
function parseOneMessage(
  ...
  const add = (
    ...
    extra: Pick<
      ParsedBlock,
      "toolCallId" | "toolName" | "arguments" | "fullOutputPath" | "isError"
    > = { isError: false },
  ): void => {
  ...
  if (summary || content !== undefined)
    add("other", summary, undefined, undefined, hasOpaqueContent(content));
  return blocks;
}
...
export function parseMessages(
  ...
  for (const callId of callIds) {
    ...
    pairs.set(callId, { assistantBlockId: call.id, resultBlockId: result.id });
  }
...
export function resultBlockFor(
