...
const PROGRESS_RE =
  /\b(?:decided|chose|selected|implemented|completed|done|fixed|progress|verified|tested|working)\b/i;
const NEXT_RE =
  /\b(?:next|todo|to do|remaining|follow[- ]?up|run the tests|verify|fix|check|needs to)\b/i;
      ...
      if (block.isError)
        ...
        addUnique(
          ...
          [
            sourced(
              block,
              `[tool result ${block.toolName ?? "tool"}] ${cleanLine(block.text, 220)}`,
            ),
          ],
          limits.maxBriefLines,
        );
...
export function compileCompactionSummary(
  input: CompactionInput,
): Result<CompactionOutput, CompactionError> {
  if (!Array.isArray(input.messagesToSummarize)) {
    return {
      ok: false,
      error: { _tag: "CompactionError", message: "Compaction messages must be an array" },
    };
  }
  const limits = {
    ...
    maxBriefLines: Math.max(
      1,
      Math.floor(input.limits?.maxBriefLines ?? DEFAULT_LIMITS.maxBriefLines),
    ),
    maxRecentTailChars: Math.max(
      0,
      Math.floor(input.limits?.maxRecentTailChars ?? DEFAULT_LIMITS.maxRecentTailChars),
    ),
  };
  ...
  const sectionLines: Array<[CompactionSectionName, string[]]> = [
    ...
    [
      "Decisions and Progress",
      section("Decisions and Progress", sections.decisions, limits.maxItemsPerSection),
    ],
    [
      "Errors and Blockers",
      section("Errors and Blockers", sections.errors, limits.maxItemsPerSection),
    ],
    ...
    [
      ...
      section(
        "Chronological Brief",
        sections.brief.slice(0, limits.maxBriefLines),
        limits.maxBriefLines,
      ),
    ],
    [
      "Recent Verbatim Tail",
      sections.recent.length > 0 ? section("Recent Verbatim Tail", sections.recent, 1) : [],
    ],
  ];
  ...
  return {
    ok: true,
    value: { summary, sections: present, estimatedTokens: Math.ceil(summary.length / 4) },
  };
}
...
export function mergeCompactionSummaries(previous: string | undefined, fresh: string): string {
  ...
  for (const key of keys)
    addUnique(result[key], [...previousSections[key], ...freshSections[key]], 100);
  const lines: string[] = [
    "# Pi Constellation checkpoint",
    "# Deterministic extraction only. Verify details against the persisted session history.",
  ];
