  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      ...
      const target = sessionId
        ? theme.fg("accent", clip(sessionId, 24))
    ...
    renderResult(result, { expanded, isPartial }, theme, context) {
      ...
      if (isPartial)
        return new Text(theme.fg("warning", `↳ ${text || "Inspecting session..."}`), 0, 0);
      if (context.isError)
        return new Text(theme.fg("error", text || "Session inspection failed"), 0, 0);
    ...
    parameters: Type.Object({
      ...
      branchId: Type.Optional(
        Type.String({
          description: "Branch ID returned by overview, or active/all where supported",
        }),
      ),
      messageId: Type.Optional(
        Type.String({ description: "Message ID returned by another inspection action" }),
      ),
      query: Type.Optional(Type.String({ description: "Search words or phrase" })),
      cursor: Type.Optional(
        Type.String({ description: "Message cursor returned by a branch page" }),
      ),
      ...
      try {
        ...
        if (action === "overview" || action === "branches") {
          ...
          if (page.messages.length === 0)
            return errorResult(`No branch page found for: ${selectedBranch}`);
          text = [
          ...
          if (context.messages.length === 0)
            return errorResult(`Message not found: ${params.messageId}`);
        ...
        return errorResult(
          `session_inspect failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
  });
