...
export type SessionNavigator = {
  ...
  readonly around: (
    messageId: string,
    options: { readonly before: number; readonly after: number },
  ) => MessageContext;
...
export function createSessionNavigator(entries: ReadonlyArray<SessionFileEntry>): SessionNavigator {
  const identified = entries.filter((entry): entry is SessionFileEntry & { readonly id: string } =>
    Boolean(entry.id),
  );
  const byId = new Map(identified.map((entry) => [entry.id, entry] as const));
  const sourceToEntryId = new Map(
    identified.flatMap((entry) =>
      sourceReferences(entry).map((sourceRef) => [sourceRef, entry.id] as const),
    ),
  );
  const resolveMessageId = (id: string): string => sourceToEntryId.get(id) ?? id;
  const parentIds = new Set(
    identified.flatMap((entry) => (entry.parentId ? [entry.parentId] : [])),
  );
  ...
  return {
    ...
    around: (messageId, options) => {
      ...
      const branch = activeBranch?.entries.some((entry) => entry.id === resolvedMessageId)
        ...
        : branches.find((candidate) =>
            candidate.entries.some((entry) => entry.id === resolvedMessageId),
          );
    ...
    search: (query, id, maxResults) => {
      ...
      for (const branch of branches) {
        const recall = searchSessionEntries(branch.entries, query, {
          maxResults,
          contextRadius: 1,
        });
        for (const result of recall.results) {
          ...
          if (!existing || result.score > existing.score)
            uniqueResults.set(result.matched.id, result);
        }
      }
      const ranked = [...uniqueResults.values()].toSorted(
