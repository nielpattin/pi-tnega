...
export function shouldFallbackRenameError(error: unknown): boolean {
...
export function writeFileAtomic(filePath: string, content: string, options: WriteFileAtomicOptions = {}) {
   fs.mkdirSync(path.dirname(filePath), { recursive: true });
   const temporary = `${filePath}.${process.pid}.${Date.now()}.tmp`;
   const rename = options.renameFn ?? fs.renameSync;
   try {
      fs.writeFileSync(temporary, content, { encoding: "utf8", mode: 0o600 });
      try {
         rename(temporary, filePath);
      } catch (error) {
         if (shouldFallbackRenameError(error)) {
