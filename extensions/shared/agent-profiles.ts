...
/** Harness values retained for compatibility with existing profile files. */
export type AgentHarness = "pi" | "agy";
...
export interface AgentProfile {
   ...
   /** Profile file compatibility field. Workflows execute Pi profiles only. */
...
function parseThinking(value: string | undefined): AgentThinkingLevel | undefined {
   const levels: ReadonlyArray<AgentThinkingLevel> = ["off", "minimal", "low", "medium", "high", "xhigh", "max"];
   return value && levels.includes(value as AgentThinkingLevel) ? (value as AgentThinkingLevel) : undefined;
}
...
function parseProfile(
   ...
   return {
      ...
      ...(thinking ? { thinking } : {}),
      harness: parsed.metadata.get("harness") === "agy" ? "agy" : "pi",
...
export function serializeAgentProfile(profile: AgentProfile): string {
   ...
   lines.push(`harness: ${profile.harness || "pi"}`);
