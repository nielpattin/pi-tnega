/**
 * pi-worker-flows: model-authored multi-agent orchestration.
 *
...
import { runWorkflowSandbox } from "./core/sandbox.ts";
import { safeStringify, writeFileAtomic } from "./utils/serialization.ts";
import { registerWorkersExtension } from "./workers/extension.ts";
...
export function isWorkflowSettingsListNavigation(
   data: string,
   keybindings: Pick<KeybindingsManager, "matches">
): boolean {
   const up = data === "up" || matchesKey(data, Key.up) || keybindings.matches(data, "tui.select.up");
   const down = data === "down" || matchesKey(data, Key.down) || keybindings.matches(data, "tui.select.down");
   const confirm = data === "enter" || matchesKey(data, Key.enter) || keybindings.matches(data, "tui.select.confirm");
   const cancel = data === "escape" || matchesKey(data, Key.escape) || keybindings.matches(data, "tui.select.cancel");
   return up || down || confirm || cancel;
}

function normalizeWorkflowSettingsListInput(data: string): string {
   if (data === "up") return "\u001b[A";
   if (data === "down") return "\u001b[B";
   if (data === "enter" || data === "\n") return "\r";
   if (data === "escape") return "\u001b";
   return data;
}
...
class WorkflowSettingsPanel {
   ...
   handleInput(data: string): void {
      if (matchesKey(data, Key.escape) || data === "escape") {
         this.cancelPicker();
         return;
      }
      ...
      if (isSearch && this.filterInput) {
         const navigation = isWorkflowSettingsListNavigation(data, this.keybindings);
         if (navigation) this.selectList.handleInput(normalizeWorkflowSettingsListInput(data));
         else {
   ...
   render(width: number): string[] {
      ...
      const contentLines = [this.theme.bold(this.theme.fg("accent", title)), this.theme.fg("muted", description), ""];
      if (isSearch && this.filterInput) {
         contentLines.push(this.theme.fg("muted", "Search models by provider or model id:"));
         contentLines.push(...this.filterInput.render(contentWidth));
         contentLines.push("");
      }
      contentLines.push(...this.selectList.render(contentWidth));
      ...
      for (const line of contentLines) {
         const padded =
            truncateToWidth(line, contentWidth, "") + " ".repeat(Math.max(0, contentWidth - visibleWidth(line)));
         bordered.push(this.theme.fg("border", "│") + ` ${padded} ` + this.theme.fg("border", "│"));
      }
...
async function openWorkflowSettings(ctx: ExtensionContext): Promise<void> {
   await ctx.ui.custom<void>(
      (tui, theme, keybindings, done) => new WorkflowSettingsPanel(ctx, tui, theme, keybindings, done),
      {
         overlay: true,
...
export default function workflows(pi: ExtensionAPI) {
   ...
   const workerRegistration = registerWorkersExtension(pi, {
      openDashboard: async (ctx, runtime, initialTab) => {
         await showRunsDashboard(ctx, {
            workerRuntime: runtime,
            initialTab,
            workflow: {
               getActive: activeDetails,
               sessionId: ctx.sessionManager.getSessionId(),
               referencedRunIds: sessionWorkflowRunIds(ctx),
               getActiveAgentSession: (runId, agentIndex) => activeRuns.get(runId)?.childSessions?.get(agentIndex),
               abortActiveAgent: (runId, agentIndex) => {
               ...
               openWorkflowSettings: () => openWorkflowSettings(ctx)
            }
         });
         completedRuns = 0;
         failedRuns = 0;
      ...
      handler: async (_rawArgs, ctx) => {
         if (ctx.hasUI) {
            await openAgentsPanel(ctx, undefined, {
               ...
               onAgentsChanged: () =>
                  workerRegistration.ok ? workerRegistration.refreshWorkerSpawnTool(ctx.cwd) : undefined
            });
            return;
         }
   ...
   pi.registerCommand("wf", {
      ...
      handler: async (rawArgs, ctx) => {
         ...
         if (ctx.mode === "tui") {
            lastUi = ctx.ui;
            if (!workerRegistration.ok) {
               ctx.ui.notify("Worker runs dashboard is unavailable.", "error");
               return;
            }
            await showRunsDashboard(ctx, {
               workerRuntime: workerRegistration.runtime,
               initialTab: "workflow",
               workflow: {
                  getActive: activeDetails,
                  sessionId: ctx.sessionManager.getSessionId(),
                  referencedRunIds: sessionWorkflowRunIds(ctx),
                  initialRunId: arg || undefined,
