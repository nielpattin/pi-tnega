            ...
            if (target !== currentParent) {
               return yield* Effect.fail(
                  new ParentSessionActivationError({
                     message: "Workers parent session mismatch: recovery target changed during activation.",
...
export const activateParentSession = Effect.fn("activateParentSession")(function* (
   ...
   return yield* gate.runExclusive(
      Effect.gen(function* () {
         ...
         // Stop worker sessions and live-output state owned by the previous
         // parent before replacing the registry. Late callbacks must not update
         // the new or ephemeral parent registry.
         yield* workerManager.cancelActiveSessions;
         ...
         Effect.matchEffect({
            onFailure: (error) =>
               Effect.gen(function* () {
                  ...
                  yield* Effect.ignore(workerManager.cancelActiveSessions);
...
export const ensureParentSessionRecovery = Effect.fn("ensureParentSessionRecovery")(function* (
   ...
   // worker jobs can both spawn without reactivating and clearing each
