...
import { persistWorkflowJson, recoverWorkflowDetails } from "../core/artifacts.ts";
import { enterAlternateScreen } from "../shared/alternate-screen.ts";
import { transcriptFromMessages } from "../core/runner.ts";
import {
   agentContext,
   countStates,
   formatAgentModel,
...
import {
   ...
   buildTranscriptCopyBody,
   buildTranscriptRows,
   normalizeWorkflowTranscript
} from "../shared/transcript-ui.ts";
export { toolMarkerColor } from "../shared/transcript-ui.ts";
...
export function enterWorkflowAlternateScreen(tui: TUI, keybindings: KeybindingsManager, screen: Component): () => void {
   ...
   const releaseAlternateScreen = enterAlternateScreen(tui, screen, {
      useAlternateScreen,
      attachScreen: useAlternateScreen,
      enterSequence: ALTERNATE_SCREEN_ENTER,
      exitSequence: ALTERNATE_SCREEN_EXIT
   });
...
export function wrapSelection(index: number, delta: number, length: number): number {
   if (length === 0) return 0;
   return (index + delta + length) % length;
}

export function matchesWorkflowDirection(
   data: string,
   direction: "up" | "down",
   keybindings?: KeybindingsManager
): boolean {
   const binding = direction === "up" ? "tui.select.up" : "tui.select.down";
   return data === direction || matchesKey(data, direction) || keybindings?.matches(data, binding) === true;
}
...
export function buildWorkflowTranscriptPayload(
   ...
   const body = buildTranscriptCopyBody(normalizeWorkflowTranscript(agent.transcript)) || "(no transcript recorded)";
...
export class WorkflowDashboard {
   ...
   private transcriptRows(agent: AgentRecord, width: number): string[] {
      ...
      if (agent.systemPrompt) {
         rows.push(
            ...buildSystemPromptRows(agent.systemPrompt, Math.max(1, width - 2), theme, this.showSystemPrompt, {
               label: "[System prompt]",
               toggleKey: "Ctrl+S"
            })
         );
      ...
      rows.push(
         ...buildTranscriptRows(normalizeWorkflowTranscript(agent.transcript), contentWidth, theme, {
