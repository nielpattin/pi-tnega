...
This document describes the code logic and approved target design of the `pi-worker-flows` extension. It is not only a description of the terminal UI.
...
- `extensions/pi-worker-flows/src/core/artifacts.ts`
- `extensions/pi-worker-flows/src/core/model.ts`
- `extensions/pi-worker-flows/src/core/meta.ts`
...
- The `workflow` tool for phased orchestration.
...
The main lifecycle is implemented in the `workflow` tool handler in `src/extension.ts`.
...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `index.ts`                         | Package entry point that re-exports workflow and direct worker APIs.                                                                                                          |
...
| `src/core/artifacts.ts`            | Workflow-folder persistence, coalesced snapshots, atomic flushes, `events.jsonl` journaling, backups, leases, and recovery transformation.                                    |
| `src/core/model.ts`                | Workflow and Worker state types, usage aggregation, phase grouping, formatting, and result bounding for display.                                                              |
| `src/core/meta.ts`                 | Static Acorn parsing, literal-only metadata decoding, metadata sanitization, phase route defaults, reusable-source metadata, and executable-source preparation.               |
...
| `src/utils/usage.ts`               | Provider usage aggregation for child sessions.                                                                                                                                |
| `src/shared/child-session-dir.ts`  | Parent-scoped child session directory derivation shared by both execution modes.                                                                                              |
...
- [Run controller](../src/core/controller.ts)
- [Workflow runner](../src/core/runner.ts)
- [Sandbox bridge](../src/core/sandbox.ts)
- [Sandbox child](../src/core/sandbox-child.cjs)
- [Workflow tool and lifecycle](../src/extension.ts)
