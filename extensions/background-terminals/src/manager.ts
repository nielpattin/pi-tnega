...
function shellInvocation(command: string): ShellInvocation {
   if (process.platform === "win32") {
      ...
      if (shPath) {
         return { shell: shPath, args: ["-c", command] };
      }
...
function killTree(child: ChildProcess, signal: NodeJS.Signals) {
   if (process.platform === "win32" && child.pid) {
      try {
         // taskkill /T /F kills the whole tree forcefully and immediately.
         // Non-force /T only sends WM_CLOSE, which leaves node/grandchildren
         // running and causes pipe hangs; always force on Windows.
...
const makeManager = Effect.gen(function* () {
   ...
   const makeSpill = (entry: () => Entry | undefined, id: string, stream: "stdout" | "stderr") => {
