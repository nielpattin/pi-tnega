...
const required = [...entries, ...declarations];
...
const initialSource = [...startupFiles].map((file) => readFileSync(file, "utf8")).join("\n");
for (const forbidden of ["src/raft-runtime-state.ts", "src/ui/settings.ts", 'from "mcporter"']) {
...
if (mandatoryPowerShellFactoryImport.test(lazySource)) {
  throw new Error("Optional Pi PowerShell factory must be accessed through the module namespace");
}
for (const expected of ["src/raft-runtime-state.ts", "src/ui/settings.ts", 'import("mcporter")']) {
...
await Promise.all(
  stable
    .filter((file) => file !== "worker.js")
    .map((file) => import(new URL(`../dist/${file}`, import.meta.url))),
);
console.log(
