...
vi.mock("../src/raft-runtime-state.js", () => ({
  RaftRuntimeState: class {
    initialized = true;
    widgetDismissedAt = 0;
...
vi.mock("../src/ui/settings.js", () => ({
  openRaftSettings: vi.fn(
    async (_context: ExtensionContext, deps: { onConfigApplied?: (id: string) => void }) => {
...
const commandHandlerOf = (pi: ExtensionAPI): CommandHandler => {
  const registerCommand = (pi as unknown as { registerCommand: ReturnType<typeof vi.fn> })
    .registerCommand;
...
const raftToolOf = (registeredTools: unknown[]): RaftExecTool => {
  const tool = registeredTools.find(
    (candidate) => (candidate as { name?: string }).name === "raft_exec",
  ) as RaftExecTool | undefined;
...
describe("Raft tool display lifecycle", () => {
  it("drops abandoned-branch invalidators when session_tree rebuilds the transcript", async () => {
    ...
    // settings apply path (openRaftSettings -> onConfigApplied ->
  ...
  it("does not leak the previous session's preference when a re-bootstrap fails", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-failed-rebootstrap-"));
    ...
    try {
      ...
      await expect(
        emit(handlers, "session_start", contextFor(cwdB, "session-b")),
      ).rejects.toThrow();
  ...
  it("renders resumed history compact from bootstrapped config without activating the runtime", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-resume-compact-"));
