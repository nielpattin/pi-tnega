...
const service = (value: string): RaftProvider => ({
  name: "svc",
...
const observableStatus = (supervisor: RaftComponentSupervisor, id: string) => {
...
describe("component calculus laws", () => {
  it("commutes whether a dependency or its consumer is inserted first", async () => {
    const run = async (providerFirst: boolean) => {
      ...
      const consumer: RaftComponentDefinition = {
        name: "consumer",
        requires: ["svc.read"],
      ...
      const result = { status: observableStatus(supervisor, "consumer"), observations };
      ...
      { id: "owner", component: "owner" },
      {
        ...
        async activate(context) {
          await context.effect(
            ...
            { label: "owned mutation", resources: ["law:owned"], ordering: "ordered" },
          );
  ...
  it("cannot register an orphan child from activation rollback", async () => {
    ...
    expect(errors).toEqual(["Raft component root can only use children while activating"]);
    ...
    await supervisor.start(
      { id: "parent", component: "parent" },
      {
        ...
        activate(context) {
          ...
          context.use({ name: "healthy-child", activate() {} }, { id: "healthy" });
        },
      },
    );
    await supervisor.settle();
          ...
          activate(context) {
            for (let index = 0; index < 257; index++) {
              context.defer(
                ...
                { label: `effect-${index}`, resources: [`resource:${index}`], ordering: "ordered" },
              );
            }
          },
        },
  ...
  it("does not let a revertible fiber relabel an emission as a tracked inverse", async () => {
    ...
    await expect(
      supervisor.start(
        { id: "unsafe", component: "unsafe" },
        {
          name: "unsafe",
          guarantee: "revertible",
          async activate(context) {
            await context.effect(() => () => {}, { kind: "emission", label: "outside boundary" });
          },
        },
      ),
    ...
    await expect(
      supervisor.start(
        { id: "deferred", component: "deferred" },
        {
          name: "deferred",
          guarantee: "revertible",
          activate(context) {
            context.defer(
              ...
              { kind: "emission", label: "already installed" },
            );
          },
        },
      ),
