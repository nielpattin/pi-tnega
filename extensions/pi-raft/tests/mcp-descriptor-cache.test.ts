...
const temporaryDirectory = (): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-mcp-cache-"));
...
describe("config layer fingerprints", () => {
  ...
  it("treats identical stat lists as equal and rejects drift", () => {
    ...
    expect(sameConfigLayers([layer], [{ ...layer, mtimeMs: layer.mtimeMs + 0.4 }])).toBe(true);
...
describe("McpDescriptorCacheStore", () => {
  it("round-trips a cache document", async () => {
    ...
    const store = new McpDescriptorCacheStore(
      path.join(directory, ".pi", "raft", "mcp-cache.json"),
    );
    const document = {
      version: 1,
