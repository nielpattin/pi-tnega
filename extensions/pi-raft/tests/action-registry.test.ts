...
import { ActionRegistry, type RaftCallAudit } from "../src/core/action-registry.js";
...
const actionProvider = (...names: string[]): RaftProvider => ({
  name: "demo",
  description: "Demo provider",
  async list() {
...
const invokeContext = (audits: RaftCallAudit[] = []) => ({
  ...context,
  approve: async () => {},
  audits,
...
describe("ActionRegistry", () => {
  it("lists, searches, describes, and invokes providers", async () => {
    ...
    const result = await registry.invoke(
      "demo.echo",
      { value: "hello" },
      { ...context, approve, audits, maxResultChars: 10_000 },
    );
    expect(result).toBe("hello");
    expect(approve).toHaveBeenCalledOnce();
    expect(audits).toMatchObject([
      { ref: "demo.echo", provider: "demo", tool: "echo", args: { value: "hello" }, success: true },
    ]);
  });

  it("uses configured risk overrides when resolving provider actions", async () => {
    const registry = new ActionRegistry();
    registry.register(provider());
    registry.setRiskOverrides({ "demo.echo": "network" });
    const action = await registry.describe("demo.echo", context);
    expect(action).toMatchObject({ risk: "network", effect: { kind: "emission" } });
    const approve = vi.fn(async () => {});
    ...
    expect(approve).toHaveBeenCalledWith(
      expect.objectContaining({ ref: "demo.echo", risk: "network" }),
      { value: "hello" },
    );
    registry.setRiskOverrides({});
    expect((await registry.describe("demo.echo", context)).risk).toBe("read");
  });
  ...
  it("leaves ambiguous synonyms unrepaired and names the candidates", async () => {
    ...
    await expect(registry.invoke("demo.say", {}, invokeContext())).rejects.toThrow(
      "Unknown Raft action: demo.say (did you mean: demo.ask, demo.tell?)",
    );
  ...
  it("keeps the plain unknown-action error when nothing is close", async () => {
    ...
    expect(error?.message).toContain("Unknown Raft action: demo.missing");
  });
  ...
  it("suggests close declared names for bare unknown actions", async () => {
    ...
    await expect(registry.describe("recal", context)).rejects.toThrow(
      "Unknown Raft action: recal (did you mean: recall?)",
    );
  });
  ...
  it("builds deterministic provider/action heads and searches the complete catalog before ranking", async () => {
    ...
    const descriptors = [
      {
        ...
        inputSchema: {
          type: "object",
          properties: { path: { type: "string", description: "Local filesystem path" } },
          required: ["path"],
          additionalProperties: false,
        },
        risk: "read" as const,
    ...
    expect((await registry.search("local filesystem path", context))[0]?.ref).toBe(
      "storage.inspect",
    );
    expect((await registry.search("filesystem discovery", context))[0]?.ref).toBe(
      "storage.inspect",
    );
  ...
  it("searches only dynamic namespaces, never the statically addressable pi, extensions, agents, or memory", async () => {
    const registry = new ActionRegistry();
    const namespacedProvider = (providerName: string, ...names: string[]): RaftProvider => ({
      name: providerName,
      description: `${providerName} provider`,
      async list() {
        return names.map((name) => ({
          name,
          description: `${providerName} ${name}`,
          inputSchema: { type: "object", properties: {} },
          risk: "read" as const,
        }));
      },
    ...
    registry.register(namespacedProvider("pi", "read", "bash"));
    registry.register(namespacedProvider("extensions", "read", "web_search"));
    registry.register(namespacedProvider("mcp", "read", "docs_search"));
    registry.register(namespacedProvider("agents", "read", "run"));
    registry.register(namespacedProvider("memory", "read", "expand"));

    const refs = (await registry.search("read", context)).map((action) => action.ref);
    expect(refs).toContain("mcp.read");
    expect(refs).not.toContain("pi.read");
    expect(refs).not.toContain("extensions.read");
    expect(refs).not.toContain("agents.read");
    expect(refs).not.toContain("memory.read");

    // describe and call still resolve statically addressable refs for computed calls.
    expect((await registry.describe("pi.read", context)).ref).toBe("pi.read");
  });

  it("names a statically addressable tool instead of ranking its prose near-misses", async () => {
    const registry = new ActionRegistry();
    const singleAction = (
      providerName: string,
      actionName: string,
      ...
      async describe(name) {
        return name === actionName ? (await this.list({}, context))[0] : undefined;
      },
      async invoke() {
        return actionName;
      },
    });
    registry.register(singleAction("pi", "read", "Read a text file with anchors"));
    registry.register(singleAction("storage", "expand", "Read exact normalized session entries"));

    // An intent query still ranks the dynamic near-match.
    const intent = await registry.search("session entries", context);
    expect(intent.map((action) => action.ref)).toContain("storage.expand");

    // A name lookup names the direct call instead of answering with the near-match.
    await expect(registry.search("read", context)).rejects.toThrow(/names pi\.read/);

    // Prose that mentions a verb but never names the namespace is an intent, not a
    // lookup: "list github pull requests" must still discover real dynamic actions
    // instead of being hijacked by the agents.list name.
    const prose = await registry.search("file read", context);
    expect(prose.map((action) => action.ref)).toContain("storage.expand");
    // A dynamic action that owns the queried name still wins.
    registry.register(singleAction("mcp", "read", "Read a document"));
    const owned = await registry.search("read", context);
    expect(owned.map((action) => action.ref)).toContain("mcp.read");
  });

  it("answers a namespace query with that namespace's refs instead of an empty ranking", async () => {
    const registry = new ActionRegistry();
    const agentActions: RaftProvider = {
      name: "agents",
      description: "agents provider",
      async list() {
        return ["run", "spawn", "wait"].map((name) => ({
          name,
    ...
    registry.register(agentActions);
    registry.register({
      name: "mcp",
      description: "mcp provider",
      async list() {
        return [
          {
            name: "list",
    ...
    // The fixed namespace is discoverable by name and by singular spelling.
    await expect(registry.search("agents", context)).rejects.toThrow(
      /names a fixed namespace\. Its actions are agents\.run, agents\.spawn, agents\.wait/,
    );
    await expect(registry.search("agent", context)).rejects.toThrow(/Its actions are agents\.run/);

    // A namespace plus one of its verbs still names the direct call.
    await expect(registry.search("spawn agent task", context)).rejects.toThrow(
      /names agents\.spawn/,
    );

    // A verb alone is prose: the dynamic near-match is ranked, never hijacked.
    const ranked = await registry.search("list github pull requests", context);
    expect(ranked.map((action) => action.ref)).toContain("mcp.list");

    // describe answers a namespace the same way instead of a dead-end unknown action.
    await expect(registry.describe("agents", context)).rejects.toThrow(
      /names a fixed Raft namespace, not an action/,
    );
  });
  ...
  it("emits structured invocation activity without exposing another model tool", async () => {
    ...
    expect(events).toMatchObject([
      ...
      { type: "call_update", update: { type: "entity", id: "demo-entity", kind: "custom" } },
      { type: "call_end", success: true, result: "hello", preview: { renderer: "rich" } },
    ]);
  });
  ...
  it("populates audit preview metadata before invoking the provider", async () => {
    const registry = new ActionRegistry();
    const audits: RaftCallAudit[] = [];
    let observed: RaftCallAudit | undefined;
    registry.register({
      ...provider(),
  ...
  it("bounds retained audit previews without shrinking provider results", async () => {
    ...
    const result = (await registry.invoke(
      "demo.echo",
      { value: "large" },
      { ...context, approve: async () => {}, audits, maxResultChars: 1_000_000 },
    )) as Record<string, string>;

    expect(result.field0).toHaveLength(20_000);
    expect(audits[0]?.result).toMatchObject({ raftTruncated: true });
  ...
  it("caps nested results before crossing the sandbox bridge", async () => {
    ...
    const result = await registry.invoke(
      "demo.echo",
      { value: "x".repeat(100) },
      { ...context, approve: async () => {}, audits, maxResultChars: 40 },
    );
    expect(result).toMatchObject({ raftTruncated: true, originalChars: 102 });
  ...
  it("explains why marked providers are unavailable", async () => {
    ...
    await expect(registry.describe("memory.recall", context)).rejects.toThrow(
      'Raft provider "memory" is unavailable: disabled by configuration (memory.enabled=false)',
    );
  });
  ...
  it("lists registered providers for unknown provider names", async () => {
    ...
    await expect(registry.describe("memry.recall", context)).rejects.toThrow(
      "Unknown Raft provider: memry (registered providers: demo)",
    );
  });
  ...
  it("rejects marking registered or malformed providers unavailable", () => {
    ...
    expect(() => registry.markUnavailable("demo", "off")).toThrow(
      "Cannot mark a registered Raft provider unavailable: demo",
    );
    expect(() => registry.markUnavailable("Bad Name", "off")).toThrow("Invalid Raft provider name");
  });
