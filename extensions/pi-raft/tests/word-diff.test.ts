...
import { changedRanges, changedRangesWithConfidence } from "../src/ui/word-diff/emphasis.js";
...
describe("Raft word-level diff emphasis", () => {
  ...
  it("suppresses low-signal wrapper syntax in smart mode", () => {
    ...
    expect(
      slices(before, changedRanges(before, after, "all").removed).some((text) =>
        text.includes(".map"),
      ),
    ).toBe(true);
  });
  ...
  it("recovers reordered changed lines above the full-matrix cutoff", () => {
    const contents = Array.from(
      { length: 33 },
      (_, index) => `const record${index}Checksum${1000 + index} = transform${index}(old${index});`,
    );
