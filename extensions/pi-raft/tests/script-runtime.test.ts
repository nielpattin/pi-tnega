...
describe("script runtime resolution", () => {
  ...
  it("falls back to PI_RAFT_NODE_BINARY when execPath is the bundled pi binary", async () => {
    const execPath = "/usr/local/bin/pi";
    const override = "/opt/node-v22/bin/node";
    expect(resolveScriptRuntimeSync({ execPath, env: { PI_RAFT_NODE_BINARY: override } })).toBe(
      override,
    );
    expect(await resolveScriptRuntime({ execPath, env: { PI_RAFT_NODE_BINARY: override } })).toBe(
      override,
    );
  });

  it("builds the full spawn argv prefix through scriptSpawnArgs", async () => {
    const args = await scriptSpawnArgs("/raft/worker.js", ["--task-file", "/tmp/task.txt"], {
      execPath: "/usr/local/bin/pi",
      env: { PI_RAFT_NODE_BINARY: "/opt/node" },
    });
    expect(args).toEqual(["/opt/node", "/raft/worker.js", "--task-file", "/tmp/task.txt"]);
  });

  it("quotes every token in workerCommand using the resolved runtime", async () => {
    const command = await workerCommand("/raft/worker.js", ["--task-file", "/tmp/task.txt"]);
    const runtime = resolveScriptRuntimeSync();
    expect(command.startsWith(`'${runtime}'`)).toBe(true);
    expect(command).toContain("'/raft/worker.js'");
  ...
  it("throws a clear error when the bundled binary has no runtime and no override", () => {
    expect(() => resolveScriptRuntimeSync({ execPath: "/usr/local/bin/pi", env: {} })).toThrow(
      /requires a Node\.js or Bun runtime|PI_RAFT_NODE_BINARY/,
    );
  });
