...
const options = { timeoutMs: 5_000, memoryLimitBytes: 128 * 1024 * 1024 };
...
describe("NodeProcessRuntime", () => {
  ...
  it("extends the active deadline for a long host call", async () => {
    const result = await new NodeProcessRuntime().execute(
      'await tools.call({ ref: "agents.run", args: { timeoutMs: 1 } }); return "ok";',
      async () => {
      ...
      },
      {
        ...
        minimumTimeoutMsForHostCall(ref) {
          return ref === "raft.$call" ? 3_000 : undefined;
        },
      },
    );
  ...
  it("accepts a heap limit above the QuickJS WASM32 ceiling", async () => {
    const result = await new NodeProcessRuntime().execute("return 1;", async () => undefined, {
      ...options,
...
describe.skipIf(!hasBun)("BunProcessRuntime", () => {
  ...
  it("forcibly terminates synchronous infinite loops", async () => {
    const result = await new BunProcessRuntime().execute("while (true) {}", async () => undefined, {
      ...options,
      timeoutMs: 50,
    });
