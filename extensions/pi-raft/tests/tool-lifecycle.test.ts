...
import {
  RaftToolLifecycle,
  ownsRaftToolSource,
  type RaftTopLevelToolAuthorizer,
...
const eventRunner = (
  ...
  Object.assign(runner as unknown as Record<string, unknown>, {
    extensions: [{ path: "/extensions/pi-raft/index.ts", handlers }],
    createContext: () => ({}),
...
const failedDetails = (outcome: RaftExecutionOutcomeV1, failureStage?: "guard" | "invoke") => {
  const recorder = new RaftExecutionTraceRecorder();
  if (failureStage) {
    recorder
      .issueCall(failureStage === "guard" ? "pi.write" : "agents.run", {})
      .fail(failureStage, new Error(`${failureStage} failure`), outcome);
  }
  return createRaftPersistedExecutionDetails({
    success: false,
...
const executeThroughPiLifecycle = async (details: unknown) => {
  ...
  const lifecycle = new RaftToolLifecycle(
    () => true,
    () => ({ authorize: async () => {} }),
  );
...
describe("Raft outer tool lifecycle", () => {
  it.each([
    ...
    expect(toolErrors).toEqual([expect.objectContaining({ toolName: "raft_exec", isError: true })]);
  });
...
describe("Direct top-level tool approval gate", () => {
  it("approves native calls while preserving owned and nested Raft boundaries", async () => {
    ...
    expect(approve).toHaveBeenCalledWith(expect.objectContaining({ toolName: "read" }), context);
...
describe("Schema top-level tool gate", () => {
  const gate = (mode: "off" | "audit" | "enforce", ownsRaft = true) => {
    const decisions: string[] = [];
    const authorizer: RaftTopLevelToolAuthorizer = {
      async authorize(ref) {
        if (mode === "off") return;
    ...
    return {
      lifecycle: new RaftToolLifecycle(
        () => ownsRaft,
        () => authorizer,
      ),
      decisions,
  ...
  it("uses canonical source provenance rather than SDK/extension metadata claims", () => {
    const entry = "/extensions/pi-raft/index.ts";
    expect(ownsRaftToolSource([{ name: "raft_exec", sourceInfo: { path: entry } }], entry)).toBe(
      true,
    );
    const sdkSpoof = [
      {
    ...
    expect(ownsRaftToolSource(sdkSpoof, entry)).toBe(false);
    expect(
      ownsRaftToolSource(
        [{ name: "raft_exec", sourceInfo: { path: "/extensions/external/index.ts" } }],
        entry,
      ),
    ).toBe(false);
  });

  it("allows only this extension's exact top-level raft_exec in enforce mode", async () => {
    const owned = gate("enforce", true);
    await expect(
      owned.lifecycle.toolCall({
        type: "tool_call",
    ...
    await expect(
      sdkCustomTool.lifecycle.toolCall({
        type: "tool_call",
        toolCallId: "call-sdk",
    ...
    const externalRaftSpoof = gate("enforce", false);
    await expect(
      externalRaftSpoof.lifecycle.toolCall({
        type: "tool_call",
        toolCallId: "call-external",
        toolName: "raft_exec",
        input: {},
      }),
    ).rejects.toThrow("blocked schema.top_level_tool.raft_exec");
  });
  ...
  it("records would-block in audit mode and leaves off mode unchanged", async () => {
    ...
    expect(audit.decisions).toEqual(["audit:schema.top_level_tool.sdk_custom_tool"]);

    const off = gate("off");
    await expect(
      off.lifecycle.toolCall({
        type: "tool_call",
        toolCallId: "call-off",
