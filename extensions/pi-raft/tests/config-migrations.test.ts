...
import {
  ...
  migrateRaftConfigDocument,
} from "../src/config-migrations.js";
import { loadRaftConfig, saveRaftConfig } from "../src/config.js";

const roots: string[] = [];

const fixture = () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-migration-"));
  roots.push(root);
  ...
  return {
    cwd,
    agentDir,
    globalPath: path.join(agentDir, "raft.json"),
    projectPath: path.join(cwd, ".pi", "raft.json"),
  };
};
...
describe("Raft configuration migrations", () => {
  it("migrates the legacy agent section without mutating its input", () => {
    const input = {
      subagents: { runner: "claude", defaultTools: ["read"] },
      ui: { enabled: false },
    };
    const result = migrateRaftConfigDocument(input);

    expect(result).toMatchObject({
      fromVersion: 0,
      toVersion: CURRENT_RAFT_CONFIG_VERSION,
  ...
  it("merges both section names with the canonical section taking precedence", () => {
    const result = migrateRaftConfigDocument({
      subagents: {
        runner: "pi",
        claude: { binary: "old", model: "claude/old" },
        defaultTools: ["bash"],
      },
  ...
  it("rejects an ambiguous malformed canonical section instead of discarding legacy values", () => {
    expect(() =>
      migrateRaftConfigDocument({ subagents: { maxConcurrent: 6 }, agents: false }),
    ).toThrow(/malformed agents section/);
  });

  it("rejects invalid and legacy keys in current documents", () => {
    expect(() => migrateRaftConfigDocument({ configVersion: -1 })).toThrow(/non-negative integer/);
    expect(() => migrateRaftConfigDocument({ configVersion: 1, subagents: {} })).toThrow(
      /removed key/,
    );
  });
  ...
  it("migrates each config layer before applying project precedence", () => {
    ...
    fs.writeFileSync(
      paths.globalPath,
      JSON.stringify({ agents: { runner: "claude", maxConcurrent: 2 } }),
    );
    fs.writeFileSync(
      paths.projectPath,
      JSON.stringify({ subagents: { runner: "pi", transport: "tmux" } }),
    );
    ...
    expect(JSON.parse(fs.readFileSync(paths.globalPath, "utf8"))).toMatchObject({
      configVersion: 5,
  ...
  it("does not rewrite an already-current config during load", () => {
    const paths = fixture();
    const current =
      JSON.stringify({ configVersion: 5, agents: { maxConcurrent: 3 } }, null, 2) + "\n";
    fs.writeFileSync(paths.globalPath, current);
    const before = fs.statSync(paths.globalPath).mtimeMs;

    loadRaftConfig({ cwd: paths.cwd, agentDir: paths.agentDir, projectTrusted: false });
  ...
  it("preserves existing file permissions during migration", () => {
    const paths = fixture();
    fs.writeFileSync(paths.globalPath, JSON.stringify({ agents: { maxConcurrent: 5 } }), {
      mode: 0o640,
    });
  ...
  it("rejects obsolete or caller-controlled migration metadata on save", () => {
    ...
    expect(() => saveRaftConfig(options, { subagents: {} })).toThrow(/current schema/);
    expect(() => saveRaftConfig(options, { configVersion: 1 })).toThrow(/current schema/);
  });

  it("renames ui.showNestedToolCalls to ui.showAgentToolPreview without mutating its input", () => {
    const input = { configVersion: 1, ui: { showNestedToolCalls: false, maxRows: 8 } };
    const result = migrateRaftConfigDocument(input);
  ...
  it("leaves version 1 documents without the legacy key unchanged in content", () => {
    const result = migrateRaftConfigDocument({ configVersion: 1, ui: { maxRows: 4 } });
  ...
  it("persists the rename when loading a legacy ui key", () => {
    ...
    expect(config.appearance.ui.showAgentToolPreview).toBe(false);
