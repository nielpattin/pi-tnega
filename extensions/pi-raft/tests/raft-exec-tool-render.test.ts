...
import { createRaftPersistedExecutionDetails } from "../src/audit/index.js";
import { createRaftExecTool } from "../src/raft-exec-tool.js";
import { defaultCodePreviewSettings } from "../src/ui/code-preview.js";
import { RaftToolDisplayController } from "../src/ui/tool-display.js";
...
const stateFor = (
  ...
  ({
    ...
    config: {
      ...
      appearance: { ui: { showAgentToolPreview: true, toolDisplay } },
    },
    ...overrides,
...
const toolFor = (
  state: RaftState,
  display?: RaftToolDisplayController,
  codePreviewSettings = defaultCodePreviewSettings(),
) => createRaftExecTool(state, codePreviewSettings, (tool) => tool, display);

const renderContext = (args: Record<string, unknown>, overrides: Record<string, unknown> = {}) => ({
  args,
  toolCallId: "raft-call-1",
  invalidate: vi.fn(),
...
const renderCall = (
  ...
  tool.renderCall!(args as never, theme, renderContext(args, { expanded }) as never)
    .render(120)
    .join("\n");
...
describe("registered raft_exec compact transcript rendering", () => {
  it("keeps full source while compact elevates intent and falls back to Raft for absent or blank names", () => {
    const args = {
    ...
    const blank = renderCall(toolFor(stateFor("compact")), {
      code: args.code,
      display: { name: "   " },
    });

    expect(full).toContain("raft");
    ...
    expect(full.indexOf("Persist the verified setting")).toBeGreaterThan(
      full.indexOf("TypeScript · 2 lines"),
    );
    expect(full.indexOf("implementationSecret")).toBeGreaterThan(
      full.indexOf("Persist the verified setting"),
    );
    expect(compact).toContain("Apply migration");
    expect(compact).toContain("Persist the verified setting");
    expect(compact).not.toContain("raft");
    expect(compact).not.toContain("TypeScript");
    expect(compact).not.toContain("implementationSecret");
    expect(composing).toContain("Raft");
    expect(fallback).toContain("Raft");
    expect(blank).toContain("Raft");
  });

  it("renders anchor-row diffs from any tool without exposing anchors", () => {
    const args = { code: 'await extensions.unseen_editor({ path: "src/app.ts" });' };
    const details = {
      success: true,
      audits: [
        {
          ref: "extensions.unseen_editor",
          provider: "extensions",
          tool: "unseen_editor",
          ...
          result: {
            ok: true,
            output:
              " nkBX│      .on_click(cx.listener);\n-NXOW│      this.close_settings();\n+avry│      this.close_settings();\n AVsg│      cx.notify();",
            details: {
              diff: " nkBX│      .on_click(cx.listener);\n-NXOW│      this.close_settings();\n+avry│      this.close_settings();\n AVsg│      cx.notify();",
            },
          },
        },
      ],
    ...
    const rendered = renderResult(toolFor(stateFor("full")), args, details, "", { expanded: true });
    expect(rendered).toContain("this.close_settings();");
    expect(rendered).not.toContain("NXOW");
    expect(rendered).not.toContain("avry");
    expect(rendered).toContain("+       this.close_settings();");
  });
  ...
  it("promotes a compact card to the full transcript while the expand toggle is on", () => {
      ...
      audits: [
        {
          ...
          success: true,
          result: "export const value = true;",
        },
        {
          ref: "pi.bash",
          provider: "pi",
    ...
    expect(renderCall(toolFor(stateFor("compact")), args, true)).toBe(
      renderCall(toolFor(stateFor("full")), args, true),
    );

    const compactCollapsed = renderResult(
      toolFor(stateFor("compact")),
      args,
      details,
      "outer return",
    );
    const compactExpanded = renderResult(
      toolFor(stateFor("compact")),
      args,
      details,
      "outer return",
    ...
    const fullExpanded = renderResult(toolFor(stateFor("full")), args, details, "outer return", {
      expanded: true,
    });

    expect(compactCollapsed).toContain("Tools");
    expect(compactCollapsed).not.toContain("Raft complete");
    expect(compactExpanded).toBe(fullExpanded);
    expect(compactExpanded).toContain("Raft complete");
  });

  it("adds configured duration metadata to completed provider calls", () => {
    const args = {
      code: "return await extensions.fovea_focus({ query: 'src/raft-exec-tool.ts' });",
    };
    const details = {
      success: true,
      audits: [
        {
          ...
          provider: "extensions",
          tool: "fovea_focus",
          args: { query: "src/raft-exec-tool.ts" },
          success: true,
          startedAt: 1_000,
          endedAt: 1_129,
        },
    ...
    expect(rendered).toContain("fovea_focus src/raft-exec-tool.ts · 129ms");
  ...
  it("uses Tools or Evaluated summaries while preserving completed nested detail and bounded returns", () => {
    ...
    expect(full).toContain("Raft failed");
  ...
  it("leaves completed nested bash and dynamic calls unchanged apart from the grouped header", () => {
    ...
    const fullCollapsed = renderResult(toolFor(stateFor("full")), args, details, "", {
      theme: semanticTheme,
    });
    const compactExpanded = renderResult(toolFor(stateFor("compact")), args, details, "", {
      expanded: true,
      theme: semanticTheme,
    });
    ...
    expect(full).toContain("Raft complete");
    expect(compact).toContain("existing-string-headline");
    expect(compact).toContain("Tools");
    expect(compact).not.toContain("Raft complete");
  ...
  it("re-renders a resumed card identically to the live one from persisted audits", () => {
    ...
    const persisted = JSON.parse(
      JSON.stringify(
        createRaftPersistedExecutionDetails({ success: true, trace: trace as never, audits }),
      ),
    );
    ...
    const expanded = renderResult(toolFor(stateFor("full")), args, persisted, "", {
      expanded: true,
    });
  ...
  it("marks trace-only resumed audits instead of raftating empty content", () => {
  ...
  it("uses a compact pre-tool live status and Tools for grouped activity without changing its nested calls", () => {
    ...
    const details = {
      audits: [
        ...
        },
        {
          ...
          provider: "extensions",
          tool: "remote",
          args: { strings: "live-string-headline" },
        },
      ],
      phases: [],
    };
    ...
    const compactBeforeFirstTool = renderResult(
      toolFor(stateFor("compact")),
      args,
      { progress: "Running Raft program…", audits: [], phases: [] },
      "",
      { partial: true },
    );

    expect(full).toContain("Raft running");
    ...
    expect(compactBeforeFirstTool).not.toContain("Raft program");
  });
      ...
      audits: partialDetails.audits.map((audit, index) =>
        index === 1
          ? {
              ...audit,
              success: true,
              result: { details: { diff: "-const value = 'before';\n+const value = 'after';" } },
            }
          : audit,
      ),
    };
  ...
  it("falls back to full before bootstrap and for an explicit full preference", () => {
    const args = { code: "const untouchedSource = true;", display: { name: "Keep full" } };
    ...
    const failedBootstrap = { cwd: "/workspace" } as unknown as RaftState;
  ...
  it("invalidates completed cards so their current display preference redraws immediately", async () => {
    ...
    const display = new RaftToolDisplayController();
    ...
    const full = tool.renderCall!(args as never, plainTheme, context as never)
      .render(120)
      .join("\n");
    const fullResult = tool.renderResult!(
      ...
      .render(120)
      .join("\n");
    (state.config.appearance.ui as { toolDisplay: "full" | "compact" }).toolDisplay = "compact";
    display.refresh();
    const compact = tool.renderCall!(args as never, plainTheme, context as never)
      .render(120)
      .join("\n");
    ...
    expect(fullResult).toContain("Raft");
    ...
    (state.config.appearance.ui as { toolDisplay: "full" | "compact" }).toolDisplay = "full";
    display.refresh();
    await flushDrainTurns(2);
    const fullAgain = tool.renderCall!(args as never, plainTheme, context as never)
      .render(120)
      .join("\n");
    ...
    expect(fullResultAgain).toContain("Raft");
