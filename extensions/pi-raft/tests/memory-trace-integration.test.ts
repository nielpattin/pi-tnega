...
const temp = (name: string): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-${name}-`));
  temporary.push(dir);
  return dir;
};
...
const timestamp = (second: number): string =>
  `2025-02-01T00:00:${String(second).padStart(2, "0")}.000Z`;

const traceToolResult = (details: unknown): Record<string, unknown> => ({
  ...toolResult("raft-call", "raft_exec", "fake prose pi.write path=prose-only.ts Error", false),
  details,
});
...
const fixture = (agentDir: string, cwd: string, details: unknown): string =>
  writeSessionFile(path.join(agentDir, "sessions", encodeCwdDir(cwd)), "trace.jsonl", [
    ...
    messageEntry(
      ...
      null,
      timestamp(1),
      assistantToolCall("raft-call", "raft_exec", {
...
describe("memory Raft trace records", () => {
  ...
  it("supports tool filters, operation-address expansion, and cold vocabulary", async () => {
    ...
    expect(
      (await searchShards([shard], { query: "BRANCH_SUMMARY_PROSE_POISON" })).matchedCount,
    ).toBe(0);
    ...
    for (const term of ["agents", "mcp", "extensions", "bash", "read"])
      expect(vocabulary.has(term)).toBe(true);
    expect(digest.toolHistogram).toMatchObject({ read: 1, bash: 2, run: 1, preview: 1 });
    ...
    expect(providerExpanded.entries).toMatchObject([
      { operationAddress: "e2/7", ref: "agents.run", tool: "run" },
    ]);
  });
  ...
  it("selects exact capability heads in hot and cold tiers without treating catalog prose as evidence", async () => {
    ...
    expect(
      structural.segments
        ...
        .every((entry) => entry.entry.ref === "pi.edit"),
    ).toBe(true);
    ...
    expect(combined.segments.flatMap((segment) => segment.exactMatches)).toEqual([
      { index: expect.any(Number), entryId: "e2/1", operationAddress: "e2/1" },
    ]);
  ...
  it("ignores malformed and unknown trace versions instead of adapting audits", () => {
    ...
    expect(
      normalizeSession(file, 2_000).entries.filter((entry) => entry.type === "raft_operation"),
    ).toEqual([]);
  });
});
