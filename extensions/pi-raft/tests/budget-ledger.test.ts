...
describe("budget ledger", () => {
  ...
  it("treats a non-positive budget as inactive", () => {
    process.env.PI_RAFT_BUDGET = "0";
    process.env.PI_RAFT_BUDGET_FILE = "/tmp/ignored";
    process.env.PI_RAFT_BUDGET_ID = "x";
    try {
      expect(activeBudgetState()).toBeUndefined();
    } finally {
      clearOwnedBudgetEnv();
  ...
  it("sums appended entries and tolerates malformed lines", () => {
    ...
    temporaryFiles.push(state.file);
    appendBudgetLedger(state.file, { id: "a", depth: 1, cost: 0.06, tokens: 30, ts: 1 });
    appendBudgetLedger(state.file, { id: "b", depth: 2, cost: 0.04, tokens: 70, ts: 2 });
  ...
  it("returns zero for a missing ledger file", () => {
    expect(readBudgetLedger(path.join(os.tmpdir(), "pi-raft-missing-cost.jsonl"))).toEqual({
      cost: 0,
      tokens: 0,
    });
  });
