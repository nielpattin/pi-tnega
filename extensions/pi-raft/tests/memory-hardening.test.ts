...
const temporaryDirectory = (name: string): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-memory-hardening-${name}-`));
...
const message = (id: string, text: string, second = 0) =>
  messageEntry(
    id,
    null,
...
describe("memory query and pointer hardening", () => {
  ...
  const config = (overrides: Partial<RaftMemoryConfig> = {}): RaftMemoryConfig => ({
    enabled: true,
    indexDir,
    maxSessions: 500,
  ...
  it("never infers regex mode from dots or paths", async () => {
    expect(planMemoryQuery("src/foo.ts")).toEqual({
      kind: "terms",
      terms: ["src", "foo", "ts"],
      match: "any",
    });
  ...
  it("rejects stale pointers after a same-path source rewrite", async () => {
    ...
    const pointer = (await provider().invoke(
      ...
      hits: Array<{ follow: { args: { expectedSourceHash: string } & Record<string, unknown> } }>;
    };
    const follow = pointer.hits[0]!.follow.args;
    fs.appendFileSync(cold, `${JSON.stringify(message("changed", "rewritten"))}\n`);

    const hydrated = (await provider().invoke("recall", follow, invocationContext(cwd))) as {
      error: { code: string };
    };
  ...
  it("reports index bounds instead of silently dropping invalid addresses", async () => {
    ...
    expect(beyond.error).toEqual(
      expect.objectContaining({ code: "index_out_of_bounds", entryCount: 2 }),
    );
  });
  ...
  it("marks oversized vocabulary and cache-sync budgets incomplete", async () => {
    ...
    const vocabulary = (await provider({ hotSessions: 0, maxColdVocabularyBytes: 10 }).invoke(
      "recall",
      { scope: "project", query: "epsilon" },
      invocationContext(cwd),
    )) as {
      total: number;
      coverage: { complete: boolean; reasons: string[]; incompleteSessions: number };
    };
    ...
    expect(budget.coverage).toEqual(
      expect.objectContaining({ complete: false, indexedSessions: 1, eligibleSessions: 3 }),
    );
  ...
  it("keeps unique-token cold caches within source ratio and configured hard bounds", async () => {
    const tokens = Array.from(
      { length: 2_000 },
      (_, index) => `unique_${String(index).padStart(5, "0")}`,
    );
    const file = seed(
      "ratio.jsonl",
      "ratio",
      tokens.map((token) => `${token} context payload`),
    );
    ...
    }).invoke("recall", { scope: "project", query: "unique_01999" }, invocationContext(cwd))) as {
      hits: unknown[];
    };
