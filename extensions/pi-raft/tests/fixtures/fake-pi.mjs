...
// A stub `pi` binary for the real-worker e2e. The Raft worker spawns this as
    ...
    while (true) {
      ...
      if (frame.type === "prompt") {
        ...
        emit({
          ...
          result: fail
            ? null
            : { summary: "child compacted", firstKeptEntryId: "entry-1", tokensBefore: 100 },
          aborted: false,
          willRetry: false,
...
switch (behavior) {
  ...
  case "raft-session-env":
    emit({
      type: "message_end",
      message: { role: "assistant", content: process.env.PI_RAFT_SESSION_ID || "missing" },
    });
    emit({ type: "agent_settled" });
    process.exit(0);
    break;
  ...
  case "usage-flow":
    ...
    emit({
      ...
      message: {
        role: "assistant",
        content: "first",
        usage: { input: 100, output: 50, cacheRead: 10, cacheWrite: 5, cost: 0.01 },
      },
    });
    setTimeout(() => {
      emit({
        ...
        message: {
          role: "assistant",
          content: "second",
          usage: { input: 200, output: 100, cacheRead: 20, cacheWrite: 10, cost: 0.02 },
        },
      });
      emit({ type: "agent_settled" });
      process.exit(0);
