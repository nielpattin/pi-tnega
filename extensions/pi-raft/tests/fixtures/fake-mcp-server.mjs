...
input.on("line", (line) => {
  ...
  if (request.method === "initialize") {
    respond(request.id, {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "pi-raft-test", version: "1.0.0" },
    });
    return;
  }
  if (request.method === "tools/list") {
    if (process.env.PI_RAFT_MCP_COUNT_FILE) {
      fs.appendFileSync(
        process.env.PI_RAFT_MCP_COUNT_FILE,
        `${process.env.PI_RAFT_MCP_COUNT_LABEL ?? "server"}\n`,
      );
    }
    respond(request.id, {
      tools: [
        ...
        content: [
          {
            type: "text",
            text:
              request.params.name === "get-model-schema"
