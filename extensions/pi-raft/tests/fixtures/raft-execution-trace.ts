import { RaftExecutionTraceRecorder, type RaftExecutionTraceV1 } from "../../src/audit/trace.js";

export const recordedIntegrationTrace = (): RaftExecutionTraceV1 => {
  const recorder = new RaftExecutionTraceRecorder();
  const read = recorder.issueCall("pi.read", { path: "src/read.ts", offset: 1, limit: 20 });
  const editFailure = recorder.issueCall("pi.edit", {
    path: "src/edit.ts",
    edits: [{ oldText: "a", newText: "b" }],
  });
  const editSuccess = recorder.issueCall("pi.edit", {
    path: "src/edit.ts",
    edits: [{ oldText: "a", newText: "b" }],
  });
...
export const recordedParallelTrace = (): RaftExecutionTraceV1 => {
  const recorder = new RaftExecutionTraceRecorder();
