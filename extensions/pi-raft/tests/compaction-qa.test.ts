...
const buildFixture = (): QaFixture => {
  ...
  const filler = Array.from({ length: 125 }, (_, index) =>
    textPart(`deterministic progress ${index}`),
  );
  const entries: SessionEntry[] = [
    user(
      "Implement reconstruction QA.\nKeep fidelity measurable.\nUse typed events.\nDo not retain prose.",
    ),
    ...
    assistant(
      toolCallPart("commit", "bash", { command: "git commit -m 'test reconstruction qa'" }),
    ),
    toolResult("commit", "bash", "[feature abc1234] test reconstruction qa\n 3 files changed"),
    user("Add mutation tests"),
    assistant(...filler),
    assistant(
      toolCallPart("qa-raft", "raft_exec", {
    ...
    toolResult("qa-raft", "raft_exec", "all reconstruction probes passed"),
...
describe("compaction reconstruction QA", () => {
  it("passes against the real normalize → project → render engine output", () => {
    ...
    expect(
      probes.some((probe) => probe.class === "content" && probe.answer === "compaction.md"),
    ).toBe(true);
    expect(
      probes.some(
        (probe) =>
          probe.class === "content" &&
          probe.answer === "read src/missing.ts: ENOENT: no such file or directory",
      ),
    ).toBe(true);
    expect(probes.some((probe) => probe.id.startsWith("commit:"))).toBe(false);
    expect(probes).toContainEqual(
      expect.objectContaining({
        id: expect.stringMatching(/^last-raft-run:/),
        answer: "Certify reconstruction QA → succeeded",
      }),
    );
    expect(probes).toContainEqual(
      expect.objectContaining({
        id: expect.stringMatching(/^last-raft-run-address:/),
        answer: "[entry qa-e14]",
      }),
    );
    ...
    expect(
      probes.some(
        (probe) => probe.class === "address" && probe.id.startsWith("earlier-turn-address:"),
      ),
    ).toBe(true);
    expect(probes.some((probe) => probe.class === "address" && probe.id === "footer-recall")).toBe(
      true,
    );
  ...
  it("fails declared run-intent probes when name or stable address is removed", () => {
    ...
    expect(failedIds(fixture, withoutName)).toContainEqual(
      expect.stringMatching(/^last-raft-run:/),
    );

    const withoutAddress = fixture.summary.replaceAll("[entry qa-e14]", "");
    expect(failedIds(fixture, withoutAddress)).toContainEqual(
      expect.stringMatching(/^last-raft-run-address:/),
    );
  });
  ...
  it("fails turn-count and address probes when Earlier Turns is truncated", () => {
    ...
    expect(
      failures.some(
        ({ probe }) =>
          probe.id.startsWith("earlier-turn-count:") && probe.answer === '"Add mutation tests"',
      ),
    ).toBe(true);
    expect(
      failures.some(
        ({ probe }) =>
          probe.id.startsWith("earlier-turn-address:") && probe.answer === '"Add mutation tests"',
      ),
    ).toBe(true);
  });
