...
import { RAFT_AGENT_MARKER, SessionExporter } from "../src/worker/session-export.js";

const savedEnv = process.env[SESSION_EXPORT_ENV];
const tempRoots: string[] = [];

const makeTempRoot = (): string => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-export-test-"));
  tempRoots.push(root);
  return root;
};

const config = (sessionExport: boolean, sessionExportDir = "") =>
  normalizeRaftConfig({ agents: { sessionExport, sessionExportDir } }).agents;
...
describe("session export config", () => {
  it("defaults to enabled with an empty directory override", () => {
    const agents = normalizeRaftConfig({}).agents;
  ...
  it("ignores a non-string directory override", () => {
    const agents = normalizeRaftConfig({ agents: { sessionExportDir: 42 } }).agents;
    expect(agents.sessionExportDir).toBe("");
  });
});
...
describe("resolveSessionExportDir", () => {
  ...
  it("defaults to pi's agent dir so trackers pick subagent usage up unconfigured", () => {
    ...
    expect(resolveSessionExportDir(config(true))).toBe(path.join(os.homedir(), ".pi", "agent"));
  });
...
describe("sessionExportFileFor", () => {
  it("builds <root>/sessions/.raft/<encoded-cwd>/<timestamp>_<runId>.jsonl", () => {
...
describe("SessionExporter", () => {
  ...
  it("writes a session header, raftagent marker, and usage-only message lines", () => {
    const { exporter, file } = makeExporter();
    ...
    expect(info.name).toBe(`${RAFT_AGENT_MARKER}website-researcher`);
  ...
  it("clamps negative reconciliation deltas instead of exporting them", () => {
    ...
    const usage = (readLines(file)[2]!.message as Record<string, unknown>).usage as Record<
      string,
      unknown
    >;
  ...
  it("satisfies the subset both parsers require (tokscale/ccusage contract)", () => {
    ...
    expect(String(lines[1]!.name)).toMatch(/^raftagent-/);
  });
});
