...
describe("raft nested rendering", () => {
  ...
  it("restores legacy digest-only bash previews from visible Raft arguments", () => {
    const literalCommand = "pnpm vitest run tests/raft-render.test.ts";
  ...
  it("extracts π write bindings in source order", () => {
    const bindings = raftWriteBindings(`
return Promise.all([
...
`);
  ...
  it("renders a growing single π value during argument composition", () => {
    const streaming = renderRaftWriteArgumentPreview(
      {
  ...
  it("streams only the latest π value in a multicall", () => {
    const composing = renderRaftWriteArgumentPreview(
      {
        bindings: [
    ...
    expect(composing[0]).toContain("Raft composing · 1/3 writes");
  ...
  it("exposes in-flight write content for single-call previews", () => {
    const audit = {
      ...
      provider: "pi",
      tool: "write",
      args: { path: "README.md", content: "# Raft\n\nLive preview" },
    };

    expect(nestedCallBody(audit)).toBe("# Raft\n\nLive preview");
    expect(nestedCallCode(audit)).toEqual({ code: "# Raft\n\nLive preview", lang: "markdown" });
  });
  ...
  it("upgrades a streaming write from plain text after lazy Shiki initialization", async () => {
    ...
    const render = (): string =>
      renderRaftWriteArgumentPreview(input, plainTheme, invalidate)!.render(100).join("\n");
  ...
  it("restores rich core arguments, results, and renderer metadata after final projection", () => {
    ...
    const previews = captureRaftCoreToolPreviews(live);
    const restored = restoreRaftCoreToolPreviews(
      [
  ...
  it("restores ephemeral write content after final trace projection", () => {
    ...
    const previews = captureRaftWritePreviews(live);
    const restored = restoreRaftWritePreviews(
      [
        {
          ref: "pi.write",
          provider: "pi",
  ...
  it("keeps a waiting agent's running tool inline ahead of stale narrative", () => {
    const lines = renderRaftMulticallPartial(
      {
        audits: [
          {
            ref: "agents.wait",
    ...
    expect(lines).toContain("◆ Raft running · 0/1 calls");
  ...
  it("restores friendly agent previews after ephemeral live details disappear", () => {
    ...
    const previews = captureRaftAgentPreviews(live);
    const restored = restoreRaftAgentPreviews(
      [
        {
          ref: "agents.wait",
          provider: "agents",
  ...
  it("keeps later active calls visible when a collapsed multicall exceeds its limit", () => {
    const audits = Array.from({ length: 10 }, (_, index) => ({
      ...
      ...(index === 9
        ? {
            preview: {
              kind: "raft-agent-tools" as const,
              id: "agent-9",
              name: "active-agent",
              status: "running",
    ...
    const lines = renderRaftMulticallPartial({ audits, phases: [], expanded: false }, plainTheme)
      .render(120)
      .join("\n");
  ...
  it("clips collapsed multicall narrative inline and reveals it when expanded", () => {
    const narrative =
      "A long agent update that must remain visible across narrow terminal rows without losing its ending.";
    const audits = [
      {
        ref: "agents.wait",
        provider: "agents",
    ...
    const collapsed = renderRaftMulticallPartial(
      { audits, phases: [], expanded: false },
      plainTheme,
    ).render(40);
    const expanded = renderRaftMulticallPartial(
  ...
  it("renders recursive descendant agent trees when expanded", () => {
    const tool = (id: string, toolName: string, status: string, args: Record<string, unknown>) => ({
      id,
      kind: "tool" as const,
      label: toolName,
      toolName,
              ...
              agents: [
                {
                  ...
                  status: "running",
                  owner: "agent" as const,
                  currentTool: "raft_exec",
                  tools: [
                    tool("child-grep", "grep", "completed", { pattern: "nested", path: "src" }),
                  ],
                  agents: [
                    {
                      id: "agent-grandchild",
    ...
    expect(lines).toContain("◐ planner · running · raft_exec");
  ...
  it("renders transcript content blocks for the two most recent expanded agent tools", async () => {
    ...
    const raw = renderRaftMulticallPartial(
      {
        audits: [
          {
            ref: "agents.run",
  ...
  it("preserves diff-specific backgrounds when nested tool backgrounds are disabled", () => {
    ...
    const diffTheme = {
      ...
      getBgAnsi: (color: string) => (color === "toolErrorBg" ? errorBackground : successBackground),
    } as unknown as Theme;
    const settings = {
      tools: ["edit"],
      editDiffPreview: true,
  ...
  it("returns null for non-edit calls and edits without operations", () => {
    ...
    expect(
      nestedEditDiff({ ref: "pi.edit", tool: "edit", args: { path: "a.ts" } }, theme),
    ).toBeNull();
  });
...
b`,
  ...
  it("keeps generic MCP headlines after final trace projection", () => {
    ...
    const previews = captureRaftCallHeadlinePreviews(live);
    const restored = restoreRaftCallHeadlinePreviews(
    ...
    expect(nestedCallTitle(restored[0]!, plainTheme)).toBe(
    ...
    const live = [
      {
        ...
        tool: "list",
        args: {},
        result: Array.from({ length: 13 }, () => ({})),
      },
      {
        ref: "agents.log",
        provider: "agents",
        tool: "log",
        args: { id: "agent-reviewer-1234", lines: 2000 },
      },
      {
        ref: "mcp.$call",
        provider: "mcp",
        tool: "$call",
        args: { server: "github", tool: "search_code", args: { q: "private" } },
      },
    ];
    const previews = captureRaftCallHeadlinePreviews(live);
    const restored = restoreRaftCallHeadlinePreviews(
  ...
  it("keeps restored headline occurrences aligned when final args already have a preview", () => {
    const restored = restoreRaftCallHeadlinePreviews(
      [
        {
          ref: "mcp.server.lookup",
          provider: "mcp",
          tool: "server.lookup",
          args: { query: "durable first" },
        },
  ...
  it("preserves the enclosing Box background when bounded rows are truncated", () => {
    ...
    expect(line).toBe("\x1b[42m " + "x".repeat(18) + "\x1b[22;23;24;27;29;39m \x1b[49m");
  ...
  it("replaces lost result rows with hidden source lines", () => {
    ...
    const final = observeResultRows(renderBoundedLines(["complete", "calls"]), balance, {
      expanded: false,
      isPartial: false,
    });
  ...
  it("does not reveal a source line that would overshoot the result deficit", () => {
    const balance: ResultRowBalance = {};
    observeResultRows(renderBoundedLines(["one", "two", "three"]), balance, {
      expanded: false,
      isPartial: true,
    }).render(20);
    observeResultRows(renderBoundedLines(["done"]), balance, { expanded: false, isPartial: false });

    const code = new HiddenRowBorrowingComponent(
      1,
      2,
      (limit) => (limit === 1 ? ["base"] : ["base", "wrapped", "source", "line"]),
      balance,
    );
  ...
  it("keeps multicall progress inline without adding completion-only rows", () => {
    const audits = [
      ...
      { ref: "pi.ls", provider: "pi", tool: "ls", args: { path: "src" } },
    ];
    const component = renderRaftMulticallPartial(
      { audits, phases: ["Inspect"], progress: "bash: one\ntwo\nthree\nfour", expanded: false },
      plainTheme,
    );
...
describe("nestedCallTitle truncation marker", () => {
  ...
  it("marks results truncated at the raft result boundary", () => {
    const title = nestedCallTitle(
  ...
  it("omits the marker for complete results", () => {
    const title = nestedCallTitle(
      bashAudit({ ok: true, output: "line", details: { truncation: { truncated: false } } }),
      plainTheme,
    );
