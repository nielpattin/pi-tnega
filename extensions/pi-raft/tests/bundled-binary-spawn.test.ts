...
// resolving the real Node runtime through PI_RAFT_NODE_BINARY. If a transport
...
describe.skipIf(!hasWorker)("agent worker launch under a bundled pi binary", () => {
  ...
  const originalOverride = process.env.PI_RAFT_NODE_BINARY;
  const originalBehavior = process.env.FAKE_PI_BEHAVIOR;

  afterEach(async () => {
    process.execPath = originalExecPath;
    if (originalOverride === undefined) delete process.env.PI_RAFT_NODE_BINARY;
    else process.env.PI_RAFT_NODE_BINARY = originalOverride;
  ...
  it("resolves a JS runtime and completes a subagent when process.execPath is the pi binary", async () => {
    ...
    process.env.PI_RAFT_NODE_BINARY = realNode;
    process.env.FAKE_PI_BEHAVIOR = "success";

    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-bundled-"));
    roots.push(root);
    const config = { ...DEFAULT_RAFT_CONFIG.agents, timeoutMs: 20_000, maxConcurrent: 1 };
