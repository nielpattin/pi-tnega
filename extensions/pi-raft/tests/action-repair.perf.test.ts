...
const AGENTS_ACTIONS = ["run", "spawn", "wait", "status", "list", "stop", "log"];
const MEMORY_ACTIONS = ["recall", "expand"];
...
describe("repair performance", () => {
  it("keeps per-repair cost trivial on realistic catalogs", () => {
    ...
    console.log(
      `repair: 14000 repairs over 39/3-name catalogs in ${elapsed.toFixed(1)}ms (${((elapsed / 14000) * 1000).toFixed(1)}µs/call)`,
    );
  ...
  it("keeps the adversarial full-scan bounded on a 3000-name catalog", () => {
    ...
    console.log(
      `adversarial repair: 3000-name catalog, no match, ${elapsed.toFixed(1)}ms for 200 calls (${(elapsed / 200).toFixed(3)}ms/call)`,
    );
  ...
  it("keeps argument normalization cheap per prepare call", () => {
    ...
    console.log(
      `arg repair: 50k prepare calls in ${elapsed.toFixed(1)}ms (${((elapsed / 50_000) * 1000).toFixed(2)}µs/call)`,
    );
    expect(elapsed).toBeLessThan(5000);
  });
  ...
  it("coerces declared numerics through the low-level path", () => {
    const out = normalizeActionArgs(
      { query: "x", limit: "20" },
      { numerics: ["limit"], knownKeys: ["query", "limit"] },
    );
