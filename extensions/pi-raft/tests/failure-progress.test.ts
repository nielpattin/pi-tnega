...
const trace = (
  outcome: RaftExecutionTraceV1["outcome"],
  operations: RaftExecutionTraceV1["operations"],
...
describe("formatFailureProgress", () => {
  it("reports completed refs and paths without exposing results", () => {
    const formatted = formatFailureProgress(
      trace("failed", [
        {
          type: "call",
          sequence: 0,
  ...
  it("stays absent for successful runs or failures with no completed calls", () => {
    ...
    expect(
      formatFailureProgress(
        trace("failed", [
          { type: "call", sequence: 0, ref: "pi.bash", args: {}, outcome: "failed" },
        ]),
      ),
    ).toBeUndefined();
  });
  ...
  it("bounds long call lists and paths", () => {
    const operations: RaftExecutionTraceV1["operations"] = Array.from(
      { length: 12 },
      (_, sequence) => ({
        type: "call",
        sequence,
