...
import { ActionRegistry, type RaftRegistryInvocationContext } from "../src/core/action-registry.js";
import type {
...
const baseContext = (): RaftInvocationContext => ({
  cwd: process.cwd(),
  signal: undefined,
...
const invocation = (): RaftRegistryInvocationContext => ({
  ...baseContext(),
  approve: async () => {},
  audits: [],
...
class RevisionProvider implements RaftProvider {
  readonly description = "Stable revision provider";
  readonly #descriptor: RaftActionDescriptor = {
    name: "status",
  ...
  async list(_request: RaftProviderListRequest): Promise<RaftActionDescriptor[]> {
    return [this.#descriptor];
  }

  async describe(name: string): Promise<RaftActionDescriptor | undefined> {
...
const harness = () => {
  ...
  const supervisor = new RaftComponentSupervisor(registry, { invocationContext: baseContext });
...
describe("provider components", () => {
  ...
  it("keeps provider holders independent from unrelated revertible effects", async () => {
    ...
    expect(supervisor.status("raft.provider.mcp").effects).toEqual([
      {
    ...
    await supervisor.start(
      { id: "guidance", component: "guidance" },
      {
        name: "guidance",
  ...
  it("owns every non-kernel first-party namespace through a pinned component", async () => {
    const { registry, catalog, loader } = harness();
    const direct = new ActionRegistry();
    const manifest = new RaftProviderComponentManifest(catalog, loader);
    for (const name of RAFT_COMPONENT_PROVIDER_NAMES) {
    ...
    manifest.assertActive(RAFT_COMPONENT_PROVIDER_NAMES, registry);
    ...
    expect(
      registry
        ...
        .sort(),
    ).toEqual([...RAFT_COMPONENT_PROVIDER_NAMES].sort());
    expect(await registry.catalog(baseContext())).toEqual(await direct.catalog(baseContext()));
    for (const name of RAFT_COMPONENT_PROVIDER_NAMES) {
      expect(loader.status(`raft.provider.${name}`)).toMatchObject({
        state: "active",
        provisions: [name],
      });
  ...
  it("fails startup completeness when an enabled namespace lacks a pinned binding", async () => {
    ...
    expect(() => manifest.assertActive(["memory", "mcp"], registry)).toThrow("Missing: mcp");
    expect(() => manifest.assertActive([], registry)).toThrow("Unexpected: memory");
  ...
  it("rolls a provider definition while committed calls retain the old generation", async () => {
    ...
    await expect(
      registry.invoke("memory.status", {}, { ...invocation(), capabilityView: committedView }),
  ...
  it("withdraws a provider committed during synchronous self-retirement", async () => {
    ...
    const component = createProviderComponent({
      provider: "memory",
      description: "Memory provider component",
      create: () => new RevisionProvider("memory", "diverted", () => events.push("provider-close")),
    });
    ...
    expect(() => supervisor.status(component.entry.id)).toThrow("Unknown Raft component");
  ...
  it("reloads a committed dependent onto the replacement provider generation", async () => {
    ...
    const v1 = createProviderComponent({
      provider: "memory",
      description: "Memory provider component",
      create: () => new RevisionProvider("memory", "v1", () => events.push("provider-v1-close")),
    });
    catalog.register(v1.definition);
    await loader.installPinned([v1.entry]);
    catalog.register({
      ...
      async activate(context) {
        ...
        return () => {
          events.push(`consumer-${result.revision}-stop`);
        };
      },
    });
  ...
  it("restores the prior provider namespace after a candidate activation fails", async () => {
    ...
    const broken = createProviderComponent({
      provider: "memory",
      description: "Memory provider component",
      create: () => new RevisionProvider("memory", "broken", () => events.push("broken-close")),
      start: () => {
  ...
  it("closes a provider whose factory returns the wrong namespace", async () => {
    ...
    await expect(manifest.install(component)).rejects.toThrow("created wrong, expected memory");
    expect(events).toEqual(["closed"]);
    expect(manifest.entries()).toEqual([]);
    expect(catalog.get("raft.provider.memory")).toBeUndefined();
    await loader.close();
    await registry.close();
  });
});
