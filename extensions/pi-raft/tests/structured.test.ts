...
import { formatRaftValue } from "../src/ui/structured.js";

describe("formatRaftValue", () => {
  const value = { status: "completed", findings: [{ fixed: true }] };

  it("formats structured auto results as YAML", () => {
    const formatted = formatRaftValue(value, "auto");
  ...
  it("preserves explicit JSON and text modes", () => {
    expect(formatRaftValue(value, "json")).toEqual({
      text: JSON.stringify(value, null, 2),
      language: "json",
    });
    expect(formatRaftValue({ text: "plain result", metadata: true }, "text")).toEqual({
      text: "plain result",
    });
  });

  it("keeps string values unchanged in auto mode", () => {
    expect(formatRaftValue("already textual", "auto")).toEqual({ text: "already textual" });
  });
});
...
describe("multi-line string fidelity", () => {
  ...
  it("hoists multi-line strings so content keeps its exact bytes", () => {
    const formatted = formatRaftValue({ content: file }, "auto");
  ...
  it("labels hoisted sections with their value path", () => {
    const formatted = formatRaftValue({ results: ["one\ntwo", { note: "a\nb" }] }, "auto");
  ...
  it("shares an output budget across every multiline section", () => {
    ...
    const formatted = formatRaftValue(
  ...
  it("keeps single-line strings inline in the YAML skeleton", () => {
    const formatted = formatRaftValue({ a: "x", b: 1 }, "auto");
  ...
  it("bounds YAML highlighting to the skeleton when raw sections are appended", () => {
    ...
    expect(formatted.text.split("\n")[formatted.highlightedLineCount! + 1]).toMatch(/^--- content/);
  });
});
