...
const temporaryDirectory = (name: string): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-memory-lineage-${name}-`));
...
const message = (id: string, parentId: string | null, text: string, offset: number): FixtureEntry =>
  messageEntry(id, parentId, timestamp(offset), userMessage(text));

const invocation = (cwd: string): RaftInvocationContext => ({
  cwd,
  signal: undefined,
...
const memoryConfig = (
  indexDir: string,
  overrides: Partial<RaftMemoryConfig> = {},
): RaftMemoryConfig => ({
  enabled: true,
  indexDir,
  maxSessions: 500,
...
describe("memory active lineage and privacy policy", () => {
  ...
  it("uses the current live SessionManager branch getter after navigation without an append", async () => {
    ...
    const provider = new MemoryProvider({
      ...
      sessionId: "live",
      config: memoryConfig(indexDir),
      getLiveBranch: () => ({ entries: manager.getBranch(), leafId: manager.getLeafId() }),
    });
  ...
  it("keeps cold active and all vocabularies in separate non-contaminating caches", async () => {
    ...
    const activeCache = JSON.parse(
      fs.readFileSync(digestPathForSession(file, indexDir), "utf8"),
    ) as { branches: string; vocabulary: string[] };
    const allCache = JSON.parse(
      fs.readFileSync(digestPathForSession(file, indexDir, "all"), "utf8"),
    ) as { branches: string; vocabulary: string[] };
  ...
  it("refuses off-lineage addresses by default, expands them in all mode, and binds active pointers to lineage", async () => {
    ...
    const recalled = (await provider.invoke(
      ...
      hits: Array<{
        follow: { args: { expectedSourceHash: string; expectedLineageFingerprint: string } };
      }>;
    };
    ...
    live = { entries: [{ id: "root" }, { id: "off-lineage" }], leafId: "off-lineage" };
    ...
    expect(staleLineage.error.actualLineageFingerprint).not.toBe(
      pointer.expectedLineageFingerprint,
    );
  ...
  it("keeps compaction and typed branch-summary carriers on the active path searchable and expandable", async () => {
    ...
    expect(expanded.entries[0]).toEqual(
  ...
  it("applies thinking and tool-output indexing policy and rebuilds policy-bound caches", async () => {
    ...
    const file = writeSessionFile(
      path.join(agentDir, "sessions", encodeCwdDir(cwd)),
      "privacy.jsonl",
      [sessionHeader("privacy", cwd), assistant, result],
    );
    const provider = (overrides: Partial<RaftMemoryConfig> = {}) =>
      new MemoryProvider({ agentDir, cwd, config: memoryConfig(indexDir, overrides) });
    ...
    expect(
      firstCache.entries.every((entry) => !entry.text.includes("PRIVATE_THINKING_TOKEN_41")),
    ).toBe(true);
    ...
    const thinkingCache = JSON.parse(
      fs.readFileSync(shardPathForSession(file, indexDir), "utf8"),
    ) as { policy: string };
    ...
    const optOutCache = JSON.parse(
      fs.readFileSync(shardPathForSession(file, indexDir), "utf8"),
    ) as { policy: string; entries: Array<{ text: string }> };
    expect(optOutCache.policy).not.toBe(thinkingCache.policy);
    expect(
      optOutCache.entries.some((entry) => entry.text.includes("toolResult(read) [error]")),
    ).toBe(true);
    expect(optOutCache.entries.every((entry) => !entry.text.includes("TOOL_OUTPUT_BODY_42"))).toBe(
      true,
    );
  });
  ...
  it("preserves project/global duplicate-session coverage and pagination in both branch modes", async () => {
    ...
    const first = writeSessionFile(
      path.join(agentDir, "sessions", encodeCwdDir(cwd)),
      "one.jsonl",
      [sessionHeader("duplicate-id", cwd), message("one", null, "SCOPE_COMMON_TOKEN_51", 0)],
    );
    const second = writeSessionFile(
      path.join(agentDir, "sessions", encodeCwdDir(otherCwd)),
      "two.jsonl",
      [sessionHeader("duplicate-id", otherCwd), message("two", null, "SCOPE_COMMON_TOKEN_51", 0)],
    );
    const provider = new MemoryProvider({ agentDir, cwd, config: memoryConfig(indexDir) });
    ...
    for (const branches of ["active", "all"] as const) {
      ...
      const pageTwo = (await provider.invoke("recall", pageOne.next!.args, invocation(cwd))) as {
        total: number;
        next: null;
        hits: unknown[];
      };
