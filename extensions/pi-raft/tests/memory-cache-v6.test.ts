...
const temporaryDirectory = (name: string): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-memory-v6-${name}-`));
...
describe("memory cache V6", () => {
  ...
  it("paginates no-query browsing without a pre-pagination session cap while query coverage is complete", async () => {
    ...
    const browse = (await provider().invoke(
      ...
      { scope: "project" },
      invocationContext(cwd),
    )) as { total: number; coverage: { eligibleSessions: number } };
    ...
    const search = (await provider({ hotSessions: 1 }).invoke(
      ...
      coverage: { complete: boolean; eligibleSessions: number };
      hits: Array<{ sessionId: string }>;
    };
    expect(search.coverage).toEqual(
      expect.objectContaining({ complete: true, eligibleSessions: 20 }),
    );
  ...
  it("rebuilds rewritten and V5 caches and removes caches for deleted sources", async () => {
    ...
    const v5 = {
      ...first,
      cacheVersion: 5,
      addresses: first.addresses.map((address) => address.slice(0, 6)),
    };
  ...
  it("hydrates bounded ranges explicitly and expands by stable entry id", async () => {
    ...
    const old = writeSessionFile(sessionDirectory, "old.jsonl", [
      ...
      messageEntry(
        ...
        assistantText("rare bounded fact"),
      ),
    ...
    expect(pointer.hits[0]!.follow).toEqual(
      expect.objectContaining({
        ref: "memory.recall",
