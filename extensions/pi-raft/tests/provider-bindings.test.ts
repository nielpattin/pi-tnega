...
const invoke = (
  ...
  invocationContext: RaftInvocationContext = context,
): Promise<unknown> =>
  registry.invoke(
    "demo.echo",
    {},
...
describe("provider binding generations", () => {
  it("keeps a retiring provider alive for committed views and protects replacements from stale leases", async () => {
    ...
    await expect(
      registry.describe("demo.uncommitted", { ...context, capabilityView: pinned.view! }),
  ...
  it("records overlapping effect footprints and enforces strict policy", async () => {
    ...
    const call = (
      ...
      registry.invoke(
        `effects.${name}`,
        {},
        { ...context, effectPolicy, approve: async () => {}, audits, maxResultChars: 10_000 },
      );
    ...
    expect(advisoryAudits[0]?.effectConflicts).toEqual([
      { withRef: "effects.first", resources: ["workspace:a"], reason: "shared_resource" },
    ]);
  ...
  it("keeps staged providers hidden until an atomic activation", async () => {
    ...
    expect(() => registry.register(provider("intruder"))).toThrow(
      "Raft provider already registered: demo",
    );
  ...
  it("only lets an explicitly overwriting staged binding replace a current provider", async () => {
    ...
    const replacement = registry.mount(provider("replacement"), { staged: true, overwrite: true });
  ...
  it("acquires scoped actions with schema checks and a single-shot disposer", async () => {
    ...
    registry.register({
      ...
      async list() {
        return [(await this.describe("open", context))!];
      },
      async describe(name) {
        return name === "open"
          ? {
    ...
    await expect(
      registry.acquireScoped("lease.open", {}, { ...context, capabilityView: pinned.view! }),
    ).rejects.toThrow("Invalid arguments");
    const acquired = await registry.acquireScoped(
      "lease.open",
      { key: "value" },
  ...
  it("rejects descriptor drift in a committed view", async () => {
    ...
    const pinned = await registry.acquireCapabilityView(["demo.echo"], context);
    description = "after";
    await expect(invoke(registry, { ...context, capabilityView: pinned.view! })).rejects.toThrow(
      "Raft capability descriptor changed: demo.echo",
    );
    await pinned.release();
    await registry.close();
  });
