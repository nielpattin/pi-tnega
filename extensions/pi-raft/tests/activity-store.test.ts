...
describe("RaftActivityStore", () => {
  it("tracks dynamic phases, calls, entities, metrics, and custom items", () => {
    ...
    store.updateCall("run-1", "call-1", { type: "metrics", tokens: 1200, toolCalls: 4 });
  ...
  it("keeps item phase ownership stable unless an update explicitly moves it", () => {
    ...
    store.upsertItem("run-item-phase", { id: "worker", label: "Worker", status: "running" });

    const collect = store.phase("run-item-phase", { name: "Collect" });
    store.upsertItem("run-item-phase", { id: "worker", label: "Worker", status: "completed" });
  ...
  it("summarizes finished call results into a detail field", () => {
    ...
    store.finishCall("run-d", "bash-1", {
      success: true,
      result: { ok: true, output: "line1\nline2" },
    });
  ...
  it("bounds retained call payloads and run history", () => {
    ...
    expect(call?.args).toMatchObject({ raftTruncated: true });
    expect(call?.result).toMatchObject({ raftTruncated: true });
    expect(call?.preview).toMatchObject({ raftTruncated: true });
  ...
  it("falls back to a lexical name hint only when no display name is declared", () => {
    ...
    expect(plain.name).toBe("Raft program");
  });
});
