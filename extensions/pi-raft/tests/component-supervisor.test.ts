...
const echoProvider = (name = "svc", value = "ok", close?: () => void): RaftProvider => ({
  name,
...
describe("RaftComponentSupervisor", () => {
  ...
  it("keeps declaration cycles waiting and reports their exact path", async () => {
    ...
    await supervisor.start(entry("alpha"), {
      ...
      provides: ["alpha"],
      activate(context) {
        context.provide(echoProvider("alpha"));
      },
    });
    await supervisor.start(entry("beta"), {
      ...
      provides: ["beta"],
      activate(context) {
        context.provide(echoProvider("beta"));
      },
    });

    expect(supervisor.status("alpha")).toMatchObject({ state: "waiting", missing: ["beta.echo"] });
    expect(supervisor.status("beta")).toMatchObject({ state: "waiting", missing: ["alpha.echo"] });
  ...
  it("retires providers, unloads dependents, then releases owner effects", async () => {
    ...
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    const events: string[] = [];
    const owner: RaftComponentDefinition = {
      name: "owner",
      provides: ["svc"],
      activate(context) {
        context.provide(
          echoProvider("svc", "owned", () => {
    ...
    const dependent: RaftComponentDefinition = {
      name: "dependent",
      requires: ["svc.echo"],
      activate(context) {
  ...
  it("rolls back a failed replacement to the previous revision", async () => {
    ...
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    const events: string[] = [];
    const stable: RaftComponentDefinition = {
      name: "service",
      activate() {
        events.push("stable-start");
        return () => {
          events.push("stable-cleanup");
        };
      },
    };
    const broken: RaftComponentDefinition = {
      name: "service",
      activate() {
    ...
    await expect(supervisor.replace("service", entry("service"), broken)).rejects.toThrow(
      "candidate failed; previous revision restored",
    );
    expect(supervisor.status("service")).toMatchObject({ state: "active", revision: 3 });
    expect(events).toEqual(["stable-start", "stable-cleanup", "broken-start", "stable-start"]);
    await supervisor.close();
    await registry.close();
  });
  ...
  it("reconciles active components when a provider descriptor changes in place", async () => {
    ...
    registry.register({
      ...
      async list() {
        return [(await this.describe("read", invocationContext()))!];
      },
      async describe(name) {
        return name === "read"
          ? {
    ...
    expect(events).toEqual(["start:version one", "stop:version two", "start:version two"]);
  ...
  it("automatically disposes scoped acquisitions", async () => {
    ...
    registry.register({
      ...
      async list() {
        return [(await this.describe("open", invocationContext()))!];
      },
      async describe(name) {
        return name === "open"
          ? {
  ...
  it("rejects emission effects from revertible components", async () => {
    ...
    registry.register({
      ...
      async list() {
        return [(await this.describe("write", invocationContext()))!];
      },
      async describe(name) {
        return name === "write"
          ? {
    ...
    await expect(
      supervisor.start(entry("safe"), {
        name: "safe",
        requires: ["emit.write"],
        guarantee: "revertible",
        async activate(context) {
          await context.call("emit.write");
        },
      }),
  ...
  it("quarantines activation when setup rollback cleanup also fails", async () => {
    ...
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    await expect(
      supervisor.start(entry("broken-setup"), {
        name: "broken-setup",
        async *activate() {
          yield () => {
  ...
  it("rejects overlapping component provisions before disturbing either fiber", async () => {
    ...
    await supervisor.start(entry("beta"), {
      ...
      activate() {
        return () => {
          events.push("beta-cleanup");
        };
      },
    });

    await expect(
      supervisor.start(entry("gamma"), {
        name: "gamma",
        provides: ["svc"],
        activate(context) {
          context.provide(echoProvider("svc"));
    ...
    await expect(
      supervisor.replace("beta", entry("beta"), {
        name: "beta",
        provides: ["svc"],
        activate(context) {
          context.provide(echoProvider("svc"));
  ...
  it("reserves staged provision names and heals externally retired provisions", async () => {
    ...
    const provisionStaged = new Promise<void>((resolve) => {
      staged = resolve;
    });
    let activations = 0;
    ...
    expect(() => registry.register(echoProvider("svc", "intruder"))).toThrow(
      "Raft provider already registered: svc",
    );
    release();
    await activation;
    expect(
      await registry.invoke(
        "svc.echo",
        {},
        { ...invocationContext(), approve: async () => {}, audits: [], maxResultChars: 10_000 },
      ),
    ).toBe("generation-1");
  ...
  it("cooperatively aborts an in-flight activation before awaiting its inertia", async () => {
    ...
    expect(() => supervisor.status("abortable")).toThrow("Unknown Raft component");
    await supervisor.close();
    await registry.close();
  });
  ...
  it("completes self-retirement requested during activation", async () => {
    ...
    expect(() => supervisor.status("self-removing")).toThrow("Unknown Raft component");
    await supervisor.close();
    await registry.close();
  });
  ...
  it("does not deadlock on lifecycle calls re-entered from teardown", async () => {
    ...
    await supervisor.stop("self-aware");
    expect(events).toEqual(["Cannot settle Raft components from unloading transition self-aware"]);
    await supervisor.close();
    await registry.close();
  });
  ...
  it("owns child fibers as registration effects and unloads them before the parent", async () => {
    ...
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    const events: string[] = [];
    let childHandle: ReturnType<RaftComponentContext["use"]> | undefined;
    const child: RaftComponentDefinition = {
      name: "child",
      activate() {
    ...
    expect(events).toEqual(["parent-start", "child-start", "child-cleanup", "parent-cleanup"]);
    expect(() => supervisor.status("parent.worker")).toThrow("Unknown Raft component");
  ...
  it("enforces and reports lifetime effect independence for revertible fibers", async () => {
    const registry = new ActionRegistry();
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    let disposeFirst!: Awaited<ReturnType<RaftComponentContext["effect"]>>;
  ...
  it("treats an unknown component footprint as top rather than a literal resource", async () => {
    ...
    await supervisor.start(entry("unknown"), {
      name: "unknown",
      async activate(context) {
        await context.effect(() => () => {}, { label: "unknown mutation", ordering: "ordered" });
      },
    });

    await expect(
      supervisor.start(entry("named"), {
        name: "named",
        guarantee: "revertible",
        async activate(context) {
          await context.effect(() => () => {}, {
    ...
    ).rejects.toThrow("unknown [*] (unknown resource footprint; declare resources and ordering)");
    await supervisor.close();
    await registry.close();
  });
  ...
  it("commits model guidance transactionally and removes it on disposal or rollback", async () => {
    const registry = new ActionRegistry();
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    let disposeGuidance!: ReturnType<RaftComponentContext["guide"]>;
    let retainedContext!: RaftComponentContext;
    ...
    expect(supervisor.status("guidance")).toMatchObject({
      guidance: [
        expect.objectContaining({ label: "deepseek-flash", placement: "append", contentChars: 23 }),
      ],
      effects: [
        expect.objectContaining({
    ...
    expect(supervisor.guidance()).toEqual([
      expect.objectContaining({ label: "live-guidance", content: "Registered after activation." }),
    ]);
    await disposeLiveGuidance();
    expect(supervisor.guidance()).toEqual([]);

    await expect(
      supervisor.start(entry("failed-guidance"), {
  ...
  it("allows witnessed commutative component effects on the same resource", async () => {
    const registry = new ActionRegistry();
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    const definition = (name: string): RaftComponentDefinition => ({
      name,
      guarantee: "revertible",
      async activate(context) {
        await context.effect(() => () => {}, {
  ...
  it("tears independent siblings down in reverse activation order", async () => {
    ...
    const supervisor = new RaftComponentSupervisor(registry, { invocationContext });
    const events: string[] = [];
    const definition = (name: string, revision: string): RaftComponentDefinition => ({
      name,
      activate() {
        return () => {
          events.push(`${name}:${revision}`);
        };
      },
    });
  ...
  it("quarantines cleanup failures instead of claiming disposal", async () => {
    ...
    await supervisor.start(entry("leaky"), {
      ...
      activate() {
        return () => {
          throw new Error("cleanup leaked");
        };
      },
    });
    ...
    expect(() => supervisor.status("leaky")).toThrow("Unknown Raft component");
    await supervisor.close();
    await registry.close();
  });
});
