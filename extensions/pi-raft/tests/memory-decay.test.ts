...
import { normalizeRaftConfig, type RaftMemoryConfig } from "../src/config.js";
...
const makeTempDir = (name: string): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-decay-${name}-`));
...
const memoryConfig = (indexDir: string, hotSessions = 1): RaftMemoryConfig => ({
  enabled: true,
  indexDir,
  maxSessions: 500,
  maxEntryChars: 2_000,
...
describe("memory sleep cycle", () => {
  ...
  const projectRefs = () => resolveScope({ agentDir, cwd, scope: "project", maxSessions: 500 });
  ...
  it("produces identical mixed-tier recall across repeated runs", async () => {
    ...
    const provider = new MemoryProvider({ agentDir, cwd, config: memoryConfig(indexDir) });
    const run = async () =>
      provider.invoke("recall", { scope: "project", query: "saturn" }, invocationContext(cwd));

    expect(await run()).toEqual(await run());
  });

  it("normalizes tier policy defaults and bounds", () => {
    expect(normalizeRaftConfig({}).memory).toEqual(
      expect.objectContaining({
        hotSessions: 50,
        digestTerms: 200,
    ...
    expect(
      normalizeRaftConfig({
        memory: { hotSessions: -1, digestTerms: 0, indexThinking: true, indexToolOutput: false },
      }).memory,
    ).toEqual(
      expect.objectContaining({
        hotSessions: 0,
