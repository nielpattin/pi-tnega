...
import type { RaftMcpConfig } from "../src/config.js";
...
const mcpConfig = (overrides: Partial<RaftMcpConfig> = {}): RaftMcpConfig => ({
  enabled: true,
  disableOAuth: true,
...
const cacheConfig = (
  configPath: string,
  cacheOverrides: Partial<RaftMcpConfig["cache"]> = {},
): RaftMcpConfig =>
  mcpConfig({
    configPath,
...
const temporaryDirectory = (): string => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-mcp-"));
    ...
    JSON.stringify({
      mcpServers: {
        test: {
          ...
          args: [FAKE_SERVER],
          env: { PI_RAFT_MCP_COUNT_FILE: options.countFile, PI_RAFT_MCP_COUNT_LABEL: "test" },
        },
        "fal-ai": {
          command: process.execPath,
          args: [FAKE_SERVER, ...(options.falExtraArgs ?? [])],
          env: { PI_RAFT_MCP_COUNT_FILE: options.countFile, PI_RAFT_MCP_COUNT_LABEL: "fal-ai" },
        },
      },
      imports: [],
    }),
    ...
    try {
      ...
      expect(listed).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: "test.echo-value", risk: "network" }),
        ]),
      );
      const described = await provider.describe("test.echo_value", context);
      expect(described?.inputSchema).toMatchObject({ required: ["value"] });
      await expect(
        provider.invoke("test.echo_value", { value: "hello" }, context),
      ).resolves.toMatchObject({ text: "echo:hello" });
      await expect(
        provider.invoke("test.echo_value", { value: "again" }, context),
      ).resolves.toMatchObject({ text: "echo:again" });
      ...
      await expect(
        provider.invoke("fal_ai.get_model_schema", { endpoint_id: "openai/gpt-image-2" }, context),
      ...
      await expect(
        provider.invoke(
          "$register",
          {
            ...
            env: { PI_RAFT_MCP_COUNT_FILE: countFile, PI_RAFT_MCP_COUNT_LABEL: "dynamic-server" },
          },
          context,
        ),
      ...
      expect(countLines(countFile).sort()).toEqual(["dynamic-server", "fal-ai", "test"]);
    } finally {
...
describe("McpProvider descriptor cache", () => {
  it("populates once, then serves warm sessions without spawning servers", async () => {
    ...
    const countFile = path.join(directory, "tools-list.log");
    const configPath = writeTwoServerConfig({ directory, countFile });
    const cachePath = path.join(directory, ".pi", "raft", "mcp-cache.json");
    ...
    try {
      ...
      expect(listed.map((descriptor) => descriptor.name)).toEqual(
        expect.arrayContaining(["test.echo-value", "test.get-model-schema", "fal-ai.echo-value"]),
      );
  ...
  it("revalidate: off never spawns in the background; explicit probes fetch exactly one server", async () => {
    ...
    const second = new McpProvider(directory, cacheConfig(configPath, { revalidate: "off" }), {
      cache: new McpDescriptorCacheStore(cachePath),
    });
    try {
      const listed = await second.list({}, context);
  ...
  it("keeps last-known tools marked stale when revalidation fails", async () => {
      ...
      JSON.stringify({
        mcpServers: {
          flaky: {
            command: process.execPath,
            args: [FLAKY_SERVER],
            env: { PI_RAFT_MCP_FLAKY_STATE: stateFile },
          },
        },
        imports: [],
      }),
    ...
    const first = new McpProvider(directory, cacheConfig(configPath, { revalidate: "all" }), {
      cache: new McpDescriptorCacheStore(cachePath),
    });
    try {
      await first.list({}, context);
      await first.settle();
      expect((await first.list({}, context)).map((descriptor) => descriptor.name)).toContain(
        "flaky.flaky-ping",
      );
    } finally {
      await first.close();
    ...
    const second = new McpProvider(directory, cacheConfig(configPath, { revalidate: "all" }), {
      cache: new McpDescriptorCacheStore(cachePath),
    });
    try {
      ...
      expect(servers).toEqual([expect.objectContaining({ name: "flaky", tools: 1, stale: true })]);
    } finally {
      await second.close();
    }
  }, 30_000);
  ...
  it("invokes through the cache, reports organic use, and relists on first contact", async () => {
    ...
    try {
      ...
      await expect(second.invoke("test.nope", {}, context)).rejects.toThrow("Unknown MCP tool");
