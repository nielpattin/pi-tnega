...
import { RaftEffectScope } from "../src/components/effect-scope.js";

const failClosing = (): never => {
  throw new Error("close failed");
};
describe("RaftEffectScope", () => {
  it("disposes effects once in reverse registration order", async () => {
    const scope = new RaftEffectScope();
    const calls: string[] = [];
    const first = await scope.effect(
      () => () => {
        calls.push("first");
      },
      "first",
  ...
  it("continues cleanup and reports quarantine after disposer failures", async () => {
    ...
    expect(() => scope.defer(() => {})).toThrow(/disposing Raft scope/);
  });

  it("diverts at a yield boundary and rolls back only landed iterations", async () => {
    let targetCurrent = true;
    const scope = new RaftEffectScope({ guard: () => targetCurrent });
    const calls: string[] = [];
