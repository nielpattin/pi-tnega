...
import { RaftModelSelector } from "../src/ui/raft-model-selector.js";
...
const render = (component: RaftModelSelector): string => component.render(80).join("\n");

describe("RaftModelSelector", () => {
  it("renders the Inherit row plus every model, Inherit pinned on top", () => {
    const component = new RaftModelSelector({
      theme,
      source,
  ...
  it("uses custom headerText and inheritName when provided", () => {
    const component = new RaftModelSelector({
      ...
      source,
      currentValue: INHERIT_VALUE,
      headerText: 'Model for actor "reviewer". Pick Inherit to use the Raft default.',
      inheritName: "Use the Raft default model (or host default)",
      onSelect: () => {},
      onCancel: () => {},
    });
    const text = render(component);
    expect(text).toContain('Model for actor "reviewer". Pick Inherit to use the Raft default.');
    // The Inherit row's footer name reflects the custom inherit description.
    expect(text).toContain("Use the Raft default model (or host default)");
    // The default global wording is no longer present.
    expect(text).not.toContain("Default model for Raft agents and actors");
  ...
  it("defaults to the global wording when headerText/inheritName are omitted", () => {
    ...
    const text = render(component);
    expect(text).toContain("Default model for Raft child agents");
    expect(text).toContain("Use the host session's default model");
  });

  it("omits the Inherit row when allowInherit is false", () => {
    const component = new RaftModelSelector({
      theme,
      source,
      currentValue: "anthropic/claude-sonnet-4-5",
  ...
  it("renders top and bottom dynamic borders when bordered is true", () => {
    const component = new RaftModelSelector({
      theme,
      source,
      currentValue: "anthropic/claude-sonnet-4-5",
