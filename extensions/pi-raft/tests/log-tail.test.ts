...
describe("readJsonlPage", () => {
  it("returns bounded tail pages with stable older-page cursors", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-log-tail-"));
    roots.push(root);
  ...
  it("reads a tail page that begins beyond the final file chunk", () => {
    ...
    expect(page.lines.map((line) => (line.parsed as { index: number }).index)).toEqual([
      1997, 1998, 1999,
    ]);
    expect(page.hasMore).toBe(true);
    const older = readJsonlPage(file, 3, page.before);
    expect(older.lines.map((line) => (line.parsed as { index: number }).index)).toEqual([
      1994, 1995, 1996,
    ]);
  });
