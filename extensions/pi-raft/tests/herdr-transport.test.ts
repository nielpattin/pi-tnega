...
const startServer = async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-herdr-"));
  roots.push(root);
  ...
  const server = net.createServer((socket) => {
    ...
    socket.on("data", (chunk: string) => {
      ...
      if (request.method === "ping") {
        ...
        response = {
          id: request.id,
          result: { type: "pane_info", pane: { pane_id: "w1:p2", terminal_id: "term_worker" } },
        };
  ...
  it("launches an argv-backed background tab and controls it by pane id", async () => {
    ...
    expect(apply?.params).toEqual({
      ...
      root: {
        ...
        label: "review worker",
        cwd: "/repo with spaces",
        command: [process.execPath, "/raft/worker.js", "--task-file", "/tmp/task with spaces.txt"],
      },
    });
