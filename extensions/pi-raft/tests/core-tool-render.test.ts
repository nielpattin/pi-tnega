...
const options = (overrides: Partial<CoreToolRenderOptions> = {}): CoreToolRenderOptions => ({
  cwd: process.cwd(),
  settings,
  expanded: false,
  maxLines: 200,
...
const audit = (
  tool: string,
  values: Omit<RaftRenderAudit, "ref" | "provider" | "tool">,
): RaftRenderAudit => ({ ref: `pi.${tool}`, provider: "pi", tool, ...values });

describe("Raft core tool parity rendering", () => {
  ...
  it.each([
    ...
    { tool: "ls", args: { path: "src" }, output: "transcript.ts", expected: "transcript.ts" },
    {
      tool: "bash",
  ...
  it("renders write result diffs with summaries, gutters, word emphasis, and full-row backgrounds", () => {
    const rendered = renderCoreToolBody(
      audit("write", {
        args: { path: "src/example.ts", content: `const value = "${"new".repeat(40)}";` },
        preview: {
          details: {
  ...
  it("renders bash warnings, timeout metadata, output limits, and full output details", () => {
    ...
    const title = coreToolTitle(call, theme, { cwd: process.cwd(), settings });
  ...
  it("keeps bash command rows aligned when the command embeds raw carriage returns", async () => {
    ...
    await vi.waitFor(() => expect(highlightCode("echo ready", "bash", invalidate)).not.toBeNull(), {
      timeout: 15_000,
    });
    const highlighted = renderCoreToolBody(call, theme, options());
    expect(highlighted!.lines.map(strip)).toEqual(expected);
    expect(highlighted!.lines.slice(0, 4).every((row) => row.includes("\x1b[38;2;"))).toBe(true);
  }, 20_000);
  ...
  it("lazily invalidates and highlights every syntax-aware core preview", async () => {
    ...
    expect(previews.map((render) => render()).every((text) => !text.includes("\x1b[38;2;"))).toBe(
      true,
    );
...
describe("Raft core tool stateful highlighting", () => {
  let dir: string;
  let file: string;
  ...
  beforeEach(async () => {
    ...
    dir = mkdtempSync((await import("node:path")).join(tmpdir(), "pi-raft-render-"));
  ...
  it("keeps compact multi-hunk edits on full-file grammar state", async () => {
    ...
    const rendered = renderCoreToolBody(
      audit("edit", { args: { path }, success: true, result: { details: { diff } } }),
      theme,
      options({ expanded: true }),
    );
    expect(rendered).not.toBeNull();
    ...
    expect(
      new Set(foregrounds(highlightCode("interface First {", "typescript")![0]!)).size,
    ).toBeGreaterThan(1);
  ...
  it("colors removed lines with pre-edit grammar state instead of a flat fragment", async () => {
    ...
    const call = audit("edit", { args: { path }, success: true, result: { details: { diff } } });
    ...
    const removedColored = (): string | undefined => {
      const rendered = renderCoreToolBody(
        call,
        theme,
        options({ expanded: true, invalidate: vi.fn() }),
      );
  ...
  it("weaves removed lines past separators and in-hunk scope openers", async () => {
    ...
    const filler = Array.from(
      { length: 13 },
      (_, index) => `const filler${index + 1} = ${index + 1};`,
    );
    const post = [
      ...filler,
    ...
    const removedCommentColored = (): string | undefined => {
      ...
      const row = rendered!.lines.find((line) =>
        stripAnsi(line).includes("a pure-consumer plugin"),
      );
      if (!row) return undefined;
      const deEmphasized = row
        ...
        .replace(/\x1b\[49m/g, "");
      return deEmphasized.includes(`${commentColor} * No runtime invariant: a `)
        ? deEmphasized
        : undefined;
    };
    ...
    const addedRow = renderCoreToolBody(
      call,
      theme,
      options({ expanded: true, invalidate: vi.fn() }),
