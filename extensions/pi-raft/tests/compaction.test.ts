...
import { normalizeRaftConfig, DEFAULT_RAFT_CONFIG } from "../src/config.js";
import {
  computeCut,
  compileRaftSummary,
  type RaftCompactionBudgetDetails,
  raftCompactionVersion,
...
const iso = (n: number): string =>
  `2024-01-0${1 + Math.floor(n / 86400)}T00:00:${String(n % 60).padStart(2, "0")}Z`;
let clock = 0;
...
const thinkingPart = (thinking: string): { type: "thinking"; thinking: string } => ({
  type: "thinking",
  thinking,
});
...
describe("thinking erasure", () => {
  it("normalizes assistant thinking parts into no events", () => {
    ...
    const entries = [
      ...
      assistant(
        thinkingPart("**Planning updater patch**\n\na long deliberation"),
        textPart("done"),
      ),
  ...
  it("erases thinking from rendered summaries and records the count in details", () => {
    resetIds();
    resetClock();
    const result = compileRaftSummary(
      buildSession(
        user("Keep the original goal"),
        assistant(
          thinkingPart("**Planning updater patch**\n\ndeliberation detail"),
          textPart("first step done"),
        ),
    ...
    expect(raftCompactionVersion(details)).toBe(2);
  });
  ...
  it("keeps v2 detail validation compatible before the thinking count existed", () => {
    ...
    expect(raftCompactionVersion(legacy)).toBe(2);
  });
...
describe("compaction config", () => {
  it("defaults to the raft engine and a 65% post-compaction ceiling", () => {
    expect(DEFAULT_RAFT_CONFIG.lifecycle.compaction.engine).toBe("raft");
    expect(DEFAULT_RAFT_CONFIG.lifecycle.compaction.targetContextRatio).toBe(0.65);
  });

  it("normalizes the engine escape hatch and bounded occupancy ceiling", () => {
    const configured = normalizeRaftConfig({
      lifecycle: { compaction: { engine: "pi", targetContextRatio: 0.7 } },
    ...
    expect(
      normalizeRaftConfig({ lifecycle: { compaction: { engine: "bogus", targetContextRatio: 2 } } })
        .lifecycle.compaction,
    ).toEqual({ engine: "raft", targetContextRatio: 0.85, thresholds: {}, tokenThresholds: {} });
    expect(
      normalizeRaftConfig({ lifecycle: { compaction: { targetContextRatio: 0.1 } } }).lifecycle
        .compaction.targetContextRatio,
    ).toBe(0.25);
    expect(
      normalizeRaftConfig({ lifecycle: { compaction: { targetContextRatio: "large" } } }).lifecycle
        .compaction.targetContextRatio,
    ).toBe(0.65);
  });
  ...
  it("normalizes model-linked thresholds", () => {
    ...
    ).toEqual({ "anthropic/sonnet": 0.8, "openai/gpt": 0.95 });
  });
  ...
  it("normalizes model-linked token thresholds", () => {
    expect(
      ...
      }).lifecycle.compaction.tokenThresholds,
    ).toEqual({
      "anthropic/sonnet": 128_000,
      "openai/gpt": 1_000,
...
describe("compaction pi-vcc interop", () => {
  it("defers to an explicit /pi-vcc sentinel", () => {
    ...
    expect(compactionHandler("raft")(event)).toBeUndefined();
    expect(event._raftCompaction).toBeUndefined();
  });

  it("marks the mutable event when raft claims compaction", () => {
    resetIds();
    resetClock();
    const event = compactionEvent(buildSession(user("compact this"), assistant(textPart("done"))));

    expect(compactionHandler("raft")(event)).toHaveProperty("compaction");
    expect(event._raftCompaction).toBe(true);
  });
  ...
  it("cancels empty compaction when pi-vcc has not produced a summary", () => {
    ...
    expect(compactionHandler("raft")(event)).toEqual({ cancel: true });
    expect(event._raftCompaction).toBeUndefined();
  });

  it("allows an unrelated later Pi before hook to replace Raft's result", () => {
    resetIds();
    resetClock();
    ...
    const pi = {
      on(name: string, handler: unknown) {
        if (name === "session_before_compact")
          handlers.push(handler as (event: SessionBeforeCompactEvent) => unknown);
      },
    } as unknown as ExtensionAPI;
    registerCompactionHook(pi, { getEngine: () => "raft" });
    handlers.push(() => ({
      compaction: { summary: "later extension", firstKeptEntryId: "", tokensBefore: 1 },
    }));
  ...
  it("leaves the pi engine passthrough unchanged", () => {
    resetIds();
    resetClock();
    const event = compactionEvent(buildSession(user("use pi core"), assistant(textPart("done"))));
...
describe("compaction instruction parity", () => {
  const summaryFor = (
    instructions: string,
  ): { summary: string; details: { instructionPolicy: { mode: string; truncated: boolean } } } => {
    resetIds();
    resetClock();
    const result = compileRaftSummary(
  ...
  it.each([
    ["malformed-json", `${RAFT_COMPACTION_REQUEST_PREFIX}{not-json}`],
    [
      "unsupported-version",
      `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({ version: 2, instructions: "fake goal" })}`,
    ],
    [
      "unknown-field",
      `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({ version: 1, goal: "fake goal" })}`,
    ],
    [
      "invalid-type",
      `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({ version: 1, preserve: ["ok", 7] })}`,
    ],
  ...
  it.each([
    ...
    [
      "escaped duplicate preserve",
      '{"version":1,"preser\\u0076e":[],"preserve":[]}',
      "duplicate-field",
    ],
    [
      "duplicate instructions",
      '{"version":1,"instructions":"a","instructions":"b"}',
      "duplicate-field",
    ],
    ...
    expect(decodeCompactionInstructions(`${RAFT_COMPACTION_REQUEST_PREFIX}${json}`)).toMatchObject({
      ok: false,
      error: { code },
    });
  });
  ...
  it("accepts paired supplementary characters and rejects raw unpaired values before canonicalization", () => {
    const paired = `${RAFT_COMPACTION_REQUEST_PREFIX}{"version":1,"instructions":"\\ud83d\\ude80"}`;
    expect(decodeCompactionInstructions(paired)).toMatchObject({ ok: true });
    const rawUnpaired = `${RAFT_COMPACTION_REQUEST_PREFIX}{"version":1,"instructions":"${String.fromCharCode(0xd800)}"}`;
    expect(decodeCompactionInstructions(rawUnpaired)).toMatchObject({
      ok: false,
      error: { code: "invalid-unicode" },
    });
    expect(() => encodeCompactionRequest({ preserve: [String.fromCharCode(0xdfff)] })).toThrow(
      /unpaired UTF-16 surrogate/,
    );
  });

  it("rejects typed source bytes before JSON parsing", () => {
    const source = `${RAFT_COMPACTION_REQUEST_PREFIX}${" ".repeat(MAX_TYPED_COMPACTION_SOURCE_BYTES)}`;
  ...
  it("rejects instruction bytes plus preserve count and item bounds before canonicalization", () => {
    const multibyteInstructions = `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({
      version: 1,
      instructions: "界".repeat(3000),
    })}`;
    const tooMany = `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({
      version: 1,
      preserve: Array.from({ length: MAX_PRESERVE_ITEMS + 1 }, () => "x"),
    })}`;
    const hugeItem = `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({
      version: 1,
  ...
  it("rejects aggregate encoded payloads and never renders embedded fake facts", () => {
    const source = `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify({
      version: 1,
      instructions: "fake goal",
      preserve: Array.from(
        { length: MAX_PRESERVE_ITEMS },
        (_, index) => `fake/path-${index}.ts ${"x".repeat(1100)}`,
      ),
    })}`;
    const decoded = decodeCompactionInstructions(source);
  ...
  it("cancels and emits a bounded notification for a reserved typed decoding error", () => {
    ...
    const event = compactionEvent(
      buildSession(user("real goal"), assistant(textPart("done"))),
      `${RAFT_COMPACTION_REQUEST_PREFIX}{"version":1,"goal":"fake/path.ts"}`,
    );
    event._piVccOverriding = true;
...
describe("Pi custom_message compaction", () => {
  it("normalizes hidden and visible context structurally while excluding plain custom entries", () => {
    ...
    const hidden = customMessage("before-agent-start", "hidden injected context", false, {
      source: "hook",
      priority: 2,
    });
    ...
    expect(events).toMatchObject([
      { customType: "pi-raft-actor", text: "actor completed <typed>", display: true },
  ...
  it("includes actor and agent completions in cumulative summaries deterministically", () => {
    ...
    const session = buildSession(
      user("Original task"),
      customMessage("pi-raft-actor", "Actor says: keep ACTOR_FACT_17", true, {
        actor: { id: "actor-17" },
        message: { status: "completed" },
      }),
      assistant(textPart("actor received")),
      customMessage("pi-raft-agent-complete", "Raft agent abc completed: AGENT_FACT_23", false, {
        id: "agent-23",
        status: "completed",
      }),
    ...
    const first = compileRaftSummary(session, 1_000);
    const second = compileRaftSummary(session, 1_000);
    ...
    expect(first.compaction.summary).toContain('custom "pi-raft-actor" (visible)');
    expect(first.compaction.summary).toContain("ACTOR_FACT_17");
    expect(first.compaction.summary).toContain('custom "pi-raft-agent-complete" (hidden)');
  ...
  it("keeps a custom-message turn boundary and the crossing tool pair together", () => {
    ...
    if (cut.ok)
      expect(cut.summarized.map((entry) => entry.id)).toEqual(session.map((entry) => entry.id));
  });
...
describe("compaction golden determinism", () => {
  it("produces byte-identical output across repeated compiles of the same fixture", () => {
    ...
    const session = buildSession(
      ...
      assistant(
        textPart("I will edit b and create c"),
        toolCallPart(c2, "edit", { path: "src/b.ts" }),
      ),
    ...
    const first = compileRaftSummary(session, 1000) as { compaction: { summary: string } };
    const second = compileRaftSummary(session, 1000) as { compaction: { summary: string } };
    expect(second.compaction.summary).toBe(first.compaction.summary);
  ...
  it("emits only non-empty sections in fixed order", () => {
    ...
    resetClock();
    const session = buildSession(
      user("single turn, no tools"),
      assistant(textPart("acknowledged")),
    );
    ...
    const { compaction } = compileRaftSummary(session, 1000) as { compaction: { summary: string } };
...
describe("compaction cumulative stability", () => {
  it("recomputes cumulative truth from raw branch entries while advancing the live cut", () => {
    ...
    const first = compileRaftSummary(session1, 1000) as {
    ...
    const second = compileRaftSummary(session2, 2000) as {
...
describe("compaction cumulative endurance", () => {
  it("retains raw cumulative truth through 100 parent-linked compaction cycles", () => {
    ...
    appendLinked(
      ...
      customMessage("pi-raft-actor", "Preserve CUSTOM_CYCLE_FACT_31", false, {
        status: "completed",
      }),
    ...
    for (let cycle = 0; cycle < 100; cycle++) {
      const first = compileRaftSummary(branch, 10_000);
      const second = compileRaftSummary(branch, 10_000);
      if (!("compaction" in first) || !("compaction" in second))
        throw new Error("expected compaction");
      expect(second.compaction.summary).toBe(first.compaction.summary);
      ...
      expect(first.compaction.details).toMatchObject({ compactor: "raft", version: 2 });
      expect(
        first.compaction.firstKeptEntryId === "" ||
          branch.some((entry) => entry.id === first.compaction.firstKeptEntryId),
      ).toBe(true);
...
describe("compaction raw branch truth", () => {
  it("strictly recognizes only structurally valid Raft v1/v2 details", () => {
    expect(raftCompactionVersion({ compactor: "raft", version: 1 })).toBeUndefined();
    expect(raftCompactionVersion({ compactor: "raft", version: 3 })).toBeUndefined();
    expect(raftCompactionVersion({ compactor: "other", version: 2 })).toBeUndefined();
  });
  ...
  it("migrates v1 details and excludes prior rendered-summary poison", () => {
    ...
    appendLinked(
      branch,
      compactionEntry(kept.id, "PRIOR_SUMMARY_POISON_991", {
        compactor: "raft",
        version: 1,
    ...
    const result = compileRaftSummary(branch, 2000);
    if (!("compaction" in result)) throw new Error("expected compaction");
    expect(result.compaction.details?.version).toBe(2);
    expect(result.compaction.details?.counts.priorRaftV1).toBe(1);
  ...
  it("uses only the supplied active branch path", () => {
    ...
    const activeResult = compileRaftSummary(active, 1000);
    const inactiveResult = compileRaftSummary(inactive, 1000);
    if (!("compaction" in activeResult) || !("compaction" in inactiveResult))
      throw new Error("expected compaction");
...
describe("compaction cut never orphans a tool_result from its tool_call", () => {
  ...
  it("recovers a safe cut after an orphan prior kept boundary", () => {
    ...
    const result = compileRaftSummary(branch, 1000);
    if (!("compaction" in result)) throw new Error("expected compaction");
...
describe("continuity-tail compaction budget", () => {
  ...
  it("retains a bounded continuity tail without crossing tool pairs", () => {
    ...
    const result = compileRaftSummary(entries, tokensBefore, [], undefined, {
      contextWindow: 100_000,
      targetContextRatio: 0.65,
      reserveTokens: 10_000,
  ...
  it("selects the largest legal raw suffix within the continuity-tail budget", () => {
    ...
    appendLinked(
      ...
      ...Array.from({ length: 20 }, (_, index) =>
        assistant(textPart(`stage ${index} ${"x".repeat(4_000)}`)),
      ),
    );
    const tokensBefore = buildSessionContext(entries).messages.reduce(
      (total, message) => total + estimateTokens(message),
    ...
    const suffixTokens = entries.map((_, start) =>
      entries
        .slice(start)
        .reduce(
          (total, entry) =>
            total +
            sessionEntryToContextMessages(entry).reduce(
    ...
    const legalSuffixes = suffixTokens
      .slice(1)
      .filter((tokens) => tokens <= cut.budget!.rawTailTokenBudget!);
  ...
  it("clamps an overflow recovery cut to the observed failing request size", () => {
    ...
    const overflowResult = handler?.(event, context) as
      | { compaction?: { details?: { budget?: RaftCompactionBudgetDetails } } }
      | undefined;
    ...
    const thresholdResult = handler?.(
      { ...event, reason: "threshold" } as SessionBeforeCompactEvent,
      context,
    ) as { compaction?: { details?: { budget?: RaftCompactionBudgetDetails } } } | undefined;
  ...
  it("cancels rather than expanding when even compact-all cannot fit the target", () => {
    ...
    expect(result).toMatchObject({
      cancel: true,
      reason: "raft: no deterministic summary fits the continuity context target",
    });
  });
  ...
  it("keeps observed fixed prompt overhead outside the shrinkable message budget", () => {
    ...
    const result = compileRaftSummary(entries, rawTokens + 40_000, [], undefined, {
      contextWindow: 100_000,
      targetContextRatio: 0.65,
      reserveTokens: 10_000,
      keepRecentTokens: 20_000,
  ...
  it("retains one cumulative summary and safe utilization through 20 continuity cycles", () => {
    ...
    for (let cycle = 0; cycle < 20; cycle++) {
      ...
      const result = compileRaftSummary(branch, tokensBefore, [], undefined, {
        contextWindow: 100_000,
        targetContextRatio: 0.65,
        reserveTokens: 10_000,
        keepRecentTokens: 20_000,
  ...
  it("uses live model metadata and Pi settings through the registered hook", () => {
    ...
    registerCompactionHook(pi, { getEngine: () => "raft", getTargetContextRatio: () => 0.65 });
    const { entries, tokensBefore } = longSingleTurn();
    const event = {
    ...
    const context = { model: { contextWindow: 100_000 } } as unknown as ExtensionContext;
...
describe("compaction empty and tiny history edge cases", () => {
  it("cancels on empty history", () => {
    ...
    const result = compileRaftSummary([], 1000);
  ...
  it("still renders a stable summary for a tiny session", () => {
    ...
    const a = compileRaftSummary(session, 1000) as { compaction: { summary: string } };
    const b = compileRaftSummary(session, 1000) as { compaction: { summary: string } };
...
describe("compaction benchmark", () => {
  it("compacts a synthetic 2000-entry session in under 200ms", () => {
    ...
    while (entries.length < 2000)
      entries.push(assistant(textPart(`progress note ${entries.length}`)));
    entries.push(user("Final review"));
    expect(entries.length).toBeGreaterThanOrEqual(2000);

    const start = performance.now();
    const result = compileRaftSummary(entries, 1000);
...
describe("compaction section composition", () => {
  it.each([
    ...
    const result = compileRaftSummary(session, 1000);
    if (!("compaction" in result)) throw new Error("expected compaction");
