...
const makeTempDir = (prefix: string): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `pi-raft-memory-${prefix}-`));
  tempDirs.push(dir);
  return dir;
};
...
const buildSession = (
  cwd: string,
  id: string,
): { entries: FixtureEntry[]; push: (e: FixtureEntry) => void } => {
...
const makeMemoryConfig = (indexDir: string): RaftMemoryConfig => ({
  enabled: true,
  indexDir,
  maxSessions: 500,
  maxEntryChars: 2_000,
...
const invocationContext = (cwd: string): RaftInvocationContext => ({
  cwd,
  signal: undefined,
...
describe("memory normalize", () => {
  ...
  it("truncates long text but expand returns the full text", () => {
    ...
    const file = writeSessionFile(
      path.join(agentDir, "sessions", encodeCwdDir(cwd)),
      "1_long.jsonl",
      [sessionHeader("long", cwd), msg("e1", null, ts(0), userMessage(longText))],
    );
...
describe("memory discovery", () => {
  ...
  const seed = (cwd: string, name: string, id: string): string => {
    ...
    const file = writeSessionFile(path.join(agentDir, "sessions", encodeCwdDir(cwd)), name, [
      sessionHeader(id, cwd),
      msg("e1", null, ts(0), userMessage(`session ${id}`)),
    ]);
    return file;
  };
  ...
  it("project scope lists all sessions for the cwd dir", () => {
    ...
    const refs = resolveScope({ agentDir, cwd, scope: "project", maxSessions: 500 });
...
describe("memory shard index", () => {
  ...
  it("BM25 ranking is deterministic: same query twice yields identical ordering", () => {
    ...
    const run = () => bm25Score(shards, ["auth", "module"], {}).map((s) => s.entry.index);
    const first = run();
    const second = run();
    expect(second).toEqual(first);
...
describe("MemoryProvider", () => {
  ...
  it("describes bounded recall hits and context expansion as public contracts", async () => {
    ...
    expect(recallInput.properties).toEqual(
    ...
    expect(expandInput.properties).toEqual(
      expect.objectContaining({
    ...
    expect(recallOutput.properties).toEqual(
      expect.objectContaining({ hits: expect.any(Object), next: expect.any(Object) }),
    );
    expect(expandOutput.properties).toEqual(
      expect.objectContaining({ entries: expect.any(Object), next: expect.any(Object) }),
    );
  });
  ...
  it("expand returns full untruncated text for indices", async () => {
    ...
    const result = (await provider().invoke(
      "expand",
      { session: file, indices: [0] },
      invocationContext(cwd),
    )) as { entries: { index: number; text: string | null }[] };
    ...
    const file = writeSessionFile(
      ...
      [
        ...
        msg(
          ...
          "root",
          ts(1),
          assistantText("Use the browser already present in Heddlework."),
        ),
        msg(
          "decision",
          "distractor",
  ...
  it("ranks human decisions ahead of recall-query plumbing", async () => {
    const file = writeSessionFile(
      ...
      [
        ...
        msg(
          ...
          "root",
          ts(1),
          assistantToolCall("recall-call", "raft_exec", {
        ...
        msg(
          ...
          "echo",
          ts(2),
          toolResult("recall-call", "raft_exec", "Use CEF Stage 1 browser Heddlework"),
        ),
        msg(
          "decision",
          "echo-result",
    ...
    const result = (await provider().invoke(
      "recall",
      { scope: `session:${file}`, query: '"Use CEF" "Stage 1" browser Heddlework', pageSize: 10 },
      invocationContext(cwd),
  ...
  it("bounds broad recall by exact hits and provides a lossless continuation", async () => {
    ...
    expect(first.next!.args).toEqual(
      expect.objectContaining({
        scope: `session:${file}`,
  ...
  it("invalidates cached expansion pages when the session changes", async () => {
    ...
    const stale = (await memory.invoke("expand", first.next!.args, invocationContext(cwd))) as {
      error: { code: string };
      entries: unknown[];
    };
    expect(stale.error.code).toBe("stale_pointer");
  ...
  it("hard-bounds expansion metadata while preserving lossless text continuations", async () => {
    ...
    let result = (await provider().invoke(
      ...
      invocationContext(cwd),
    )) as { entries: Array<{ text: string }>; next: { args: Record<string, unknown> } | null };
    let reconstructed = "";
    let calls = 0;
    while (true) {
      calls += 1;
  ...
  it("lists and describes its actions", async () => {
    ...
    expect(list.map((d) => d.name).sort()).toEqual(["expand", "recall"]);
