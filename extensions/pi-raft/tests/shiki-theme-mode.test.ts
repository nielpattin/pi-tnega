...
describe("classifyPiTheme", () => {
  ...
  it("classifies custom themes from message background luminance", () => {
    expect(classifyPiTheme({ name: "mine", getBgAnsi: () => "\x1b[48;2;240;240;235m" })).toBe(
      "light",
    );
    expect(classifyPiTheme({ name: "mine", getBgAnsi: () => "\x1b[48;2;30;30;34m" })).toBe("dark");
...
describe("code preview settings", () => {
  ...
  it("normalizes raft.json codePreview sections", () => {
  ...
  it("exposes codePreview through the unified raft config", () => {
    const configured = normalizeRaftConfig({
      appearance: { codePreview: { shikiTheme: "nord", syntaxHighlighting: false } },
    });
    expect(configured.appearance.codePreview.shikiTheme).toBe("nord");
    expect(configured.appearance.codePreview.syntaxHighlighting).toBe(false);
    expect(normalizeRaftConfig({}).appearance.codePreview.shikiTheme).toBe("auto");
  });
});
