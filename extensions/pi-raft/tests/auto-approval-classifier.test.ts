...
import { RaftAutoApprovalClassifier } from "../src/core/auto-approval-classifier.js";
        ...
        { type: "message", message: { role: "user", content: "Run the test suite" } },
        {
          ...
          message: {
            ...
            content: [
              ...
              { type: "toolCall", name: "raft_exec", arguments: { code: "pi.bash(...)" } },
            ],
          },
        },
        {
...
describe("RaftAutoApprovalClassifier", () => {
  ...
  it("uses the selected Pi model and returns a structured verdict", async () => {
    ...
    const ctx = context();
    const classifier = new RaftAutoApprovalClassifier();
    ...
    expect(
      (ctx.modelRegistry as unknown as { find: ReturnType<typeof vi.fn> }).find,
    ).toHaveBeenCalledWith("anthropic", "classifier");
    ...
    expect(evidence).toContain("raft_exec");
  ...
  it("dispatches custom APIs through Pi's native provider runtime", async () => {
    ...
    const result = await new RaftAutoApprovalClassifier().classify(
      action,
      { command: "pnpm test" },
      ctx,
    );
  ...
  it("fails closed when structured output is missing", async () => {
    ...
    await expect(
      new RaftAutoApprovalClassifier().classify(action, { command: "rm -rf /" }, context()),
