...
import type { RaftUiAgent } from "../src/ui/types.js";
...
describe("agent transcript projection", () => {
  it("projects streamed assistant text and tool lifecycle events", () => {
    const transcript = projectAgentTranscript([
      {
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: "Reviewing the dashboard" }],
        },
      },
      {
      ...
      },
      {
        ...
        message: {
          role: "assistant",
          content: [{ type: "text", text: "Found the transcript path" }],
        },
      },
    ]);
    ...
    expect(transcript.entries).toEqual([
      expect.objectContaining({
        kind: "assistant",
        text: "Found the transcript path",
        status: "completed",
      }),
      expect.objectContaining({
        kind: "tool",
    ...
    expect(transcript.entries).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          kind: "tool",
          label: "bash",
          text: expect.stringContaining("exit 1"),
          status: "failed",
        }),
  ...
  it("redacts secrets and strips terminal and bidi controls without splitting emoji", () => {
    ...
    const transcript = projectAgentTranscript([
      ...
      },
      {
        ...
        message: {
          role: "assistant",
          content: `\u001b]8;;https://evil.test\u0007safe\u001b]8;;\u0007 ${long}`,
        },
      },
    ]);
  ...
  it("retains nested raft tool structure for rich code-change rendering", () => {
    const transcript = projectAgentTranscript([
      {
        type: "tool_execution_start",
        toolCallId: "outer",
  ...
  it("returns the complete log in one read without paging", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-transcript-"));
    temporaryDirectories.push(directory);
    const logFile = path.join(directory, "events.jsonl");
    const events = Array.from({ length: 120 }, (_, index) => ({
      type: "message_end",
      message: { role: "assistant", content: `full-${index}` },
    }));
    ...
    const transcript = reader.read(agent);
    expect(transcript.entries).toHaveLength(120);
    expect(transcript.entries[0]?.text).toBe("full-0");
    expect(transcript.entries.at(-1)?.text).toBe("full-119");
    expect(transcript.hasMore).toBe(false);
    expect(transcript.hasNewer).toBe(false);
  ...
  it("tails a live JSONL file and refreshes when it grows", () => {
    ...
    expect(reader.read(agent).entries[0]).toMatchObject({
      text: "First update",
      status: "running",
    });
    fs.appendFileSync(
