...
import { ApprovalController, RaftSessionApprovals } from "../src/core/approval-controller.js";
import type { RaftAutoApprovalClassifier } from "../src/core/auto-approval-classifier.js";
import type { ResolvedRaftAction } from "../src/core/action-registry.js";
...
const tuiContext = (
  ...
  ({ hasUI: true, mode: "tui", ui: { custom, notify } }) as unknown as ExtensionContext;
...
describe("ApprovalController", () => {
  ...
  it("shares an Always allow grant across the Pi session", async () => {
    ...
    const notify = vi.fn();
    const session = new RaftSessionApprovals();
    const firstExecution = new ApprovalController(policies, tuiContext(custom, notify), session);
    const laterExecution = new ApprovalController(policies, tuiContext(custom, notify), session);
    ...
    expect(notify).toHaveBeenLastCalledWith("Allowed write access for this Pi session", "info");
  });
  ...
  it("uses an RPC-compatible three-choice dialog", async () => {
    ...
    expect(select).toHaveBeenCalledWith(
      "Pi Raft permission · demo.write requests write access. Write data",
  ...
  it("auto-allows only the exact classified action", async () => {
    ...
    expect(classify).toHaveBeenCalledWith(action, args, expect.anything(), "anthropic/classifier");
  });
  ...
  it("escalates an unsafe auto decision to the approval wizard", async () => {
    ...
    } as unknown as RaftAutoApprovalClassifier;
    const custom = vi.fn(async () => "allow-once");
    const notify = vi.fn();
    const controller = new ApprovalController(
      { ...policies, write: "auto" },
    ...
    expect(custom).toHaveBeenCalledOnce();
    expect(notify).toHaveBeenCalledWith(expect.stringContaining("Auto mode escalated"), "warning");
  });
  ...
  it("fails closed and notifies when the user denies or dismisses", async () => {
    ...
    expect(notify).toHaveBeenLastCalledWith("Denied write access for demo.write", "warning");
  });
  ...
  it("denies actions blocked by policy without prompting", async () => {
    ...
    await expect(controller.approve({ ...action, risk: "execute" })).rejects.toThrow(
      "denied by the Raft execute policy",
    );
