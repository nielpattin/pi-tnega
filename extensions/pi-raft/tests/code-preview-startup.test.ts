...
describe("code preview startup", () => {
  it("keeps pi-code-previews out of the package and runtime graph", () => {
    ...
    const toolSource = fs.readFileSync(
      path.join(process.cwd(), "src", "raft-exec-tool.ts"),
      "utf8",
    );
  ...
  it("renders call and result inside a local border shell", () => {
    ...
    const decorated = withCodePreviewShell(tool, { mode: "border", toolCallTiming: false });
    const state = {};
    const theme = {
    ...
    decorated.renderResult({ content: [] }, { expanded: false, isPartial: true }, theme, context);
  ...
  it("preserves shell mode and timing preferences", () => {
    ...
    const decorated = withCodePreviewShell(tool, { mode: "off", toolCallTiming: false });
    expect(decorated.renderShell).toBe("self");
    const context = { state: {}, executionStarted: true, isPartial: false } as any;
    const theme = {
      fg: (_color: string, text: string) => text,
      bold: (text: string) => text,
    } as any;
