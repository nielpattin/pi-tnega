...
import {
  parseSkillBlock as localParseSkillBlock,
  formatSkillsForPrompt as localFormatSkillsForPrompt,
} from "../src/core/skill-block.js";
import {
  calculateContextTokens as localCalculateContextTokens,
  DEFAULT_COMPACTION_SETTINGS as localCompactionSettings,
  estimateTokens as localEstimateTokens,
} from "../src/core/token-math.js";
import {
  buildSessionContext as localBuildSessionContext,
  sessionEntryToContextMessages as localSessionEntry,
...
describe("host parity", () => {
  it("formats skill blocks identically", () => {
    ...
    const skills = [
      {
        name: "A&B <cool> 'quotes' \"inside\"",
        description: "desc <1> & 'two'",
        filePath: "/tmp/skills/a/SKILL.md",
      },
      {
        name: "disabled",
        description: "hidden",
        filePath: "/tmp/skills/b/SKILL.md",
    ...
    expect(localFormatSkillsForPrompt(skills.slice(1))).toBe(
      formatSkillsForPrompt(skills.slice(1)),
    );
  });
...
body
  ...
  it("estimates and context-counts tokens identically", () => {
    ...
    // Host accesses .totalTokens directly; Raft's port also tolerates
    ...
    for (const usage of usages) {
      expect(localCalculateContextTokens(usage)).toBe(calculateContextTokens(usage as never));
    }
