...
describe("normalizeRunDisplay", () => {
  ...
  it("repairs a bare objective string to { name }", () => {
    expect(normalizeRunDisplay("Collect package.json stats")).toEqual({
      name: "Collect package.json stats",
    });
  });

  it("parses a JSON-stringified object, a common escaped-JSON near-miss", () => {
    expect(normalizeRunDisplay('{"name": "Probe", "description": "d"}')).toEqual({
      name: "Probe",
      description: "d",
    });
  });
