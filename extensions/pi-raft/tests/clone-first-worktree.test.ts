...
import { RAFT_WORKTREE_EXCLUDE, raftWorktreePath } from "../src/agents/worktree-paths.js";
...
const initRepository = (): string => {
  const repository = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-cow-wt-"));
...
describe("clone-first worktrees", () => {
  it("creates a managed worktree under .pi/raft/worktrees and keeps ignored artifacts", async () => {
    const repository = initRepository();
    const dest = raftWorktreePath(repository, "agentid0123456789abcdef012345");
    const result = await addCloneFirstWorktree({
      gitRoot: repository,
      dest,
      branch: { flag: "-b", name: "pi-raft/clone-test-agentid01" },
      startPoint: "HEAD",
      quiet: true,
    });
    ...
    expect(fs.realpathSync(dest)).toBe(
      fs.realpathSync(
        path.join(repository, ".pi", "raft", "worktrees", "agentid0123456789abcdef012345"),
      ),
    );
    ...
    expect(fs.readFileSync(path.resolve(repository, exclude), "utf8")).toContain(
      RAFT_WORKTREE_EXCLUDE,
    );
    if (result.cloned) {
      expect(fs.readFileSync(path.join(dest, "node_modules", "pkg", "index.js"), "utf8")).toBe(
        "artifact\n",
      );
    }
  });
  ...
  it("places WorktreeManager leases on the managed path", async () => {
    ...
    expect(fs.realpathSync(lease.path)).toBe(
      fs.realpathSync(raftWorktreePath(repository, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")),
    );
