...
describe("boundModelOutput", () => {
  ...
  it("bounds visible text and links the complete artifact", async () => {
    ...
    const writer = vi.fn(async () => "/tmp/pi-raft-output/output.txt");
    ...
    expect(result.artifactPath).toBe("/tmp/pi-raft-output/output.txt");
  ...
  it("stays bounded if artifact persistence fails", async () => {
    ...
    expect(result.text).toContain("characters omitted by Pi Raft");
  });
});
