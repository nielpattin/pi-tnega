...
const CODEX_BLOB = '{"id":"rs_074b3fd","type":"reasoning","encrypted_content":"gAAAABlob"}';

const codexSource = { provider: "openai-codex", modelId: "gpt-5.6-sol", api: "openai-responses" };
...
describe("thinking transfer translation", () => {
  ...
  it("re-signs textual thinking and drops opaque blocks for reasoning_content targets", () => {
    ...
    expect(original.content.find((part) => part.type === "toolCall")).toHaveProperty(
      "thoughtSignature",
    );
  });
  ...
  it("strips every thinking part and foreign thought signature", () => {
    ...
    expect(message.content.find((part) => part.type === "toolCall")).not.toHaveProperty(
      "thoughtSignature",
    );
