...
const legacyProvider = (): RaftProvider => ({
  name: "legacy",
...
describe("ordinary execution compatibility", () => {
  it("keeps uncommitted calls on the established resolve, approve, and result path", async () => {
    ...
    expect(audits).toMatchObject([{ ref: "legacy.write", success: true, result: "write:arc" }]);
  ...
  it("pins the model-visible default effects and capability hash format", async () => {
    ...
    expect(
      catalog.providers[0]?.actions.map((action) => [action.ref, action.descriptorHash]),
    ).toEqual([
