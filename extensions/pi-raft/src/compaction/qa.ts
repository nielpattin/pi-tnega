...
const goalAnswer = (text: string): string => {
  const lines = text
    .split("\n")
    .filter((line, index, all) => line.trim() !== "" || (index === 0 && all.length === 1))
...
const unresolvedErrors = (
  ...
  | Extract<CompactionEvent, { kind: "toolResult" }>
  | Extract<CompactionEvent, { kind: "bash" }>
)[] => {
  ...
  for (const event of events) {
    if (event.kind === "toolResult" && event.isError) {
      ...
      const resolved =
        path !== undefined &&
        successfulPaths.some((success) => success.path === path && success.index > event.index);
...
export const generateProbes = (events: CompactionEvent[], cutIndex: number): Probe[] => {
  ...
  const modifyingByKind = new Map<
    string,
    { entryId: string; event: ToolCallEvent; path: string }[]
  >();
  ...
  const lastRun = [...source]
    .reverse()
    .find(
      (event): event is Extract<CompactionEvent, { kind: "raftRun" }> => event.kind === "raftRun",
    );
  if (lastRun) {
    probes.push({
      id: `last-raft-run:${lastRun.index}`,
      class: "content",
      question: "What was the latest declared Raft execution intent and paired outcome?",
      answer: `${clipUtf8(truncate(lastRun.name, MAX_RAFT_RUN_STATUS_NAME), 256)} → ${lastRun.outcome}`,
    });
    probes.push({
      id: `last-raft-run-address:${lastRun.index}`,
      class: "address",
      question: "Which stable address expands the latest declared Raft execution intent?",
