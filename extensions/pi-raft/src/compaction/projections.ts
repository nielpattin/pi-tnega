...
const MAX_RAFT_RUN_NAME = 80;
const MAX_RAFT_RUN_DESCRIPTION = 180;
const MAX_RAFT_RUN_TRANSCRIPT_NAME = 60;
const MAX_RAFT_RUN_EARLIER_NAME = 48;
const MAX_RAFT_RUN_STATUS_NAME = 96;
...
const isBashOperation = (operation: StructuralOperation): boolean =>
  operation.tool === "bash" && (operation.ref === "bash" || operation.ref === "pi.bash");
...
const stripRoot = (root: string, path: string): string => (root ? path.replace(root, "") : path);
...
const projectGoal = (events: CompactionEvent[]): ProjectedSection => {
  ...
  const firstLines = first.text
    .split("\n")
    .filter((line, i, arr) => line.trim() !== "" || (i === 0 && arr.length === 1))
...
const raftRunPointer = (event: RaftRunEvent): string =>
  event.source === "branch" ? event.address : event.entryId;

const projectActivity = (events: CompactionEvent[]): ProjectedSection => {
  const items: ActivityItem[] = [];
  for (const event of events) {
    if (event.kind === "raftRun") {
      const name = clipUtf8(truncate(event.name, MAX_RAFT_RUN_NAME), 192);
      const description = event.description
        ? clipUtf8(truncate(event.description, MAX_RAFT_RUN_DESCRIPTION), 512)
        : "";
      items.push({
        entryId: raftRunPointer(event),
        line: `- ${name}${description ? ` — ${description}` : ""} → ${event.outcome}`,
      });
    } else if (event.kind === "raftPhase") {
      items.push({ entryId: event.address, line: `- Phase: ${truncate(event.phase, MAX_LINE)}` });
    } else if (event.kind === "raftOperation") {
      if (
        FILE_TOOLS.has(event.tool) &&
        (event.ref === event.tool || event.ref === `pi.${event.tool}`)
      )
        continue;
      ...
      const detail =
        typeof primary === "string" && primary.trim()
          ...
          : "";
      items.push({ entryId: event.address, line: `- ${event.ref}${detail} → ${event.outcome}` });
    }
  }
...
const projectOutstandingWithMetadata = (events: CompactionEvent[]): ProjectedSection => {
  ...
  const keyOf = (operation: StructuralOperation): string => {
    ...
    const command =
      isBashOperation(operation) && typeof operation.args.command === "string"
  ...
  for (const operation of operations) {
    ...
    const error = operation.error
      ? `: ${truncate(firstLine(operation.error), MAX_LINE)}`
      : `: ${operation.outcome}`;
    const key = keyOf(operation);
    const resolved = successes.some(
      (success) => success.index > operation.index && success.key === key,
    );
    items.push({
  ...
  const append = (sampled: ReturnType<typeof sampleAddressed<ErrorItem>>, noun: string): void => {
    for (let index = 0; index < sampled.values.length; index++) {
      ...
      const item = sampled.values[index]!;
      lines.push(
        `- ${item.description}${item.resolved ? " [RESOLVED]" : ""} [entry ${item.entryId}]`,
      );
    }
  };
  ...
  function* earlierTurns(): Generator<EarlierTurnAddress> {
    ...
    const completed = (): EarlierTurnAddress | undefined => {
      ...
      return {
        ...
        run: lastRun
          ? `raft:${quoted(clipUtf8(lastRun.name, 128), MAX_RAFT_RUN_EARLIER_NAME)}→${lastRun.outcome}`
          : "",
      };
    };
    for (const event of events) {
      ...
      if (event.kind === "raftRun") lastRun = event;
      const name =
        event.kind === "toolCall"
          ? event.name === "raft_exec"
            ? undefined
            : event.name
          : event.kind === "bash"
            ? "bash"
            : event.kind === "raftOperation"
              ? event.tool
              : undefined;
      if (!name) continue;
...
const projectStatus = (events: CompactionEvent[]): string[] => {
  ...
  const lastRun = [...events]
    .reverse()
    .find((event): event is RaftRunEvent => event.kind === "raftRun");
  if (lastRun) {
    lines.push(
      `Last execution: ${clipUtf8(truncate(lastRun.name, MAX_RAFT_RUN_STATUS_NAME), 256)} → ${lastRun.outcome} [entry ${raftRunPointer(lastRun)}]`,
    );
  }
  const lastAssistant = [...events]
    .reverse()
    .find(
      (e): e is Extract<CompactionEvent, { kind: "assistantText" }> => e.kind === "assistantText",
    );
  if (lastAssistant) {
...
const summarizeArgs = (name: string, args: Record<string, unknown>): string => {
  if (name === "raft_exec") return "structured execution";
  const primary =
    name === "bash"
      ? (args.command ?? args.cmd ?? args.shell)
      : name === "grep"
        ? (args.pattern ?? args.query ?? args.regex)
        : name === "find"
          ? args.pattern
          : pathOf(args);
...
const projectTranscript = (events: CompactionEvent[]): ProjectedSection => {
  const completedRaftCalls = new Set(
    events
      .filter((event): event is RaftRunEvent => event.kind === "raftRun")
      .map((event) => event.toolCallId),
  );
  const transcriptEvents = events.filter((event) => {
    if (event.kind === "toolCall") {
      return event.name !== "raft_exec" || !completedRaftCalls.has(event.toolCallId);
    }
    if (event.kind === "toolResult") {
      return event.toolName !== "raft_exec" || !completedRaftCalls.has(event.toolCallId);
    }
    return true;
  });
  ...
  for (const e of window) {
    ...
    } else if (e.kind === "raftRun") {
      lines.push(
        `${ref} raft_exec ${quoted(clipUtf8(e.name, 160), MAX_RAFT_RUN_TRANSCRIPT_NAME)} → ${e.outcome} [entry ${raftRunPointer(e)}]`,
      );
    } else if (e.kind === "raftPhase") {
      lines.push(`${ref} phase(${truncate(e.phase, MAX_TRANSCRIPT_CMD)}) [${e.address}]`);
    } else if (e.kind === "raftOperation") {
