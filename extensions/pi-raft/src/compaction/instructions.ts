...
export const RAFT_COMPACTION_REQUEST_PREFIX = "__pi_raft_compact_request_v1__:";
const MAX_COMPACTION_INSTRUCTIONS_CHARS = 8 * 1024;
...
export type DecodedCompactionInstructions =
  | { ok: true; requestLines: string[]; policy: CompactionInstructionPolicy }
  | { ok: false; requestLines: []; error: CompactionInstructionDecodeError };
...
export const compactionRequestBoundsError = (
  ...
  if (request.instructions !== undefined) {
    ...
    if (
      request.instructions.length > MAX_COMPACTION_INSTRUCTIONS_CHARS ||
...
export const encodeCompactionRequest = (
  request: Omit<TypedCompactionRequest, "version">,
): string => {
  ...
  const encoded = `${RAFT_COMPACTION_REQUEST_PREFIX}${JSON.stringify(payload)}`;
  ...
  private parseValue(depth: number): unknown {
    if (depth > MAX_TYPED_JSON_DEPTH) {
      throw new StrictJsonError(
        "structure-too-complex",
        "typed compaction JSON exceeds the structural depth limit",
      );
    }
    this.nodes += 1;
    if (this.nodes > MAX_TYPED_JSON_NODES) {
      throw new StrictJsonError(
        "structure-too-complex",
        "typed compaction JSON exceeds the structural node limit",
      );
    }
    ...
    while (this.index < this.source.length) {
      ...
      if (unit === 0x22) {
        ...
        if (!hasPairedSurrogates(output)) {
          throw new StrictJsonError(
            "invalid-unicode",
            "typed compaction JSON string contains an unpaired UTF-16 surrogate",
          );
        }
        return output;
      }
...
export const decodeCompactionInstructions = (
  ...
  if (!source.startsWith(RAFT_COMPACTION_REQUEST_PREFIX)) return plainInstructions(source);
  ...
  let parsed: unknown;
  try {
    parsed = new StrictJsonParser(source.slice(RAFT_COMPACTION_REQUEST_PREFIX.length)).parse();
  } catch (error) {
    ...
    return rejection(
      "malformed-json",
      "typed compaction request contains malformed JSON",
      encodedSourceBytes,
    );
  }
  if (!isRecord(parsed)) {
    return rejection(
      "invalid-object",
      "typed compaction request must be a JSON object",
      encodedSourceBytes,
    );
  }
  ...
  const unknownField = keys.find(
    (key) => key !== "version" && key !== "instructions" && key !== "preserve",
  );
  if (unknownField !== undefined) {
    return rejection(
      "unknown-field",
      `typed compaction request contains unknown field ${JSON.stringify(unknownField)}`,
      encodedSourceBytes,
    );
  }
  if (parsed.version !== 1) {
    return rejection(
      "unsupported-version",
      "typed compaction request version must be 1",
      encodedSourceBytes,
    );
  }
  if (parsed.instructions !== undefined && typeof parsed.instructions !== "string") {
    return rejection(
      "invalid-type",
      "typed compaction request instructions must be a string",
      encodedSourceBytes,
    );
  }
  if (parsed.preserve !== undefined && !Array.isArray(parsed.preserve)) {
    return rejection(
      "invalid-type",
      "typed compaction request preserve must be an array",
      encodedSourceBytes,
    );
  }
  ...
  if (preserve !== undefined && !preserve.every((item) => typeof item === "string")) {
    return rejection(
      "invalid-type",
      "typed compaction preserve items must be strings",
      encodedSourceBytes,
    );
  }
