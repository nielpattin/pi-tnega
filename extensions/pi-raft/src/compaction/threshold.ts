...
const configuredCompactionTokenThreshold = (
  ...
  modelKey === undefined ? undefined : config.lifecycle.compaction.tokenThresholds[modelKey];

const configuredCompactionThreshold = (
  config: RaftConfig,
  modelKey: string | undefined,
): number | undefined =>
  modelKey === undefined ? undefined : config.lifecycle.compaction.thresholds[modelKey];

const runThresholdCompact = (context: ExtensionContext): Promise<boolean> =>
  new Promise<boolean>((resolve) => {
    context.compact({
      onComplete: () => resolve(true),
      onError: (error) => {
        if (context.hasUI) {
          context.ui.notify(`Raft threshold compaction failed: ${error.message}`, "warning");
        }
        resolve(false);
      },
    });
