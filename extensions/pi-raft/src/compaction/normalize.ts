...
import { readRaftProjectionTrace, type RaftProjectionSource } from "./trace-events.js";
import {
  RAFT_BRANCH_RUN_DESCRIPTION_MAX_BYTES,
  RAFT_BRANCH_RUN_NAME_MAX_BYTES,
...
interface RaftPhaseEvent extends EventBase {
  kind: "raftPhase";
  subordinal: string;
  address: string;
  phase: string;
...
export interface RaftRunEvent extends EventBase {
  kind: "raftRun";
  toolCallId: string;
  subordinal: string;
  address: string;
  ...
  source: RaftProjectionSource | "result" | "branch";
}

interface RaftOperationEvent extends EventBase {
  kind: "raftOperation";
  subordinal: string;
  address: string;
  ref: string;
  ...
  source: RaftProjectionSource | "branch";
}
...
const boundedJsonValue = (
  ...
  if (typeof value !== "object" || depth > MAX_CUSTOM_DETAILS_DEPTH || state.ancestors.has(value))
    return undefined;
  ...
  try {
    ...
    const output = Object.create(null) as Record<string, RaftTraceJsonValue>;
...
const customDetails = (value: unknown): RaftTraceJsonValue | undefined => {
  if (value === undefined) return undefined;
  try {
    const sanitized = boundedJsonValue(value, { nodes: 0, ancestors: new Set<object>() });
    if (sanitized === undefined || utf8Bytes(JSON.stringify(sanitized)) > MAX_CUSTOM_DETAILS_BYTES)
      return undefined;
    return sanitized;
  } catch {
    return undefined;
...
interface RaftRunIntent {
  name: string;
  description?: string;
}

const raftRunIntent = (call: PendingCall | undefined): RaftRunIntent | undefined => {
  if (!call || call.name !== "raft_exec") return undefined;
  ...
  const intentName = display?.name?.trim()
    ...
    : typeof call.args.code === "string"
      ? raftExecTitleHintCached(call.args.code)
      : undefined;
  if (intentName === undefined) return undefined;
  const name = clipUtf8(intentName.trim(), RAFT_BRANCH_RUN_NAME_MAX_BYTES);
  if (!name) return undefined;
  const description =
    display?.description !== undefined
      ? clipUtf8(display.description.trim(), RAFT_BRANCH_RUN_DESCRIPTION_MAX_BYTES)
      : "";
...
export const normalizeEntries = (entries: SessionEntry[]): CompactionEvent[] => {
  ...
  const pushBranchFacts = (entry: SessionEntry & { details?: unknown }): void => {
    const details = readRaftBranchSummaryDetails(entry.details);
    if (!details) return;
  ...
  for (const entry of entries) {
    ...
    if (role === "user") {
      push({
        ...
        text: textOfContent((message as { content: unknown }).content),
      });
      continue;
    }
    ...
    if (role === "toolResult") {
      ...
      if (toolName === "bash") {
        const command =
          pending && typeof pending.args.command === "string" ? pending.args.command : "";
        push({
          kind: "bash",
          entryId,
          sourceEntryId: entryId,
      ...
      if (toolName === "raft_exec") {
        const nested = readRaftProjectionTrace(toolResult.details);
        const intent = raftRunIntent(pending);
        if (intent && pending) {
    ...
    if (role === "bashExecution") {
      const bash = message as { command?: string; exitCode?: number | undefined; output?: string };
      ...
      push({
        ...
        isError,
        exitCode,
        ...(isError && typeof bash.output === "string" && bash.output
          ? { error: bash.output }
          : {}),
      });
      continue;
...
export const countErasedThinkingBlocks = (entries: SessionEntry[]): number => {
  ...
  for (const entry of entries) {
    ...
    for (const part of message.content) {
      if (!part || typeof part !== "object" || !("type" in part)) continue;
      if (part.type === "thinking" && typeof (part as { thinking?: unknown }).thinking === "string")
        count += 1;
    }
  }
  return count;
