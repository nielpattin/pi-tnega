...
import { compileRaftBranchSummary } from "./branch-summary.js";
import {
...
import {
  countErasedThinkingBlocks,
  isPiCustomMessageEntry,
  normalizeEntries,
} from "./normalize.js";
import { projectWithMetadata, type ProjectionOmittedCounts, type Sections } from "./projections.js";
import { renderSummary } from "./render.js";

type CompactionEngine = "pi" | "raft";
...
export interface RaftCompactionBudget {
...
type RaftCompactionBindingConstraint = "continuity" | "occupancy" | "reserve" | "reduction";

export interface RaftCompactionBudgetDetails {
  ...
  bindingConstraint?: RaftCompactionBindingConstraint;
}
...
const findLastCompaction = (
  entries: SessionEntry[],
): { index: number; firstKeptEntryId: string } | undefined => {
...
const callResultSpans = (entries: SessionEntry[]): Map<string, CallResultSpan> => {
  ...
  const record = (id: string, index: number, kind: "call" | "result"): void => {
    if (!id) return;
    const span = spans.get(id) ?? { first: index, last: index, hasCall: false, hasResult: false };
...
const spanCrosses = (span: CallResultSpan, boundaryIndex: number): boolean =>
  span.hasCall && span.hasResult && span.first < boundaryIndex && span.last >= boundaryIndex;
...
const tokenCalibration = (
  ...
  const checkpoints = usageCheckpoints(branchEntries).filter(
    (checkpoint) =>
      checkpoint.rawTokens < rawTokensBefore && checkpoint.contextTokens <= tokensBefore,
  );
  const slopes = checkpoints
    .flatMap((checkpoint) => {
  ...
  if (slopes.length === 0) {
    return { tokenScale: 1, fixedOverheadTokens: Math.max(0, tokensBefore - rawTokensBefore) };
  }
...
const continuityCutPlan = (
  ...
  const continuityTargetTokens = Math.max(
    1,
    Math.ceil(fixedOverheadTokens + (keepRecentTokens + SUMMARY_RAW_TOKEN_BUDGET) * tokenScale),
  );
  ...
  const constraints: Array<{ binding: RaftCompactionBindingConstraint; tokens: number }> = [
...
export type CutResult =
  | {
      ...
      budget?: Omit<RaftCompactionBudgetDetails, "projectedTokensAfter">;
    }
...
const computeContinuityCut = (
  ...
  const suffixTokens = Array.from({ length: live.length + 1 }, () => 0);
  for (let index = live.length - 1; index >= 0; index--) {
...
export const computeCut = (
  branchEntries: SessionEntry[],
  options?: { tokensBefore: number; budget: RaftCompactionBudget },
): CutResult => {
...
interface RaftCompactionDetailsV1 {
  compactor: "raft";
  version: 1;
  sections: string[];
...
export interface RaftCompactionDetailsV2 {
  compactor: "raft";
  version: 2;
  sections: string[];
  coverage: { cumulativeSourceRange: EntryRange; liveCutRange: EntryRange };
  counts: {
  ...
  budget?: RaftCompactionBudgetDetails;
  timestamp: string;
}
...
const isRaftV1Details = (value: Record<string, unknown>): boolean =>
...
const isRaftV2Details = (value: Record<string, unknown>): boolean => {
  if (!isStringArray(value.sections) || !isRecord(value.coverage) || !isRecord(value.counts))
    return false;
  if (
    !isRecord(value.omittedCounts) ||
    !isRecord(value.instructionPolicy) ||
    !isRecord(value.stableAddresses)
  ) {
    return false;
  }
  const instructionModes = new Set(["none", "plain", "typed-v1", "malformed-typed-prefix"]);
  return (
    isEntryRange(value.coverage.cumulativeSourceRange) &&
    ...
    (value.omittedCounts.activity === undefined ||
      (typeof value.omittedCounts.activity === "number" &&
        Number.isFinite(value.omittedCounts.activity))) &&
    (value.omittedCounts.commits === undefined ||
      (typeof value.omittedCounts.commits === "number" &&
        Number.isFinite(value.omittedCounts.commits))) &&
    (value.omittedCounts.thinking === undefined ||
      (typeof value.omittedCounts.thinking === "number" &&
        Number.isFinite(value.omittedCounts.thinking))) &&
...
export const raftCompactionVersion = (details: unknown): 1 | 2 | undefined => {
  if (!isRecord(details) || details.compactor !== "raft") return undefined;
  if (details.version === 1 && isRaftV1Details(details)) return 1;
  if (details.version === 2 && isRaftV2Details(details)) return 2;
  return undefined;
};
...
const cumulativeSource = (
  ...
  return {
    ...
    events,
    range: { first: entries[0]?.id ?? "", last: entries.at(-1)?.id ?? "" },
    timestamp: entries.at(-1)?.timestamp ?? "",
  };
};

const priorRaftVersions = (entries: SessionEntry[]): { v1: number; v2: number } => {
  let v1 = 0;
  let v2 = 0;
  for (const entry of entries) {
    if (entry.type !== "compaction") continue;
    const version = raftCompactionVersion((entry as SessionEntry & { details?: unknown }).details);
    if (version === 1) v1 += 1;
    if (version === 2) v2 += 1;
  }
  return { v1, v2 };
...
export const compileRaftSummary = (
  branchEntries: SessionEntry[],
  tokensBefore: number,
  ...
  | { compaction: CompactionResult<RaftCompactionDetailsV2> }
  | { cancel: true; reason: string; instructionError?: CompactionInstructionDecodeError } => {
  const instructions = decodeCompactionInstructions(customInstructions);
  if (!instructions.ok) {
    return {
      cancel: true,
      reason: `raft: ${instructions.error.code}: ${instructions.error.message}`,
      instructionError: instructions.error,
    };
  }
  const cut = computeCut(branchEntries, budget ? { tokensBefore, budget } : undefined);
  if (!cut.ok) return { cancel: true, reason: "raft: nothing to compact" };

  const source = cumulativeSource(branchEntries, cut.firstKeptEntryId);
  if (source.events.length === 0) return { cancel: true, reason: "raft: no raw cumulative source" };
  ...
  const projectedTokensAfter = cut.budget
    ? Math.ceil(
        cut.budget.fixedOverheadTokens +
          (cut.budget.retainedRawTokens + Math.ceil(summary.length / 4)) * cut.budget.tokenScale,
      )
    : undefined;
  const budgetDetails =
    cut.budget && projectedTokensAfter !== undefined
  ...
  const versions = priorRaftVersions(branchEntries);
  const sectionHeaders = SECTION_HEADERS.filter(({ key }) => sections[key].length > 0).map(
    ({ header }) => header,
  );
  if (instructions.requestLines.length > 0) sectionHeaders.splice(1, 0, "[Compaction Request]");

  const details: RaftCompactionDetailsV2 = {
    compactor: "raft",
    version: 2,
    sections: sectionHeaders,
    coverage: {
      cumulativeSourceRange: source.range,
      liveCutRange: { first: cut.firstSummarizedEntryId, last: cut.lastSummarizedEntryId },
    },
    counts: {
      ...
      priorRaftV1: versions.v1,
      priorRaftV2: versions.v2,
    },
    omittedCounts: {
  ...
  return { compaction: { summary, firstKeptEntryId: cut.firstKeptEntryId, tokensBefore, details } };
};

const SECTION_HEADERS: { key: keyof Sections; header: string }[] = [
  { key: "goal", header: "[Session Goal]" },
  { key: "files", header: "[Files And Changes]" },
  { key: "activity", header: "[Raft Activity]" },
...
const notifyInstructionError = (
  ...
  context.ui.notify(
    clipUtf8(`Raft compaction rejected: ${error.code}: ${error.message}`, 512),
    "error",
  );
};
...
export const registerCompactionHook = (pi: ExtensionAPI, options: CompactionHookOptions): void => {
  pi.on("session_before_compact", (event: SessionBeforeCompactEvent, context: ExtensionContext) => {
    ...
    const thresholdTokens =
      modelKey === undefined ? undefined : options.getThresholdTokens?.(modelKey);
    if (
    ...
    const threshold =
      modelKey === undefined || typeof thresholdTokens === "number"
        ? undefined
    ...
    if (options.getEngine() !== "raft") return;
    ...
    (event as SessionBeforeCompactEvent & { _raftCompaction?: boolean })._raftCompaction = true;
  ...
  pi.on("session_before_tree", (event: SessionBeforeTreeEvent, context: ExtensionContext) => {
    ...
    // Pi's replacement mode delegates an arbitrary summarizer prompt. Raft's
