...
import type { RaftTraceJsonValue } from "../audit/trace.js";
import { clipUtf8, sampleAddressed } from "./bounds.js";
import {
  RAFT_BRANCH_SUMMARY_KIND,
  ...
  type RaftBranchOperationFactV1,
  type RaftBranchSummaryDetailsV2,
...
): Record<string, RaftTraceJsonValue> | undefined => {
  try {
    const cloned = JSON.parse(JSON.stringify(value)) as unknown;
    return cloned && typeof cloned === "object" && !Array.isArray(cloned)
      ? (cloned as Record<string, RaftTraceJsonValue>)
      : undefined;
  } catch {
    return undefined;
  }
...
const directOperationFact = (
  ...
  error?: string,
): RaftBranchOperationFactV1 | undefined => {
  if (call.name === "raft_exec") return undefined;
...
const factsFromEvents = (events: CompactionEvent[]): RaftBranchFactV2[] => {
  const facts: RaftBranchFactV2[] = [];
  ...
  for (const event of events) {
    if (event.kind === "user") {
    ...
    } else if (event.kind === "raftRun") {
      facts.push({
        kind: "raftRun",
        entryId: event.entryId,
        subordinal: event.subordinal,
        address: event.address,
        name: clipUtf8(event.name, RAFT_BRANCH_RUN_NAME_MAX_BYTES),
        ...(event.description !== undefined
          ? { description: clipUtf8(event.description, RAFT_BRANCH_RUN_DESCRIPTION_MAX_BYTES) }
          : {}),
      ...
      if (!call) continue;
      const fact = directOperationFact(
        call,
        event.isError ? "failed" : "succeeded",
        event.isError ? event.text : undefined,
      );
...
): RaftBranchSummaryDetailsV2 => {
  const sampled = sampleAddressed(facts, RAFT_BRANCH_SUMMARY_MAX_FACTS);
  const details: RaftBranchSummaryDetailsV2 = {
    kind: RAFT_BRANCH_SUMMARY_KIND,
    version: RAFT_BRANCH_SUMMARY_VERSION,
    source: {
    ...
    sections,
    request: { text: request.text, sourceBytes: request.sourceBytes, truncated: request.truncated },
  };
  for (
    let index = details.facts.length - 1;
    serializedBytes(details) > RAFT_BRANCH_SUMMARY_MAX_BYTES && index >= 0;
    index--
  ) {
    const fact = details.facts[index]!;
    if (fact.kind === "operation" && fact.result !== undefined) delete fact.result;
  }
  while (serializedBytes(details) > RAFT_BRANCH_SUMMARY_MAX_BYTES && details.facts.length > 0) {
...
export interface RaftBranchSummaryCompilation {
  summary: string;
  details: RaftBranchSummaryDetailsV2;
}

export const compileRaftBranchSummary = (
  entriesToSummarize: SessionEntry[],
  customInstructions?: string,
  enrichers: readonly CompactionEnricher[] = NO_BUILTIN_ENRICHERS,
  oldLeafId: string | null = null,
): RaftBranchSummaryCompilation | undefined => {
  ...
  const sections = SECTION_HEADERS.filter(({ key }) => projected.sections[key].length > 0).map(
    ({ header }) => header,
  );
