...
// pi-raft-guest.js (wrapper header plus TypeScript printer layout), not the
...
const GUEST_FRAME_PATTERN = /pi-raft-guest\.js:(\d+):(\d+)/g;
...
export const remapGuestErrorText = (
  ...
  if (!stackMap || !text.includes("pi-raft-guest.js:")) return text;
  return text.replace(GUEST_FRAME_PATTERN, (match, lineText, columnText) => {
    ...
    if (guestLineCount !== undefined && line > guestLineCount) return "raft driver";
    return match;
  });
};
