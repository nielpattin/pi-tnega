...
const run = async (message) => {
  ...
  const sandbox = {
    __raftHostCall: hostCall,
    __raftTokenBudget: message.tokenBudget ?? Number.POSITIVE_INFINITY,
    print,
    π: jsonCompatible(message.strings),
  };
  const context = vm.createContext(sandbox, {
    name: "pi-raft-node-process",
    codeGeneration: { strings: true, wasm: false },
  });

  try {
    vm.runInContext(message.setup, context, { filename: "pi-raft-setup.js" });
    const promise = vm.runInContext(message.code + "\n__piRaftMain()", context, {
      filename: "pi-raft-guest.js",
    });
...
process.on("message", (message) => {
  ...
  if (message.ok) operation.resolve(message.value);
    ...
    if (message.bashExit) error.__raftBashExit = message.bashExit;
