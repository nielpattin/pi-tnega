...
import type { RaftHostCall, RaftSandboxOptions, RaftSandboxResult } from "./kernel.js";
...
export class NodeProcessRuntime {
  ...
  async execute(
    ...
    const runtimeOptions =
      this.#interpreter === "bun" ? { requireBun: true } : { requireNode: true };
    ...
    return new Promise<RaftSandboxResult>((resolve) => {
      const finish = (result: RaftSandboxResult): void => {
        if (settled) return;
        settled = true;
      ...
      child.on("message", (raw: unknown) => {
        ...
        if (message.type === "result") {
          ...
          if (deadline) clearTimeout(deadline);
          if (
            message.result.terminationReason !== "completed" &&
            !hostAbortController.signal.aborted
          ) {
            hostAbortController.abort(
              new Error(message.result.error ?? "Process execution stopped"),
            );
          }
          void (async () => {
