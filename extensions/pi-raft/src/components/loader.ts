...
import { RaftComponentCatalog } from "./catalog.js";
import { RaftComponentSupervisor } from "./supervisor.js";
import type {
...
const cloneEntry = (entry: RaftComponentEntry): RaftComponentEntry => structuredClone(entry);
const entryHash = (entry: RaftComponentEntry): string => stableJsonHash(entry);

export class RaftComponentLoader {
  ...
  #desired = new Map<string, RaftComponentEntry>();
  #pinned = new Map<string, RaftComponentEntry>();
  #tail: Promise<void> = Promise.resolve();
  #closed = false;

  constructor(
    readonly catalog: RaftComponentCatalog,
    readonly supervisor: RaftComponentSupervisor,
  ) {
  ...
  pinnedEntries(): RaftComponentEntry[] {
  ...
  list(): RaftComponentInfo[] {
    ...
    for (const entry of this.#targetEntries().values()) {
      ...
      live.set(entry.id, {
        ...
        optionalMissing: [],
        effects: [],
        error: `Raft component definition is unavailable: ${entry.component}`,
        revision: 0,
        createdAt: now,
        updatedAt: now,
      });
  ...
  status(id: string): RaftComponentInfo {
    ...
    if (!info) throw new Error(`Unknown Raft component: ${id}`);
    return info;
  }
  ...
  reload(id?: string): Promise<RaftComponentInfo[]> {
    this.supervisor.assertLifecycleEntryAllowed("reload the component loader");
    return this.#enqueue(async () => {
      const targets = id ? [[id, this.#loaded.get(id)] as const] : [...this.#loaded.entries()];
      if (id && !targets[0]?.[1]) {
        const desired = this.#targetEntries().get(id);
        if (desired && !this.catalog.get(desired.component)) {
          throw new Error(`Raft component definition is unavailable: ${desired.component}`);
        }
        throw new Error(`Unknown Raft component: ${id}`);
      }
  ...
  installPinned(entries: readonly RaftComponentEntry[]): Promise<RaftComponentInfo[]> {
    this.supervisor.assertLifecycleEntryAllowed("install pinned components");
    if (entries.length > 256) throw new Error("Raft supports at most 256 pinned components");
    const next = this.#entryMap(entries);
    return this.#enqueue(async () => {
      for (const id of next.keys()) {
        if (this.#desired.has(id)) {
          throw new Error(`Pinned Raft component id conflicts with configured entry: ${id}`);
        }
      }
  ...
  reconcile(entries: readonly RaftComponentEntry[]): Promise<RaftComponentInfo[]> {
    this.supervisor.assertLifecycleEntryAllowed("reconcile the component loader");
    if (entries.length > 256) throw new Error("Raft configuration supports at most 256 components");
    const next = this.#entryMap(entries);
    return this.#enqueue(async () => {
      for (const id of next.keys()) {
        if (this.#pinned.has(id)) {
          throw new Error(`Raft component entry id is reserved by a pinned component: ${id}`);
        }
      }
  ...
  #entryMap(entries: readonly RaftComponentEntry[]): Map<string, RaftComponentEntry> {
    const next = new Map<string, RaftComponentEntry>();
    for (const rawEntry of entries) {
      const entry = cloneEntry(rawEntry);
      if (!ID_PATTERN.test(entry.id)) throw new Error(`Invalid Raft component id: ${entry.id}`);
      if (!entry.component.trim()) {
        throw new Error(`Raft component entry ${entry.id} has an empty component name`);
      }
      if (next.has(entry.id)) throw new Error(`Duplicate Raft component entry id: ${entry.id}`);
      next.set(entry.id, entry);
    }
    return next;
  }

  #targetEntries(): Map<string, RaftComponentEntry> {
    return new Map([...this.#pinned, ...this.#desired]);
  }

  #enqueue<T>(operation: () => Promise<T>): Promise<T> {
    if (this.#closed) return Promise.reject(new Error("Raft component loader is closed"));
  ...
  async #applyDesired(): Promise<void> {
    ...
    try {
      ...
      for (const [id, current] of Array.from(this.#loaded)) {
      ...
      if (rollbackErrors.length > 0) {
        // The original failure is the cause; the aggregate lists every rollback failure.
        const failure = new AggregateError(
          [error, ...rollbackErrors],
          "Raft component transaction and rollback failed",
          { cause: error },
        );
        throw failure;
      }
