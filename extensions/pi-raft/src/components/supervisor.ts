...
import {
  ActionRegistry,
  type RaftCallAudit,
  type RaftCapabilityViewLease,
} from "../core/action-registry.js";
import { formatRaftEffectConflict } from "../core/effect-conflict.js";
import { stableJsonHash } from "../core/stable-hash.js";
import {
  RaftComponentIndependenceError,
  actionEffect,
  compareEffectInfo,
...
import type { RaftInvocationContext, RaftScopedProviderResult } from "../protocol.js";
import { RaftEffectDivertedError, RaftEffectScope } from "./effect-scope.js";
import {
...
interface ManagedComponent {
  ...
  requirements: RaftCapabilityRequirement[];
  provisions: string[];
  missing: string[];
  ...
  scope: RaftEffectScope | undefined;
  viewLease: RaftCapabilityViewLease | undefined;
  providerLeases: RaftComponentProviderLease[];
  actionEffects: RaftComponentEffectInfo[];
  modelGuidance: NormalizedRaftModelGuidance[];
  abortController: AbortController | undefined;
  transition: Promise<void> | undefined;
  tearingDown?: boolean;
}

export interface RaftComponentSupervisorOptions {
...
interface RaftComponentLifecycleFrame {
  supervisor: RaftComponentSupervisor;
  componentId: string;
  phase: "loading" | "unloading";
}

type RaftComponentLifecycleStorage =
  import("node:async_hooks").AsyncLocalStorage<RaftComponentLifecycleFrame>;
let componentLifecycleStorage: RaftComponentLifecycleStorage | undefined;
let componentLifecycleStorageTask: Promise<RaftComponentLifecycleStorage> | undefined;
const lifecycleStorage = (): Promise<RaftComponentLifecycleStorage> => {
  componentLifecycleStorageTask ??= import("node:async_hooks").then(({ AsyncLocalStorage }) => {
    componentLifecycleStorage ??= new AsyncLocalStorage<RaftComponentLifecycleFrame>();
...
const normalizeRequirements = (
  ...
  for (const requirement of definition.requires ?? []) {
    ...
    if (!ref || ref.length > 256 || !ref.includes(".")) {
      throw new Error(
        `Raft component ${definition.name} requirement must use provider.action: ${ref || "<empty>"}`,
      );
    }
...
const normalizeProvisions = (definition: RaftComponentDefinition): string[] => {
...
const defaultInvocationContext = (): RaftInvocationContext => ({
  cwd: process.cwd(),
  signal: undefined,
...
export class RaftComponentSupervisor {
  ...
  constructor(
    ...
    readonly options: RaftComponentSupervisorOptions = {},
  ) {
  ...
  list(): RaftComponentInfo[] {
    const components = [...this.#components.values()].sort((left, right) =>
      left.entry.id.localeCompare(right.entry.id),
    );
    const effects = new Map(
      components.map((component) => [component.entry.id, this.#effects(component)]),
    );
    ...
    const conflicts = new Map(
      components.map((component) => [component.entry.id, [] as RaftComponentEffectConflict[]]),
    );
    ...
    return components.map((component) =>
      this.#info(component, effects.get(component.entry.id), conflicts.get(component.entry.id)),
    );
  }

  status(id: string): RaftComponentInfo {
    return this.#info(this.#require(id));
  }

  guidance(): RaftOwnedModelGuidance[] {
    return [...this.#components.values()]
      .filter((component) => component.state === "active")
      .flatMap((component) =>
        component.modelGuidance.map((guidance) => ({
      ...
      .sort(compareRaftOwnedModelGuidance);
  }

  graph(): RaftComponentGraph {
    const providers = new Map<string, string>();
    for (const component of this.#components.values()) {
      for (const provider of component.provisions) providers.set(provider, component.entry.id);
    }
    const dependencyEdges: RaftComponentGraph["edges"] = [];
    const ownershipEdges: RaftComponentGraph["edges"] = [];
    for (const component of this.#components.values()) {
  ...
  async start(
    entry: RaftComponentEntry,
    definition: RaftComponentDefinition,
  ): Promise<RaftComponentInfo> {
    this.#assertOpen();
    this.#assertLifecycleCallAllowed("start Raft components");
    ...
    if (component.state === "failed" || component.state === "quarantined") {
      throw new Error(component.error ?? `Raft component ${entry.id} failed to start`);
    }
  ...
  async replace(
    ...
    this.#assertOpen();
    this.#assertLifecycleCallAllowed("replace Raft components");
    const component = this.#require(id);
    if (entry.id !== id) throw new Error("Raft component replacement cannot change its entry id");
    ...
    const unloadedState = component.state as RaftComponentState;
    if (unloadedState === "quarantined") {
      throw new Error(component.error ?? `Raft component ${id} cleanup failed`);
    }
    this.#applyReplacement(component, entry, definition, previous.revision + 1, nextProvisions);
    await this.#requestReconcile();
    const candidateState = component.state as RaftComponentState;
    if (candidateState !== "failed" && candidateState !== "quarantined") {
      return this.#info(component);
    }

    const replacementError = component.error ?? `Raft component ${id} replacement failed`;
    ...
    await this.#requestReconcile();
    const rollbackState = component.state as RaftComponentState;
    if (rollbackState === "failed" || rollbackState === "quarantined") {
      throw new AggregateError(
        ...
        `Raft component ${id} replacement and rollback failed`,
      );
    }
    throw new Error(`${replacementError}; previous revision restored`);
  }

  async stop(id: string, options: RaftComponentStopOptions = {}): Promise<void> {
    const component = this.#require(id);
    ...
    this.#assertLifecycleCallAllowed("stop Raft components");
    this.#retire(component);
    await this.#unload(component, new Set());
  ...
  async settle(): Promise<void> {
    this.#assertLifecycleCallAllowed("settle Raft components");
    await this.#waitForReconcile();
  }

  async close(): Promise<void> {
    if (this.#closed) return;
    this.#assertLifecycleCallAllowed("close Raft components");
    this.#closed = true;
  ...
  #insert(
    ...
    if (this.#components.size >= MAX_SUPERVISED_COMPONENTS) {
      throw new Error(
        `Raft component supervisor supports at most ${MAX_SUPERVISED_COMPONENTS} fibers`,
      );
    }
    if (this.#components.has(entry.id)) {
      throw new Error(`Raft component already exists: ${entry.id}`);
    }
  ...
  #insertChild<TConfig>(
    ...
    if (
      ...
      throw new Error(`Raft component ${parent.entry.id} can only use children while activating`);
    }
    ...
    if (childCount >= MAX_CHILDREN_PER_COMPONENT) {
      throw new Error(
        `Raft component ${parent.entry.id} supports at most ${MAX_CHILDREN_PER_COMPONENT} children`,
      );
    }
    ...
    const localId = options.id
      ? rawLocalId
      : rawLocalId.replace(/[^a-zA-Z0-9._-]/g, "-").replace(/^[^a-zA-Z0-9]+/, "") ||
        `child-${sequence}`;
    if (!COMPONENT_ID_PATTERN.test(localId)) {
      throw new Error(`Invalid Raft child component id: ${rawLocalId}`);
    }
    ...
    const entry: RaftComponentEntry = {
      id,
    ...
    return {
      ...
      status: () => {
        if (this.#components.get(child.entry.id) !== child) {
          throw new Error(`Raft child component is no longer installed: ${child.entry.id}`);
        }
      ...
      stop: async (stopOptions) => {
        ...
        if (child.state === "quarantined" && !stopOptions?.force) {
          throw new Error(child.error ?? `Raft component ${child.entry.id} cleanup failed`);
        }
  ...
  #validateEntry(entry: RaftComponentEntry, definition: RaftComponentDefinition): void {
    if (!COMPONENT_ID_PATTERN.test(entry.id)) {
      throw new Error(`Invalid Raft component id: ${entry.id}`);
    }
    if (entry.component !== definition.name) {
      throw new Error(
        `Raft component entry ${entry.id} selects ${entry.component}, not ${definition.name}`,
      );
    }
  }
  ...
  #assertProvisionsAvailable(id: string, provisions: readonly string[]): void {
    for (const component of this.#components.values()) {
      ...
      if (overlap.length > 0) {
        throw new Error(
          `Raft components ${id} and ${component.entry.id} declare the same providers: ${overlap.join(", ")}`,
        );
      }
    }
    ...
    if (occupied.length > 0) {
      throw new Error(
        `Raft component ${id} providers are already registered outside the component: ${occupied.join(", ")}`,
      );
    }
  }
  ...
  async #reconcile(component: ManagedComponent): Promise<void> {
    ...
    const resolution = await this.registry.inspectCapabilities(component.requirements, baseContext);
    ...
    if (component.state === "active") {
      const provisionsActive =
        component.providerLeases.length === component.provisions.length &&
  ...
  async #load(
    component: ManagedComponent,
    baseContext: RaftInvocationContext,
    key: string,
  ): Promise<void> {
  ...
  async #performLoad(
    ...
    let scope: RaftEffectScope | undefined;
    let viewLease: RaftCapabilityViewLease | undefined;
    const providerLeases: RaftComponentProviderLease[] = [];
    const actionEffects: RaftComponentEffectInfo[] = [];
    const modelGuidance: NormalizedRaftModelGuidance[] = [];
    try {
      viewLease = await this.registry.acquireCapabilityView(component.requirements, {
        ...baseContext,
      ...
      if (!this.#transitionCurrent(component, epoch)) throw new RaftEffectDivertedError();
      const committedView = viewLease.view;
      scope = new RaftEffectScope({
      ...
      const assertRegistrationOpen = (): void => {
        if (component.tearingDown || scope?.state !== "open") {
          throw new Error(
            `Raft component ${component.entry.id} cannot register effects during teardown`,
          );
        }
      };
      ...
      const context: RaftComponentContext = {
        ...
        effect: async (setup, registration) => {
          ...
          if (component.guarantee === "revertible") {
            ...
            if (tracked.kind === "emission") {
              throw new Error(
                `Revertible Raft component ${component.entry.id} cannot register an emission effect`,
              );
            }
        ...
        defer: (disposer, registration) => {
          ...
          if (component.guarantee === "revertible" && tracked.kind === "emission") {
            throw new Error(
              `Revertible Raft component ${component.entry.id} cannot defer an emission effect`,
            );
          }
          return async () => {
            await dispose();
        ...
        guide: (guidance) => {
          assertRegistrationOpen();
          const normalized = normalizeRaftModelGuidance(guidance);
          if (modelGuidance.some((entry) => entry.label === normalized.label)) {
            throw new Error(
              `Raft component ${component.entry.id} registered guidance label ${normalized.label} more than once`,
            );
          }
          if (modelGuidance.length >= MAX_RAFT_MODEL_GUIDANCE_PER_COMPONENT) {
            throw new Error(
              `Raft component ${component.entry.id} supports at most ${MAX_RAFT_MODEL_GUIDANCE_PER_COMPONENT} guidance registrations`,
            );
          }
          const totalChars =
            modelGuidance.reduce((sum, entry) => sum + entry.content.length, 0) +
            normalized.content.length;
          if (totalChars > MAX_RAFT_MODEL_GUIDANCE_TOTAL_CHARS) {
            throw new Error(
              `Raft component ${component.entry.id} guidance exceeds ${MAX_RAFT_MODEL_GUIDANCE_TOTAL_CHARS} characters`,
            );
          }
          const projection = [...this.#components.values()].flatMap(
            (candidate) => candidate.modelGuidance,
          );
          if (projection.length >= MAX_RAFT_MODEL_GUIDANCE_REGISTRATIONS) {
            throw new Error(
              `Raft component guidance supports at most ${MAX_RAFT_MODEL_GUIDANCE_REGISTRATIONS} registrations`,
            );
          }
          const projectionChars =
            projection.reduce((sum, entry) => sum + entry.content.length, 0) +
            normalized.content.length;
          if (projectionChars > MAX_RAFT_MODEL_GUIDANCE_SNAPSHOT_CHARS) {
            throw new Error(
              `Raft component guidance snapshot exceeds ${MAX_RAFT_MODEL_GUIDANCE_SNAPSHOT_CHARS} characters`,
            );
          }
          const registration = {
            label: `guidance:${normalized.label}`,
            kind: "transactional" as const,
            resources: [`raft:guidance:${component.entry.id}:${normalized.label}`],
          ...
          let dispose: RaftComponentDisposer;
          try {
        ...
        provide: (provider) => {
          ...
          if (component.guarantee === "revertible" && !provider.close) {
            throw new Error(
              `Revertible Raft component ${component.entry.id} provider ${provider.name} must implement close()`,
            );
          }
          if (!declared.has(provider.name)) {
            throw new Error(
              `Raft component ${component.entry.id} mounted undeclared provider ${provider.name}`,
            );
          }
          if (providerLeases.some((lease) => lease.name === provider.name)) {
            throw new Error(
              `Raft component ${component.entry.id} mounted provider ${provider.name} more than once`,
            );
          }
        ...
        acquire: async <T = unknown>(ref: string, args?: Record<string, unknown>) => {
          if (!committedView.bindings[ref]) {
            throw new Error(
              `Raft component ${component.entry.id} acquired undeclared or unavailable capability ${ref}`,
            );
          }
        ...
        call: async (ref, args) => {
          if (!committedView.bindings[ref]) {
            throw new Error(
              `Raft component ${component.entry.id} called undeclared or unavailable capability ${ref}`,
            );
          }
          const callInvocation: RaftInvocationContext = {
            ...invocation,
            signal: component.tearingDown ? undefined : invocation.signal,
          };
          const action = await this.registry.describe(ref, callInvocation);
          if (action.effect?.kind === "scoped") {
            throw new Error(`Raft scoped action ${ref} must be used through context.acquire()`);
          }
          if (
            ...
            throw new Error(
              `Revertible Raft component ${component.entry.id} cannot emit non-revertible action ${ref}`,
            );
          }
          const effect = actionEffect(action);
      ...
      if (unprovided.length > 0) {
        throw new Error(
          `Raft component ${component.entry.id} did not mount declared providers: ${unprovided.join(", ")}`,
        );
      }
      if (!(await this.#targetMatches(component, epoch, committedView.digest, baseContext))) {
        throw new RaftEffectDivertedError(
          `Raft component ${component.entry.id} capability target changed during activation`,
        );
      }
      if (component.guarantee === "revertible") {
        this.#assertIndependent(component, this.#effects(component));
      }
      this.registry.activateProviderBindings(providerLeases.map((lease) => lease.bindingId));
      if (!this.#transitionCurrent(component, epoch)) {
        throw new RaftEffectDivertedError(
          `Raft component ${component.entry.id} was retired during activation`,
        );
      }
      ...
      const report = scope ? await scope.dispose() : { status: "disposed" as const, failures: [] };
      ...
      const cleanupErrors = [
        ...
        ...(error instanceof RaftEffectDivertedError && error.cleanupError !== undefined
      ...
      let diverted =
        error instanceof RaftEffectDivertedError || !this.#transitionCurrent(component, epoch);
      ...
      if (cleanupErrors.length > 0) {
        ...
        component.consecutiveDiversions = 0;
        component.blockedOnEffects = error instanceof RaftComponentIndependenceError;
        component.error = errorMessage(error);
  ...
  async #unload(component: ManagedComponent, visited: Set<string>): Promise<void> {
    ...
    component.abortController?.abort(
      new Error(`Raft component ${component.entry.id} is unloading`),
    );
    if (component.transition) await component.transition;
  ...
  async #performUnload(component: ManagedComponent, visited: Set<string>): Promise<void> {
    ...
    if (cleanupErrors.length > 0) {
      component.state = "quarantined";
      component.error = `Raft component ${component.entry.id} cleanup failed`;
      component.cleanupErrors = cleanupErrors;
    } else {
      component.state = "waiting";
      delete component.cleanupErrors;
  ...
  async #retireOwnedChild(
    ...
    const cleanupError =
      child.state === "quarantined"
        ? new Error(child.error ?? `Raft child component ${childId} cleanup failed`)
        : undefined;
  ...
  #effects(component: ManagedComponent): RaftComponentEffectInfo[] {
  ...
  #effectConflicts(
    ...
    const conflicts: RaftComponentEffectConflict[] = [];
  ...
  #assertEffectCapacity(component: ManagedComponent, additional = 0): void {
    ...
    const count =
      (component.scope?.footprint(MAX_COMPONENT_EFFECTS + 1).length ?? 0) +
      component.actionEffects.length +
      additional;
    if (count > MAX_COMPONENT_EFFECTS) {
      throw new Error(
        `Revertible Raft component ${component.entry.id} exceeds ${MAX_COMPONENT_EFFECTS} tracked effects`,
      );
    }
  }
  ...
  #assertIndependent(
    ...
    throw new RaftComponentIndependenceError(
      `Revertible Raft component ${component.entry.id} has non-independent effects: ` +
        conflicts
          .map((conflict) =>
            formatRaftEffectConflict(conflict.withComponent, conflict.resources, conflict.reason),
          )
          .join("; "),
    );
  }

  #invocationContext(component: ManagedComponent): RaftInvocationContext {
  ...
  #info(
    component: ManagedComponent,
    projectedEffects?: RaftComponentEffectInfo[],
    projectedConflicts?: RaftComponentEffectConflict[],
  ): RaftComponentInfo {
    ...
    return {
      ...
      ...(component.state === "active" && component.modelGuidance.length > 0
        ? { guidance: component.modelGuidance.map(raftModelGuidanceInfo) }
        : {}),
  ...
  #cycles(edges: RaftComponentGraph["edges"]): string[][] {
  ...
  #assertLifecycleCallAllowed(operation: string): void {
    ...
    throw new Error(`Cannot ${operation} from ${frame.phase} transition ${frame.componentId}`);
  }

  #require(id: string): ManagedComponent {
    const component = this.#components.get(id);
    if (!component) throw new Error(`Unknown Raft component: ${id}`);
    return component;
  }

  #assertOpen(): void {
    if (this.#closed) throw new Error("Raft component supervisor is closed");
  }
