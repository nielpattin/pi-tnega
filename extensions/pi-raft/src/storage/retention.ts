...
export const RAFT_RUN_ROOT_PREFIX = "pi-raft-runs-";
const RUN_ROOT_OWNER_FILE = ".raft-owner.json";
...
export const markRunRootActive = (root: string, now = Date.now()): void => {
  ...
  writeOwner(root, { pid: process.pid, startedAt: existing?.startedAt ?? now, heartbeatAt: now });
};
...
const pruneClosedRunRoot = (
  ...
  for (const entry of entries) {
    ...
    let fallback = now;
    try {
      fallback = fs.statSync(runDirectory).mtimeMs;
    } catch {}
...
export const sweepTempRunRoots = (options: {
  ...
  for (const entry of entries) {
    if (!entry.isDirectory() || !entry.name.startsWith(RAFT_RUN_ROOT_PREFIX)) continue;
