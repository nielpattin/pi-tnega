...
// Argument-shape normalization shared by the stable Raft providers,
...
const VALUE_CLASS_FORMS = ENUM_VALUE_CLASSES.map((cls) => new Set(cls.map(normalizeForm)));
...
const stringEnumValues = (property: unknown): readonly string[] | undefined => {
  ...
  for (const branch of [schema.oneOf, schema.anyOf]) {
    ...
    const consts = branch
      .map((entry) =>
        entry && typeof entry === "object" ? (entry as { const?: unknown }).const : undefined,
      )
...
type DerivedAction = {
  ...
  // True when the schema declares exactly `id` and requires it: the whole
  // argument may then be spelled as a bare id or as a returned handle.
  idOnly: boolean;
...
const deriveAction = (
  ...
  const required = Array.isArray(inputSchema?.required)
    ? (inputSchema.required as unknown[]).filter((key): key is string => typeof key === "string")
    : [];
  const idOnly = declared.size === 1 && declared.has("id") && required.includes("id");
...
// Id-only actions take their whole argument as a bare id, wait("a1"), or as the
// handle a spawn returned, wait(handle), because that is what a natural
// spawn-then-wait chain has in hand. Only declared keys survive the unwrap, so
// stray handle fields cannot reach a schema with additionalProperties: false.
const unwrapIdOnly = (
  args: Record<string, unknown>,
  derived: DerivedAction,
): Record<string, unknown> => {
  if (!derived.idOnly) return args;
  const original = args as unknown;
  if (typeof original === "string") {
    const trimmed = original.trim();
    return trimmed ? { id: trimmed } : args;
  }
  if (!original || typeof original !== "object" || Array.isArray(original)) return args;
  const id = args.id;
  if (typeof id !== "string" || !id.trim()) return args;
  return { id };
};
...
const applyDerived = (
  ...
  for (const key of Array.from(Object.keys(out))) {
...
export const normalizeActionArgs = (
  ...
  const shaped = unwrapIdOnly(args, derived);
  return applyDerived(shaped, derived);
};
...
export const actionArgNormalizer = (
  describeActions: () => ReadonlyArray<Pick<RaftActionDescriptor, "name" | "inputSchema">>,
  ...
  return (actionName, args) => {
    ...
    const shaped = unwrapIdOnly(args, action);
    return applyDerived(shaped, action);
  };
};
