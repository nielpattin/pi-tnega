...
import type { RaftMcpConfig } from "../config.js";
import type {
...
const managementDescriptors: RaftActionDescriptor[] = [
  {
    name: "$servers",
...
export interface McpProviderHooks {
  ...
  onSliceChanged?: (descriptors: RaftActionDescriptor[]) => void;
}
...
/** Authorized metadata and transport only; never configuration or credentials. */
export interface HostedMcpSource {
  listServers(): string[];
  listTools(server: string): Promise<HostedMcpTool[]>;
  callTool(
    server: string,
    tool: string,
    options: {
...
type ToolRuntime =
  | Pick<Runtime, "listServers" | "listTools" | "callTool" | "close">
  | HostedMcpSource;
...
export class McpProvider implements RaftProvider {
  readonly name = "mcp";
  ...
  get #cacheOn(): boolean {
    return !this.#source && this.config.cache.enabled;
  }
  ...
  async list(
    ...
    const filterQuery = (descriptors: RaftActionDescriptor[]): RaftActionDescriptor[] =>
      query
    ...
    return [...managementDescriptors, ...filterQuery(this.sliceDescriptors())];
  }
  ...
  async describe(
    ...
    if (!this.#cacheOn) {
      ...
      // from turning an unknown source/tool (including ambiguous aliases) into a call.
      if (this.#source && !descriptor) throw new Error(`Unknown hosted MCP action: ${actionName}`);
      return descriptor;
    }
  ...
  async invoke(
    ...
    if (!this.config.enabled) throw new Error("MCP support is disabled in Raft configuration");
    ...
    if (actionName === "$register") {
      if (!this.config.allowDynamicServers) {
        throw new Error("Dynamic MCP server registration is disabled in Raft configuration");
      }
  ...
  // ephemeral.
  sliceDescriptors(): RaftActionDescriptor[] {
  ...
  async #hydrateInternal(): Promise<void> {
    ...
    try {
      ...
      if (snapshot) {
        ...
        console.warn(
          "[pi-raft] MCP config could not be parsed; serving last-known cached MCP tools.",
        );
      }
    }
    this.#scheduleNotify();
    ...
    for (const [name, entry] of this.#servers) {
      ...
      servers[name] = {
        ...
        tools: entry.tools.map((tool) => {
          const annotations = (tool as { annotations?: RaftToolAnnotations }).annotations;
          return {
            name: tool.name,
  ...
  async #getToolRuntime(): Promise<ToolRuntime> {
    if (this.#closed) throw new Error("MCP provider is closed");
    return this.#source ?? this.#getRuntime();
  }

  async #getRuntime(): Promise<Runtime> {
    if (this.#source) throw new Error("Hosted MCP cannot construct an ambient runtime");
    if (this.#closed) throw new Error("MCP provider is closed");
    ...
    const promise = import("mcporter")
      .then(({ createRuntime }) =>
        createRuntime({
          ...
          clientInfo: { name: "pi-raft", version: "0.1.0" },
        }),
      )
      .then(async (runtime) => {
  ...
  #serverDefinition(args: Record<string, unknown>): ServerDefinition {
    const name = stringifyUnknown(args.name ?? "").trim();
    if (!/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/.test(name)) {
      throw new Error(
        "Dynamic MCP server names may contain letters, numbers, dots, underscores, and hyphens",
      );
    }
    ...
    if (typeof args.baseUrl === "string" && args.baseUrl.trim()) {
      ...
      return {
        name,
        ...(description ? { description } : {}),
        command: { kind: "http", url: new URL(args.baseUrl), ...(headers ? { headers } : {}) },
        ...(env ? { env } : {}),
      };
    }
  ...
  #resolveTool(serverTools: ServerToolInfo[], requested: string): ServerToolInfo | undefined {
    const exact = serverTools.find((tool) => tool.name === requested);
    if (exact) return exact;
    const matches = serverTools.filter((tool) => sanitizeMcpRefPart(tool.name) === requested);
    return matches.length === 1 ? matches[0] : undefined;
  }
  ...
  #toolDescriptor(server: string, tool: ServerToolInfo): RaftActionDescriptor {
  ...
  async #listLegacy(
    ...
    const selected = request.namespace
