...
export interface RaftSpeculationTapOptions {
  enabled(): boolean;
  maxBufferBytes(): number;
  /** Static cheap gate (Tier-A set / MCP allowlist) before the registry re-validates. */
  isEligible(ref: string): boolean;
  launch(toolCallId: string, candidate: RaftSpeculationCandidate, context: ExtensionContext): void;
}

const toolCallBlock = (partial: unknown, contentIndex: number): { name?: string; id?: string } => {
...
/**
 * Watches assistant message streaming for raft_exec tool calls, incrementally
 ...
 */
export class RaftSpeculationStreamTap {
  readonly #options: RaftSpeculationTapOptions;
  ...
  constructor(options: RaftSpeculationTapOptions) {
  ...
  setScannerFactory(factory: () => CallScanner): void {
    ...
    for (const stream of this.#streams.values()) {
      ...
      if (!stream.isRaftExec || code === undefined) continue;
      try {
  ...
  readonly #pendingCatchUp: { stream: StreamState; candidate: RaftSpeculationCandidate }[] = [];

  /** Drain candidates recovered while the scanner module loaded. */
  flushCatchUp(context: Parameters<RaftSpeculationTapOptions["launch"]>[2]): void {
    ...
    try {
      ...
      if (assistantEvent.type === "toolcall_start") {
        ...
        this.#streams.set(assistantEvent.contentIndex, {
          ...
          isRaftExec: block.name !== undefined ? block.name === "raft_exec" : true,
      ...
      if (assistantEvent.type === "toolcall_delta") {
        ...
        if (block.name !== undefined) stream.isRaftExec = block.name === "raft_exec";
        stream.extractor.push(assistantEvent.delta);
        const code = stream.extractor.code;
        if (!stream.isRaftExec || code === undefined) return;
      ...
      if (assistantEvent.type === "toolcall_end") {
        ...
        if (block.name !== undefined) stream.isRaftExec = block.name === "raft_exec";
        const code = stream.extractor.code;
        if (stream.isRaftExec && code !== undefined) {
