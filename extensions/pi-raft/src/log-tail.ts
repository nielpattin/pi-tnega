...
const parseLine = (offset: number, raw: string): RaftLogLine => {
  let parsed: unknown;
  try {
...
const readJsonlPageFromDescriptor = (
  descriptor: number,
  limit: number,
  before?: number,
  knownSize?: number,
  ...
  try {
    ...
    const fileEnd =
      typeof before === "number" && Number.isSafeInteger(before)
        ? Math.max(0, Math.min(before, size))
        : size;
    const boundedLimit = Math.max(1, Math.trunc(limit));
    const boundedBytes = Math.max(1, Math.trunc(maxBytes ?? DEFAULT_READ_MAX_BYTES));
    const chunks: Buffer[] = [];
