...
import { externalToolCallDetail } from "./external-tool-renderers/index.js";
import {
  isRaftAgentToolPreview,
...
} from "./raft-code-parser.js";
export { raftWriteBindings, type RaftWriteBinding } from "./raft-code-parser.js";
import {
  applyDiffBackground,
...
export interface RaftRenderAudit {
  ref: string;
  tool?: string;
  provider?: string;
  success?: boolean;
    ...
    for (let lineIndex = 0; lineIndex < this.lines.length; lineIndex++) {
      ...
      if (!kind) {
        ...
        if (this.wrapLineIndexes?.has(lineIndex)) {
          ...
          renderedRows = wrapped.map((row, index) =>
            truncateBoundedLine(index === 0 ? row : continuationIndent + row, width),
          );
        } else renderedRows = [truncateBoundedLine(line, width)];
        rows.push(
          ...(this.inheritBackground ? renderedRows.map(inheritEnclosingBackground) : renderedRows),
        );
        continue;
      }
...
export const raftMulticallCallLimit = (expanded: boolean): number =>
  expanded ? EXPANDED_MULTICALL_LIMIT : COLLAPSED_MULTICALL_LIMIT;

const visibleMulticallAudits = (
  audits: RaftRenderAudit[],
  expanded: boolean,
): Array<{ audit: RaftRenderAudit; auditIndex: number }> => {
  const limit = raftMulticallCallLimit(expanded);
...
const legacyCommandsFrom = (raftArgs: unknown): ReadonlyMap<string, string> => {
  const args = recordOf(raftArgs);
  ...
  const code =
    typeof rawCode === "string"
      ...
      : Array.isArray(rawCode) && rawCode.every((value) => typeof value === "string")
        ...
        : undefined;
  if (code) {
    for (const literal of raftStringLiterals(code)) remember(literal);
  }
...
export const restoreLegacyBashCommands = (
  ...
  const hasLegacyCommand = audits.some((audit) => {
    const digest =
      audit.ref === "pi.bash" ? argString(audit.args ?? {}, "commandDigest") : undefined;
    return Boolean(digest && LEGACY_COMMAND_DIGEST.test(digest));
  });
  if (!hasLegacyCommand) return audits;

  const commands = legacyCommandsFrom(raftArgs);
  return audits.map((audit) => {
    ...
    return { ...audit, args: command ? { ...argsWithoutDigest, command } : argsWithoutDigest };
  });
};

export interface RaftWriteArgumentPreviewInput {
  bindings: RaftWriteBinding[];
...
const renderWriteArgumentBody = (
  ...
  const highlighted =
    lang && shown.length > 0 ? highlightCode(shown.join("\n"), lang, invalidate) : null;
  return {
...
export const renderRaftWriteArgumentPreview = (
  input: RaftWriteArgumentPreviewInput,
  theme: Theme,
  invalidate?: () => void,
): Component | null => {
  ...
  const rows = [
    theme.fg("warning", `◆ Raft composing · ${completed}/${input.bindings.length} writes`),
  ];
  const callLimit = raftMulticallCallLimit(input.expanded);
...
const providerCallDetail = (
  ...
  if (provider === "agents") {
    ...
    const previewName = isRaftAgentToolPreview(preview) ? preview.name : undefined;
...
const callHeadlinePreview = (audit: RaftRenderAudit): string | undefined => {
  const ref = audit.ref;
...
const nestedResultTruncated = (audit: RaftRenderAudit): boolean => {
...
/** Compact one-line title for a nested Raft call, e.g. `read src/index.ts` or `$ ls -la`. */
export function nestedCallTitle(
  ...
  return nestedResultTruncated(audit) ? `${title} ${theme.fg("warning", "· truncated")}` : title;
}
...
const nestedCallTitleText = (
  ...
  const externalDetail = externalToolCallDetail(audit);
  ...
  if (path) detail = path;
    ...
    // An external renderer's empty detail means "handled, show nothing" (hashline hides
    // anchors this way), so it stays authoritative; the generic helpers return "" for
    // "no detail" and must fall through to the restored preview headline.
    detail =
      externalDetail ??
      (headlineArg(args) ||
        structuralCallDetail(provider, tool, args, audit.result) ||
...
const transcriptToolAudit = (entry: RaftTranscriptEntry): RaftRenderAudit => {
...
const descendantBreadcrumb = (node: RaftAgentToolPreviewNode, theme: Theme): string | undefined => {
  let tail: RaftAgentToolPreviewNode | undefined =
  ...
  for (let depth = 0; depth < AGENT_PREVIEW_RENDER_MAX_DEPTH; depth += 1) {
    const next: RaftAgentToolPreviewNode | undefined =
    ...
    ? options.compact
      ...
      : rawPreviewText.split("\n").flatMap((line) => {
          ...
          for (
            ...
            chunks.push(
              codePoints.slice(offset, offset + AGENT_RESPONSE_LINE_CODE_POINTS).join(""),
            );
          }
          return chunks;
        })
...
interface RaftMulticallPreview {
  auditIndex: number;
  body: string;
  hidden: number;
}

export interface RaftMulticallPartialInput {
  audits: RaftRenderAudit[];
  phases: string[];
  progress?: string | undefined;
  expanded: boolean;
  preview?: RaftMulticallPreview | undefined;
  core?: { cwd: string; settings: CodePreviewSettings } | undefined;
...
export const renderRaftMulticallPartial = (
  input: RaftMulticallPartialInput,
  theme: Theme,
  invalidate?: () => void,
): Component => {
  ...
  let header = theme.fg(
    "warning",
    `◆ ${input.activityLabel ?? "Raft"} running · ${done}/${input.audits.length} calls`,
  );
  ...
  if (callsHidden > 0) {
    ...
    rows.push(
      theme.fg("dim", label) + (input.expanded ? "" : theme.fg("dim", " · ") + expandHint(theme)),
    );
  }
...
export interface RaftCoreToolPreview extends RaftRenderAudit {
  ref: string;
}
...
export const captureRaftCoreToolPreviews = (
  audits: RaftRenderAudit[],
  previous: RaftCoreToolPreview[] = [],
): RaftCoreToolPreview[] => {
  ...
  return audits.flatMap((audit) => {
    ...
    const index = prior.findIndex(
      (candidate) =>
        candidate.ref === audit.ref && argString(candidate.args ?? {}, "path") === path,
    );
...
export const restoreRaftCoreToolPreviews = (
  audits: RaftRenderAudit[],
  previews: RaftCoreToolPreview[],
): RaftRenderAudit[] => {
...
export interface RaftAgentPreview {
  ref: string;
  id: string;
  preview: unknown;
}

export const captureRaftAgentPreviews = (
  audits: RaftRenderAudit[],
  previous: RaftAgentPreview[] = [],
): RaftAgentPreview[] => {
  ...
  for (const audit of audits) {
    if (!isRaftAgentToolPreview(audit.preview)) continue;
...
export const restoreRaftAgentPreviews = (
  audits: RaftRenderAudit[],
  previews: RaftAgentPreview[],
): RaftRenderAudit[] => {
  const remaining = previews.slice();
  return audits.map((audit) => {
    if (isRaftAgentToolPreview(audit.preview)) return audit;
...
export interface RaftWritePreview {
  ref: string;
  path?: string | undefined;
  content: string;
}

export const captureRaftWritePreviews = (audits: RaftRenderAudit[]): RaftWritePreview[] =>
...
export interface RaftCallHeadlinePreview {
  ref: string;
  headline: string;
}

export const captureRaftCallHeadlinePreviews = (
  audits: RaftRenderAudit[],
): RaftCallHeadlinePreview[] =>
  audits.flatMap((audit) => {
    const headline = callHeadlinePreview(audit);
    return headline ? [{ ref: audit.ref, headline }] : [];
  });

export const restoreRaftCallHeadlinePreviews = (
  audits: RaftRenderAudit[],
  previews: RaftCallHeadlinePreview[],
): RaftRenderAudit[] => {
  const remaining = previews.slice();
  return audits.map((audit) => {
...
export const restoreRaftWritePreviews = (
  audits: RaftRenderAudit[],
  previews: RaftWritePreview[],
): RaftRenderAudit[] => {
  const remaining = previews.slice();
  return audits.map((audit) => {
...
export function nestedCallBody(audit: RaftRenderAudit): string | undefined {
...
export function nestedCallCode(audit: RaftRenderAudit): { code: string; lang: string } | null {
  const args = audit.args ?? {};
...
const lineDiff = (oldLines: string[], newLines: string[]): DiffLine[] => {
  ...
  for (let i = m - 1; i >= 0; i--) {
    ...
    for (let j = n - 1; j >= 0; j--) {
      ...
      cur[j] =
        oldLine === newLine ? (next[j + 1] ?? 0) + 1 : Math.max(next[j] ?? 0, cur[j + 1] ?? 0);
    }
  }
  const out: DiffLine[] = [];
  let i = 0;
...
// Mirrors pi core's read range notice: surface how many lines a raft_exec
...
export function modelReadHint(audits: RaftRenderAudit[], output: string, theme: Theme): string {
  if (!output) return "";
