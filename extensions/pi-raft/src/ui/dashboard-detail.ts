...
import type { RaftActivityCall } from "../activity/types.js";
...
import { nestedEditDiff, renderBoundedLines } from "./raft-render.js";
import {
  formatClock,
  formatDuration,
  formatTokens,
...
import type { RaftAgentTranscript, RaftTranscriptEntry } from "./transcript.js";
...
export type RaftTranscriptTarget = RaftUiAgent;
...
interface DashboardDetailRendererOptions {
  agentTranscript: ((agent: RaftUiAgent, followLatest: boolean) => RaftAgentTranscript) | undefined;
...
export class DashboardDetailRenderer {
  ...
  private transcriptTarget(entity: Entity): RaftTranscriptTarget | undefined {
    if (entity.kind === "agent") return entity.value;
    return undefined;
  }

  private hasTranscript(entity: Entity): boolean {
    return entity.kind === "agent" && this.agentTranscript !== undefined;
  }

  private transcriptFor(entity: Entity): RaftAgentTranscript | undefined {
  ...
  private renderDetail(width: number, snapshot: RaftDashboardSnapshot, entity: Entity): string[] {
    ...
    const actionLines = wrapPlainText(
    ...
    const viewLabel = transcriptView
      ...
      : "";
    const kindLabel = entity.kind === "main" ? "main agent" : entity.kind;
  ...
  private transcriptLines(entity: Entity, width: number): string[] {
    ...
    const transcriptCwd = entity.kind === "agent" ? entity.value.cwd : undefined;
  ...
  private transcriptToolLines(
    ...
    const headline =
      title ?? this.theme.fg("toolTitle", this.theme.bold(entry.toolName ?? entry.label));
  ...
  private transcriptToolAudit(entry: RaftTranscriptEntry): {
    ref: string;
    provider: string;
    tool: string;
    ...
    const tool =
      normalizedName === "glob"
        ? "find"
        : ["read", "write", "edit", "bash", "powershell", "grep", "find", "ls"].includes(
              normalizedName,
            )
          ? normalizedName
          : rawName;
  ...
  private transcriptStructuredLines(
    ...
    for (const row of highlighted) {
      for (const wrapped of wrapTextWithAnsi(
        row,
        Math.max(1, width - visibleWidth(nestedPadding)),
      )) {
  ...
  private detailLines(entity: Entity, width: number, now: number, cwd: string): string[] {
    ...
    const coreCallPreview = (call: RaftActivityCall): boolean => {
      const settings = this.codePreviewSettings;
      const tool = call.ref.startsWith("pi.") ? call.ref.slice(3) : "";
      if (
        !settings ||
        !["bash", "powershell", "read", "write", "edit", "grep", "find", "ls"].includes(tool)
      ) {
        return false;
      }
      const success =
        call.status === "completed" ? true : call.status === "failed" ? false : undefined;
      const audit = {
        ref: call.ref,
        provider: "pi",
        tool,
    ...
    const argumentField = (call: RaftActivityCall): void => {
      ...
      if (call.ref === "pi.edit" && edits.length > 0) {
        ...
        const diff = nestedEditDiff(
          { ref: call.ref, tool: call.ref.split(".")[1] ?? call.ref, args },
          this.theme,
    ...
    if (entity.kind === "main") {
      ...
      const visibleEffects = componentEffects
        .slice(0, 8)
        .map(
          (effect) =>
