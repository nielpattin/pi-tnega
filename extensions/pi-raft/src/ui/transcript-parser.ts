...
export class TranscriptAccumulator {
  readonly entries: RaftTranscriptEntry[] = [];
  readonly #tools = new Map<string, RaftTranscriptEntry>();
  readonly #anonymousTools = new Map<string, RaftTranscriptEntry[]>();
  readonly #activeTools: RaftTranscriptEntry[] = [];
  #assistant: RaftTranscriptEntry | undefined;
  #retry: RaftTranscriptEntry | undefined;
  #compaction: RaftTranscriptEntry | undefined;
  readonly #userMessages = new Map<string, RaftTranscriptEntry>();
  ...
  snapshot(
    ...
    const entries =
      maxEntries > 0 && this.entries.length > maxEntries
  ...
  #pushMessage(
    ...
    id: string,
    text: string,
    status: RaftTranscriptEntryStatus = "completed",
    ...
    if (!safe) return;
    const entry: RaftTranscriptEntry = { id, kind, label, text: safe, status };
  ...
  #toolParent(id: string): RaftTranscriptEntry | undefined {
    if (!id.startsWith("raft_")) return undefined;
    for (let index = this.#activeTools.length - 1; index >= 0; index--) {
      const candidate = this.#activeTools[index];
      if (candidate?.toolName === "raft_exec" && candidate.status === "running") return candidate;
    }
    return undefined;
  }

  #startTool(id: string, label: string, args: unknown): RaftTranscriptEntry {
    ...
    const entry: RaftTranscriptEntry = {
      id,
      kind: "tool",
      label: safeLabel,
      toolName: safeLabel,
  ...
  #appendSessionMessage(event: Record<string, unknown>, message: Record<string, unknown>): void {
    ...
    if (message.role === "user") {
      this.#pushMessage(
        ...
        "completed",
        "User",
        this.#userIdentity(event, message),
      );
      return;
    }
  ...
  #append(event: Record<string, unknown>): void {
    ...
    if (event.type === "result") {
      ...
      if (event.is_error === true || event.subtype !== "success") {
        const errors = Array.isArray(event.errors)
          ...
          : "";
        const text =
          errors || (typeof event.result === "string" ? event.result : "Claude run failed");
        this.entries.push({
          id,
          kind: "error",
          label: "Claude result",
    ...
    if (event.type === "extension_error" || event.type === "worker_stderr") {
      ...
      const warningCount =
        event.type === "worker_stderr" &&
        warningLines.length > 0 &&
        warningLines.every((line) => /^\s*Warning:/.test(line))
          ? warningLines.length
          : 0;
      this.entries.push({
        id,
...
export const parsedEvents = (lines: RaftLogLine[]): Array<Record<string, unknown>> =>
  lines
