...
/** A single pickable row in the Raft model selector. */
interface ModelEntry {
  /** Value persisted to raft.json: "Inherit" or `provider/id`. */
  value: string;
...
export interface RaftModelSelectorChoice {
  value: string;
  label: string;
  description: string;
  current: boolean;
...
export interface RaftModelSelectorOptions {
  theme: Theme;
  source: ModelSource;
  ...
  inheritName?: string;
  /** Whether to include the Inherit row. Defaults to true. */
  allowInherit?: boolean;
  /** Whether to wrap the selector with top and bottom dynamic borders. Defaults to false. */
  bordered?: boolean;
}

/**
 * A /model-style searchable model picker adapted for Raft: same look
 * (search input, list with `[provider]` badges and a ✓ on
 * the current row, scroll indicator, and a "Model Name:" footer) but it writes
 * the Raft default-model setting instead of the host's default model, and
 ...
 */
export class RaftModelSelector extends Container implements Focusable {
  ...
  constructor(options: RaftModelSelectorOptions) {
    super();
    ...
    this.allowInherit = options.allowInherit ?? true;
    this.headerText =
      options.headerText ??
      (this.allowInherit
        ? "Default model for Raft child agents. Pick Inherit to use the host session's model."
        : "Select a model.");
    ...
    this.addChild(new Text(this.theme.fg("muted", this.headerText), 0, 0));
    this.addChild(new Spacer(1));
  ...
  rpcChoices(): RaftModelSelectorChoice[] {
  ...
  private updateList(): void {
    ...
    if (total === 0) {
      ...
      this.listContainer.addChild(
        new Text(this.theme.fg("muted", `  Model Name: ${selected ? selected.name : ""}`), 0, 0),
      );
    }
  }
}
