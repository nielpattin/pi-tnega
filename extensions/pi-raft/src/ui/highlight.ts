...
export const effectiveShikiThemeIsLight = (): boolean => THEME_TYPE.get(currentTheme) === "light";
...
export function configureHighlighting(themePreferenceValue: string, syntaxEnabled = true): void {
  ...
  // /raft settings save even when nothing display-related had changed.
...
export async function initHighlighting(theme: string, syntaxEnabled = true): Promise<void> {
  ...
  try {
    ...
    // Resolve the theme object from pi-raft's own module graph and hand
    ...
    initializingTheme = undefined;
    console.warn("[pi-raft] Shiki failed to initialize; previews will be plain text.", error);
    highlighter?.dispose();
    highlighter = undefined;
    readyTheme = undefined;
    highlighterGeneration++;
