export interface RaftWriteBinding {
  path: string;
  stringKey: string;
}
...
export const raftStringLiterals = (code: string): string[] =>
  tokenize(code)
...
const objectBinding = (
  tokens: Token[],
  start: number,
): { binding?: RaftWriteBinding; next: number } => {
  let depth = 1;
  let index = start + 1;
...
export const raftWriteBindings = (code: string): RaftWriteBinding[] => {
  const tokens = tokenize(code);
  const bindings: RaftWriteBinding[] = [];
...
const titleBasename = (value: string): string => value.split("/").filter(Boolean).pop() ?? value;
...
const windowFirstString = (tokens: Token[], start: number, end: number): string | undefined => {
  for (let index = start; index < end; index++) {
    if (tokens[index]?.kind === "string" && !isNamedStringToken(tokens, index))
      return tokens[index]!.text;
  }
  return undefined;
};
...
const windowPathLike = (tokens: Token[], start: number, end: number): string | undefined => {
  for (let index = start; index < end; index++) {
    if (
      tokens[index]?.kind === "string" &&
      !isNamedStringToken(tokens, index) &&
      titleAnchorPathLike(tokens[index]!.text)
    ) {
      return tokens[index]!.text;
    }
  }
...
const searchTarget = (tokens: Token[], start: number, end: number): string | undefined => {
  ...
  if (pattern !== undefined) {
    if (titleAnchorPathLike(pattern))
      head = titleClip(titleBasename(pattern), TITLE_MAX_ANCHOR_CHARS);
    else if (TITLE_SAFE_ANCHOR.test(pattern) && pattern.length <= TITLE_MAX_PATTERN_CHARS)
      head = `"${pattern}"`;
  }
  ...
  if (tail === undefined && pathValue === undefined) {
    ...
    if (
      ...
      TITLE_SAFE_ANCHOR.test(positional) &&
      positional.length <= TITLE_MAX_ANCHOR_CHARS
    ) {
      tail = titleAnchorPathLike(positional)
        ? titleClip(titleBasename(positional), TITLE_MAX_ANCHOR_CHARS)
        : positional;
    }
  }
...
const piCallTarget = (
  ...
  if (label === "Shell") {
    ...
    return command !== undefined
      ? titleClip(command.split("\n")[0]!, TITLE_MAX_COMMAND_CHARS)
      : undefined;
  }
...
// Deterministic lexical title for a raft_exec program, derived without
...
// titles. Returns undefined when the program holds no recognizable Raft
// call, letting callers keep a neutral fallback.
export const raftExecTitleHint = (code: string): string | undefined => {
  const tokens = tokenize(code);
  ...
  for (let index = 0; index < tokens.length; index++) {
        ...
        token.text === "agents"
          ? (() => {
              ...
              return task !== undefined
                ? clipWords(task.split("\n")[0]!, TITLE_MAX_TASK_CHARS)
                : undefined;
            })()
          : undefined;
    ...
    if (token.text === "extensions" && tokens[index + 3]?.text === "(") {
      const label = humanizeIdentifier(leaf.text);
      const window = callWindow(tokens, index + 3);
      record(label, piCallTarget(label, tokens, window.start, window.end));
      continue;
    }
    ...
    if (tokens[index + 3]?.text === "(") {
      ...
      if (token.text === "memory" || token.text === "state") {
        ...
        record(
          label,
          key !== undefined ? clipWords(key.split("\n")[0]!, TITLE_MAX_KEY_CHARS) : undefined,
        );
      } else {
  ...
  for (const [verb, targets] of groups) {
    ...
    if (first !== undefined) {
      segment = `${verb} ${first}`;
      if (targets.length > 1) {
        segment += targets.every((target) => target === first)
          ? ` ×${targets.length}`
          : ` +${targets.length - 1}`;
      }
