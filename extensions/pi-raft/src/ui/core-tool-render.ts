...
import type { RaftRenderAudit } from "./raft-render.js";
import {
...
export const isCoreToolAudit = (audit: RaftRenderAudit): boolean =>
...
const argString = (audit: RaftRenderAudit, key: string): string | undefined =>
...
const normalizedResult = (audit: RaftRenderAudit): unknown => {
...
const resultOutput = (audit: RaftRenderAudit): string | undefined => {
...
const resultDetails = (audit: RaftRenderAudit): Record<string, unknown> | undefined => {
  const preview = recordOf(audit.preview);
...
const nativeTruncated = (audit: RaftRenderAudit): boolean => {
...
const toolLimit = (audit: RaftRenderAudit, options: CoreToolRenderOptions): number => {
...
const previewEntries = <T>(
  lines: T[],
  limit: number,
): { entries: PreviewEntry<T>[]; hidden: number } => {
...
const warningLine = (
  ...
  return warnings.length > 0
    ? theme.fg(
        "warning",
        `⚠ Preview ${countLabel(warnings.length, "warning")}: possible ${warnings.join(", ")}`,
      )
    : undefined;
};
...
const highlightRemovedDiffLines = (
  ...
  for (let offset = 0; offset < block.length; offset++) {
    ...
    if (line.kind === "-")
      removed.push({ row: start + offset, n, content: expandTabs(line.content) });
    else if (line.kind === " " && removed.length === 0)
      contextBefore.push({ n, content: expandTabs(line.content) });
  }
...
const renderDiff = (
  ...
  const language =
    options.settings.syntaxHighlighting && !skipHighlight ? resolveLanguage(filePath) : undefined;
  const highlighted: Array<string | undefined> = Array.from(
    { length: shown.length },
    () => undefined,
  );
  if (language) {
  ...
  const lines = shown.map((raw, index) => {
    ...
    let content = truncationNotice
      ...
      : (highlighted[index] ??
        theme.fg("toolOutput", escapeControlChars(expandTabs(line.content)) || " "));
    ...
    if (line.kind === "-") {
      return markDiffLine(
        "remove",
        `${theme.fg("toolDiffRemoved", `-${lineNumber} │ `)}${content}`,
      );
    }
...
const renderWrite = (
  ...
  if (audit.success === true && beforeRecord?.kind === "skipped") {
    ...
    return {
      lines: [
        theme.fg("success", "✓ Write applied") + theme.fg("muted", ` · diff skipped: ${reason}`),
      ],
      hidden: 0,
    };
  }
  if (
    audit.success === true &&
    recordOf(audit.preview)?.writeBeforeCaptured === true &&
    before === undefined
  ) {
    const rendered = renderContent(content, filePath, theme, options, {
...
const editOperations = (audit: RaftRenderAudit): Array<{ oldText: string; newText: string }> => {
...
const renderEdit = (
  ...
  if (operations.length > maxOperations) {
    pushArcItem(
      sections,
      arcItem(theme, `Showing ${maxOperations} of ${operations.length} edit blocks`),
    );
  }
...
const renderPathList = (
  ...
  const enabled =
    audit.tool === "find" ? options.settings.findResultPreview : options.settings.lsResultPreview;
  if (!options.expanded && !enabled) return null;
  const output = resultOutput(audit)?.replace(/\r?\n$/, "") ?? "";
  const emptyMarker =
    audit.tool === "find" ? "No files found matching pattern" : "(empty directory)";
  if (!output || output === emptyMarker) {
    return {
      lines: [
        theme.fg("muted", audit.tool === "find" ? output || "No files found" : "Empty directory"),
      ],
      hidden: 0,
    };
  }
...
const renderBash = (
  ...
  const commandName = audit.tool === "bash" ? firstShellCommandName(bashCommand(audit)) : "";
  const resultEnabled =
  ...
  if (!output || output === "(no output)") {
    ...
    return { lines: [...lines, theme.fg("muted", output || "No output")], hidden: 0 };
  }
  const raw = output.split("\n");
  const selected = previewEntries(raw, toolLimit(audit, options));
  ...
  for (const entry of selected.entries) {
    ...
    const text = theme.fg(
      audit.success === false ? "error" : "muted",
      escapeControlChars(entry.line) || " ",
    );
    lines.push(text);
  }
  ...
  if (fullOutputPath)
    pushArcItem(lines, arcItem(theme, `Full output: ${escapeControlChars(fullOutputPath)}`));
  return { lines, hidden: selected.hidden };
};
...
export const coreToolPreviewEnabled = (
  ...
  switch (audit.tool) {
    ...
    case "powershell": {
      ...
      const command = audit.tool === "bash" ? firstShellCommandName(bashCommand(audit)) : "";
...
export const coreToolTitle = (
  ...
  if (audit.tool === "bash" || audit.tool === "powershell") {
    ...
    const highlighted = firstLine
      ? highlightCode(firstLine, language, options.invalidate)?.[0]
      : undefined;
    const timeout = numberOf(audit.args?.timeout);
    const warnings =
      audit.tool === "bash" && options.settings.bashWarnings ? bashWarnings(command) : [];
    const prompt = audit.tool === "powershell" ? "PS>" : "$";
    return `${title} ${theme.fg("dim", prompt)} ${highlighted ?? theme.fg("accent", escapeControlChars(firstLine))}${metadata(
      theme,
      [
  ...
  if (audit.tool === "write") {
    ...
    const lineCount =
      numberOf(rendererPreview?.writeLineCount) ??
