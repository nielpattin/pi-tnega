...
export const resolveShikiTheme = (preference: string, variant: ShikiThemeVariant): string => {
  const parsed = parseShikiThemePreference(preference);
  return variant === "light" ? parsed.lightTheme : parsed.darkTheme;
};

const TOOLS: CodePreviewToolName[] = [
  "bash",
  "powershell",
  "read",
  "write",
...
/** Environment-backed defaults; raft.json "codePreview" layers override these. */
export const defaultCodePreviewSettings = (): CodePreviewSettings => ({
  ...
  editCollapsedLines:
    process.env.CODE_PREVIEW_EDIT_LINES === "all"
      ? "all"
      : positiveEnv("CODE_PREVIEW_EDIT_LINES", 160),
...
/**
 * Validate a raft.json "codePreview" section on top of the environment-backed
...
export const normalizeCodePreviewSettings = (raw: unknown): CodePreviewSettings => {
  ...
  for (const [key, fallback] of Object.entries(settings) as [
