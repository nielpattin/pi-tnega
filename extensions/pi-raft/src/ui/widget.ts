...
import type { RaftUiWidgetMode } from "../config.js";
import { spinnerFrame } from "./spinner.js";
import type { RaftActivityRun, RaftActivityStatus } from "../activity/types.js";
...
const phaseProgress = (
  ...
  const statuses: RaftActivityStatus[] = [
...
const totalTokens = (snapshot: RaftDashboardSnapshot, run: RaftActivityRun | undefined): number =>
  snapshot.agents
...
const totalCost = (snapshot: RaftDashboardSnapshot, run: RaftActivityRun | undefined): number =>
  snapshot.agents
    .filter((agent) => (run ? agent.runId === run.id : isActiveStatus(agent.status)))
    .reduce((sum, agent) => sum + (agent.usage?.cost ?? 0), 0);

const agentLines = (theme: Theme, agent: RaftUiAgent, now: number): string[] => {
...
export const shouldShowRaftWidget = (
  snapshot: RaftDashboardSnapshot,
  mode: RaftUiWidgetMode,
): boolean => {
...
export class RaftWidget implements Component {
  constructor(
  ...
  #lastSnapshot: RaftDashboardSnapshot | undefined;
  #lastLines: string[] | undefined;
  #pending: { width: number; snapshot: RaftDashboardSnapshot; lines: string[] } | undefined;
  ...
  #renderLines(snapshot: RaftDashboardSnapshot, width: number): string[] {
    return this.#boundContent(this.#buildContent(snapshot), width);
  }

  #buildContent(snapshot: RaftDashboardSnapshot): string[] {
    const candidateRun = snapshot.runs[0];
    const candidateFinishedAt = candidateRun?.finishedAt ?? candidateRun?.updatedAt ?? 0;
    const run =
      candidateRun &&
      (candidateRun.status === "running" || candidateFinishedAt > (snapshot.widgetDismissedAt ?? 0))
        ? candidateRun
        : undefined;
    ...
    const terminalAgents = run
      ...
      : [];
    const nestedCalls = run?.calls.filter((call) => call.kind !== "agent") ?? [];
    const title = run?.name ?? "Raft session";
    ...
    const header = `${glyph} ${this.theme.fg("accent", "Raft")} ${this.theme.fg(
      "muted",
      safeText(title),
