...
export interface RaftModelCandidate {
  provider: string;
  id: string;
  name?: string;
}

export type RaftModelResolution =
  | { kind: "resolved"; model: RaftModelCandidate; via?: string }
  | { kind: "already-active"; model: RaftModelCandidate }
  | { kind: "ambiguous"; query: string; candidates: RaftModelCandidate[] }
  | { kind: "not-found"; query: string; tried?: string[] };

const modelKey = (model: RaftModelCandidate): string => `${model.provider}/${model.id}`;

const sameModel = (left: RaftModelCandidate, right: RaftModelCandidate | undefined): boolean =>
  right !== undefined &&
...
const levenshteinRatio = (left: string, right: string): number => {
  ...
  for (let i = 0; i < left.length; i += 1) {
    ...
    for (let j = 0; j < right.length; j += 1) {
      current.push(
        Math.min(previous[j]! + 1, current[j]! + 1, previous[j]! + (left[i] === right[j] ? 0 : 1)),
      );
    }
    previous = current;
  }
 ...
 */
const closenessScore = (query: string, model: RaftModelCandidate): number => {
...
export type RaftModelUsage = Record<string, number>;
...
const pickClosestCandidate = (
  query: string,
  pool: readonly RaftModelCandidate[],
  lastUsed: RaftModelUsage | undefined,
): { model: RaftModelCandidate; via: "closest" | "recent" | "latest" } | undefined => {
  const usage: RaftModelUsage = {};
  for (const [key, value] of Object.entries(lastUsed ?? {})) {
    usage[key.toLowerCase()] = value;
  }
  const recency = (model: RaftModelCandidate): number =>
    usage[`${model.provider}/${model.id}`.toLowerCase()] ?? 0;
  let best: RaftModelCandidate | undefined;
  let bestScore = 0;
  let tied = false;
  for (const model of pool) {
    ...
    if (score === bestScore) {
      ...
      if (
        nextRecent > bestRecent ||
        (nextRecent === bestRecent && modelKey(model) > modelKey(best))
      ) {
        best = model;
      }
    }
  ...
  return { model: best, via: !tied ? "closest" : wonOnRecency ? "recent" : "latest" };
};
 ...
 */
export const resolveRaftModel = (
  query: string,
  options: {
    aliases: Record<string, string[]>;
    available: readonly RaftModelCandidate[];
    current?: RaftModelCandidate;
    provider?: string;
    lastUsed?: RaftModelUsage;
  },
): RaftModelResolution => {
  ...
  const exact =
    available.find((model) => modelKey(model).toLowerCase() === normalized) ??
...
const unavailablePiModelError = (
  query: string,
  resolution?: Extract<RaftModelResolution, { kind: "ambiguous" | "not-found" }>,
): Error => {
  const detail =
    resolution?.kind === "not-found" && resolution.tried !== undefined
 ...
 * aliases and inexact selectors retain the normal Raft resolution policy.
 */
export const resolveAvailablePiModel = (
  ...
  const resolution = resolveRaftModel(query, options);
