...
export interface RaftNestedToolResultProxy {
  proxy(request: RaftToolResultProxyRequest): Promise<unknown>;
}
...
export class RaftToolResultProxy implements RaftNestedToolResultProxy {
  constructor(readonly runner: () => ExtensionRunner | undefined) {}

  async proxy(request: RaftToolResultProxyRequest): Promise<unknown> {
    ...
    const details: RaftToolResultProxyDetailsV1 = {
      kind: RAFT_TOOL_RESULT_PROXY_KIND,
    ...
    if (patch.isError === true) {
      throw new Error(
        textFromContent(patchedContent).trim() ||
          `Raft result middleware marked ${request.action.ref} as failed.`,
      );
    }

    const patchedDetails = readRaftToolResultProxyDetailsV1(patch.details);
    if (
