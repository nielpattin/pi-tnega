...
const CLASSIFIER_SYSTEM_PROMPT = `You are the Pi Raft auto-permission safety classifier. Decide whether one exact tool action may run without human approval.
...
const classifierTool = {
  ...
  description: "Report whether the exact Raft action may run without human approval",
  parameters: Type.Object(
    { decision: Type.String({ enum: ["allow", "escalate"] }), reason: Type.String() },
    { additionalProperties: false },
  ),
};

export interface RaftAutoApprovalDecision {
...
const transcriptEvidence = (context: ExtensionContext): string => {
  ...
  for (const entry of branch) {
    ...
    const calls = record.content.flatMap((part) => {
      ...
      return [
        { name: typeof call.name === "string" ? call.name : "unknown", arguments: call.arguments },
      ];
    });
...
const loadCompleteSimple = (): Promise<CompleteSimpleFn> => {
  completeSimpleLoader ??= import("@earendil-works/pi-ai/compat").then(
    (module) => module.completeSimple,
  );
...
const configuredModel = (context: ExtensionContext, modelKey?: string) => {
  ...
  return context.modelRegistry.find(modelKey.slice(0, separator), modelKey.slice(separator + 1));
};

export class RaftAutoApprovalClassifier {
  async classify(
    action: ResolvedRaftAction,
    args: Record<string, unknown>,
    context: ExtensionContext,
    modelKey?: string,
  ): Promise<RaftAutoApprovalDecision> {
