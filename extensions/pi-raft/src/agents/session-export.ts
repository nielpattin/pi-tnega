...
import type { RaftAgentConfig } from "../config.js";
 ...
 *
 *   <root>/sessions/.raft/<encoded-cwd>/<timestamp>_<runId>.jsonl
 *
 ...
 * export under the ".raft" namespace inside pi's store makes trackers count
 * subagent usage with zero configuration while pi itself never lists these
 * files. Root resolves as:
 *
 *   PI_RAFT_AGENT_DIR env  >  agents.sessionExportDir  >  ~/.pi/agent
 *
 * Prefer an isolated store instead? Set agents.sessionExportDir to
 * ~/.pi-raft/agent and register it as a ccusage pi.stores named store.
 */

export const SESSION_EXPORT_ENV = "PI_RAFT_AGENT_DIR";
 ...
 * pi's encoding keeps raft sessions visually consistent with native ones.
...
export const resolveSessionExportDir = (config: RaftAgentConfig): string | undefined => {
...
/**
 * Final JSONL path for one run: `<root>/sessions/.raft/<encoded-cwd>/<ts>_<runId>.jsonl`.
 * The ".raft" namespace is deliberate: tokscale (walkdir) and ccusage both
