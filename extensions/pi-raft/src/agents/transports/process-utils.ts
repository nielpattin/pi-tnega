...
const runtimeOverride = (env: NodeJS.ProcessEnv): string | undefined => {
  const value = env.PI_RAFT_NODE_BINARY;
...
const missingRuntimeError = (execPath: string, requireNode: boolean, requireBun = false): Error => {
  ...
  return new Error(
    `Raft requires ${required} to launch a JavaScript worker, but ` +
      `process.execPath is ${execPath} ${shape} and PI_RAFT_NODE_BINARY is unset. ` +
      "Install Node.js or Bun, or set PI_RAFT_NODE_BINARY to the runtime binary.",
  );
};
...
// to PI_RAFT_NODE_BINARY, then the first node/bun on PATH.
const resolveScriptRuntimeUncached = async (
...
export const spawnDetached = async (
  ...
  if (!child.pid) throw new Error("Failed to launch Raft worker process");
  const pid = child.pid;
  child.unref();
  return {
    pid,
