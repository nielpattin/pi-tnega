...
import { raftWorktreePath } from "./worktree-paths.js";
...
export class WorktreeManager {
  ...
  async create(
    ...
    const branch = `pi-raft/${safeLabel(name)}-${id.slice(0, 8)}`;
    const worktreePath = raftWorktreePath(gitRoot, id);
    ...
    if (prefix && prefix.length > 0) {
      ...
      try {
        ...
        if (
          ...
          isInside(canonicalWorktreePath, canonicalCandidate)
        ) {
