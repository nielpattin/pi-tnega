...
/**
 * Cross-process cost budget ledger for a Raft recursion tree.
 *
 ...
 * ledger path and budget travel to descendants through PI_RAFT_BUDGET*
...
const ENV_BUDGET = "PI_RAFT_BUDGET";
const ENV_BUDGET_FILE = "PI_RAFT_BUDGET_FILE";
const ENV_BUDGET_ID = "PI_RAFT_BUDGET_ID";
...
export function initBudgetLedger(budget: number): BudgetLedgerState {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "pi-raft-budget-"));
...
export function readBudgetLedgerDetailed(file: string): BudgetLedgerDetail {
  const detail: BudgetLedgerDetail = { cost: 0, tokens: 0, byRunner: {}, byActor: {}, entries: [] };
  let raw: string;
  try {
    raw = fs.readFileSync(file, "utf8");
  } catch {
