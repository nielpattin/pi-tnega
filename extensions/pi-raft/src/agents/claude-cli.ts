...
export const mapClaudeTools = (tools: readonly string[]): string[] => {
  ...
  for (const tool of tools) {
    const claudeTool = Object.hasOwn(CLAUDE_TOOL_NAMES, tool) ? CLAUDE_TOOL_NAMES[tool] : undefined;
    if (!claudeTool) {
      throw new Error(
        `Claude runner does not support Raft tool ${JSON.stringify(tool)}. Supported tools: ${Object.keys(
          CLAUDE_TOOL_NAMES,
        ).join(", ")}`,
      );
    }
...
export const claudeEffort = (thinking: RaftThinking): string =>
...
export const discoverClaudeModels = async (
  ...
  new Promise<ClaudeModelInfo[]>((resolve, reject) => {
    const requestId = `raft-models-${randomUUID()}`;
