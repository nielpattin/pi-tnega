...
type RemoveFn = (target: string, options: { recursive: boolean; force: boolean }) => Promise<void>;
...
export const removeTree = async (target: string, rm: RemoveFn = defaultRemove): Promise<void> => {
  for (let attempt = 0; attempt < RM_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (attempt < RM_MAX_ATTEMPTS - 1 && code !== undefined && RETRYABLE_RM_CODES.has(code)) {
