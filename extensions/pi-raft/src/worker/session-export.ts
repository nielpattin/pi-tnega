 ...
 * ccusage). Raft subagents run with `--no-session`, so nothing they spend is
 ...
 * a minimal pi-format session file under `~/.pi-raft/agent/sessions/`
 * (or PI_RAFT_AGENT_DIR): a `session` header, a `session_info` marker naming
 * the run as "raftagent-<name>", and `message` entries carrying only
...
export const RAFT_AGENT_MARKER = "raftagent-";
...
export class SessionExporter {
  ...
  push(
    ...
    const header = { type: "session", version: 3, id: this.#sessionId, timestamp, cwd: this.#cwd };
    ...
    const info = {
      ...
      parentId: null,
      timestamp,
      name: `${RAFT_AGENT_MARKER}${this.#agentName}`,
    };
