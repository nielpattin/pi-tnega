...
import {
  ...
  type RaftBranchFactV2,
} from "../compaction/branch-details.js";
import { readRaftProjectionTrace } from "../compaction/trace-events.js";
...
export interface NormalizedEntry {
  ...
  operation?: NormalizedRaftOperation;
  branchFact?: NormalizedRaftBranchFact;
  factAddress?: string;
...
interface NormalizedRaftOperation {
  address: string;
...
interface NormalizedRaftBranchFact {
  kind: RaftBranchFactV2["kind"];
  address: string;
  entryId: string;
  subordinal: string;
  carrierEntryId: string;
...
const extractFilesTouched = (raw: Record<string, unknown>): string[] => {
  ...
  if (!message || asString(message.role) !== "assistant" || !Array.isArray(message.content))
    return [];
  const paths: string[] = [];
...
const boundedTraceOperation = (
  parentEntryId: string,
  operation: NonNullable<ReturnType<typeof readRaftProjectionTrace>>["operations"][number],
  includeToolOutput: boolean,
): NormalizedRaftOperation => {
  const address = `${parentEntryId}/${operation.sequence}`;
  const normalized: NormalizedRaftOperation = {
    address,
    parentEntryId,
  ...
  if (
    Buffer.byteLength(JSON.stringify(normalized), "utf8") > TRACE_OPERATION_MAX_BYTES &&
...
const traceChildren = (
  ...
  if (base.type !== "message" || base.role !== "toolResult" || base.toolName !== "raft_exec")
    return [];
  const message = raw.message;
  if (!message || typeof message !== "object" || Array.isArray(message)) return [];
  const nested = readRaftProjectionTrace((message as Record<string, unknown>).details);
  ...
  return nested.operations.map((operation) => {
    ...
    const text = `Raft operation ${normalized.ref}\n${JSON.stringify(normalized)}`;
    return {
...
const branchDetails = (raw: Record<string, unknown>) => {
  ...
  if (
    type !== "branch_summary" &&
    !(type === "message" && asString(messageRecord?.role) === "branchSummary")
  ) {
    return undefined;
  }
  return (
    readRaftBranchSummaryDetails(raw.details) ??
    readRaftBranchSummaryDetails(messageRecord?.details)
  );
};

const branchFilesTouched = (fact: Extract<RaftBranchFactV2, { kind: "operation" }>): string[] => {
  const paths: string[] = [];
...
const branchFactChild = (
  ...
  const normalizedFact: NormalizedRaftBranchFact = {
    kind: fact.kind,
    ...
    ...(fact.kind === "customMessage"
      ? { text: fact.text, customType: fact.customType, display: fact.display }
      : {}),
  ...
  if (fact.kind === "phase") {
    return {
      ...
      role: "raftPhase",
      toolName: null,
      text: `Raft phase ${fact.phase}`,
      isError: false,
      branchFact: normalizedFact,
    };
  }
  if (fact.kind === "raftRun") {
    const description = fact.description ? ` — ${fact.description}` : "";
    return {
      ...common,
      role: "raftRun",
      toolName: "raft_exec",
      text: `Raft run ${fact.name}${description} → ${fact.outcome}`,
  ...
  const operation: NormalizedRaftOperation = {
    address: fact.address,
  ...
  if (
    Buffer.byteLength(JSON.stringify(operation), "utf8") > TRACE_OPERATION_MAX_BYTES &&
  ...
  return {
    ...
    role: "raftOperation",
    toolName: fact.tool,
    text: `Raft branch operation ${fact.ref}\n${JSON.stringify(operation)}`,
    isError: fact.outcome !== "succeeded",
    operationAddress: fact.address,
    ref: fact.ref,
    ...(fact.provider ? { provider: fact.provider } : {}),
  ...
  if (type === "message") {
    ...
    if (role === "toolResult") {
      ...
      const prefix = toolName
        ? `toolResult(${toolName})${errorSuffix}`
        : `toolResult${errorSuffix}`;
...
export const normalizeRecords = (
  ...
  for (const value of records) {
    ...
    if (
      options.lineage &&
      options.lineage.entryOrdinals !== null &&
      !options.lineage.entryOrdinals.has(ordinal)
    ) {
      continue;
    }
    if (
      ...
      if (duplicateAddress) {
        ...
        const carrierFromId =
          typeof raw.fromId === "string"
            ? raw.fromId
            : typeof messageRecord?.fromId === "string"
              ? messageRecord.fromId
              : null;
  ...
  if (header === null && (identity.sessionId !== undefined || identity.cwd !== undefined)) {
    header = { sessionId: identity.sessionId ?? "", cwd: identity.cwd ?? "" };
    sessionId = sessionId || identity.sessionId || "";
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.operationAddress !== undefined) {
      if (operationAddresses.has(entry.operationAddress))
        reasons.add("duplicate_operation_address");
...
export const normalizeSession = (
  ...
  try {
    ...
    return {
      entries: [],
      header: null,
      indexCoverage: { complete: false, reasons: ["source_unavailable"] },
    };
  }
...
export const expandSessionEntriesChecked = (
  ...
  const entries =
    typeof sessionFileOrEntries === "string"
