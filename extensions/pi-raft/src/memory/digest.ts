...
/** Compact structural posting: source identity, typed role/tool fields, time, and exact Raft capability identity. */
  ...
  for (const entry of input.entries) {
    ...
    if (!vocabularyLimitReached) {
      for (const term of tokenizeLexical(entry.text)) {
        ...
        const termBytes =
          Buffer.byteLength(JSON.stringify(term), "utf8") + (vocabulary.size === 0 ? 0 : 1);
