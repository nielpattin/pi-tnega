...
export const digestPathForSession = (
  ...
  path.join(indexDir, `${cacheBaseName(sessionFile)}${cacheModeSuffix(branches)}.digest.json`);
...
const applyCacheMetrics = <T extends CacheRecord>(value: T): T => {
  ...
  value.cacheSourceRatio =
    value.size === 0 ? 0 : Number((value.cacheBytes / value.size).toFixed(6));
  return value;
};
...
const isCacheFresh = (cache: CacheRecord | null, state: SourceState, policy: string): boolean =>
  cache !== null &&
...
const maxFittingPrefix = <T>(values: T[], fits: (candidate: T[]) => boolean): T[] => {
  let low = 0;
  ...
  try {
    while (true) {
      ...
      try {
        ...
        const source =
          kind === "shard" && typeof parsed.sessionFile === "string"
            ? parsed.sessionFile
            : kind === "digest" && typeof parsed.file === "string"
              ? parsed.file
              : null;
        const branches =
          parsed.branches === "active" || parsed.branches === "all" ? parsed.branches : null;
        if (branches !== null && branches !== branchesToClean) continue;
        const expected =
          source && branches && kind === "shard"
        ...
        const structurallyValid =
          kind === "shard"
            ...
            : kind === "digest"
              ? readDigestFile(cacheFile, cacheBytes) !== null
              : false;
        if (
          parsed.cacheVersion !== MEMORY_CACHE_VERSION ||
          source === null ||
  ...
  for (const ref of refs) {
    ...
    try {
      if (hydrate) {
        ...
        removeCacheFile(
          digestPathForSession(ref.file, options.indexDir, options.branches ?? "active"),
        );
        shards.push(shard);
        if (shard.sourceHash) indexedSessions += 1;
        else reasons.add(shard.indexReason ?? "source_unavailable");
        ...
        removeCacheFile(
          shardPathForSession(ref.file, options.indexDir, options.branches ?? "active"),
        );
        digests.push(digest);
...
export const bm25Score = (
  ...
  const lengths = matching.map((item) =>
    Math.max(
      1,
      [...item.counts.values()].reduce((a, b) => a + b, 0),
    ),
  );
  ...
  for (let index = 0; index < matching.length; index += 1) {
    ...
    for (const term of queryTerms) {
      ...
      const normalized =
        (tf * (K + 1)) / (tf + K * (1 - B + B * (lengths[index]! / averageLength)));
