...
function detectExtensions(pi: ExtensionAPI): ExtEntry[] {
  ...
  // From the API
  try {
    for (const t of pi.getAllTools()) add(srcPath(t.sourceInfo));
  } catch {
    /* nop */
  }
  try {
...
async function spawnPi(args: string[], timeoutMs: number): Promise<ProcResult> {
  return new Promise((resolve) => {
    ...
    const proc = spawn(cmd, cmdArgs, { env, shell: !directOk, stdio: ["ignore", "pipe", "pipe"] });
    let stderr = "";
    const timer = setTimeout(() => {
      proc.kill();
      resolve({ ms: 0, error: "timeout" });
    }, timeoutMs);
    proc.on("error", (err) => {
      clearTimeout(timer);
      resolve({ ms: 0, error: String(err.message ?? err) });
    });
...
async function measureOne(
  ext: ExtEntry,
  timeoutMs: number,
): Promise<{ label: string; ms: number; error?: string }> {
...
class ReportViewer implements Component {
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, Key.down) && this.scrollOffset < max) {
      this.scrollOffset++;
      changed = true;
    }
...
function buildOutput(
  ...
  lines.push(
    `  ${items.length} extensions  |  ${ok} ok, ${failed} failed  |  ${elapsedSec.toFixed(1)}s`,
  );
  lines.push("");
    ...
    try {
      ...
      ctx.ui
        .custom<string | null>(
          ...
          {
            overlay: true,
            overlayOptions: { width: "78%", minWidth: 44, maxHeight: "70%", anchor: "center" },
          },
        )
        .catch(() => {});
    } catch (err) {
