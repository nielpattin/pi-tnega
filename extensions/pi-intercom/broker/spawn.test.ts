...
test("getWindowsBrokerCommandLine falls back to configured broker command when local tsx is missing", () => {
  const commandLine = getWindowsBrokerCommandLine(
    "C:/repo/broker.ts",
    "C:/repo",
    "C:/Program Files/nodejs/node.exe",
  ...
  assert.equal(commandLine, `"npx" "--no-install" "tsx" "C:/repo/broker.ts"`);
});

test("getWindowsBrokerCommandLine resolves npx beside node when available", () => {
  const commandLine = getWindowsBrokerCommandLine(
    "C:/repo/broker.ts",
    "C:/repo",
    "C:/Program Files/nodejs/node.exe",
    "npx",
    ["--no-install", "tsx"],
    (candidate) => candidate.endsWith(path.join("nodejs", "npx.cmd")),
  );
  assert.equal(
    commandLine,
    `"${path.join("C:/Program Files/nodejs", "npx.cmd")}" "--no-install" "tsx" "C:/repo/broker.ts"`,
  );
});
