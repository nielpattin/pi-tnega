...
function usesDefaultBrokerCommand(brokerCommand: string, brokerArgs: string[]): boolean {
   return (
      brokerCommand === "npx" && brokerArgs.length === 2 && brokerArgs[0] === "--no-install" && brokerArgs[1] === "tsx"
   );
}
...
function resolveWindowsBrokerCommand(
   ...
   const npxCmdPath = join(dirname(nodePath), "npx.cmd");
   return fileExists(npxCmdPath) ? npxCmdPath : brokerCommand;
}
...
export function getWindowsBrokerCommandLine(
   ...
   if (usesDefaultBrokerCommand(brokerCommand, brokerArgs) && fileExists(tsxCliPath)) {
      return [quoteWindowsArg(nodePath), quoteWindowsArg(tsxCliPath), quoteWindowsArg(brokerPath)].join(" ");
   }

   const resolvedBrokerCommand = resolveWindowsBrokerCommand(brokerCommand, nodePath, fileExists);
   return [
