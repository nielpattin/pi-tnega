...
`ask` sends the message and blocks until the recipient responds. The reply comes back as the tool result, so the agent continues in the same turn with full context. No confirmation dialog — if you're asking and waiting, the intent is clear.
...
| ------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `need_decision`     | Sends an ask and blocks until the supervisor replies               | The subagent is blocked, uncertain, needs approval, or faces a product/API/scope decision |
...
**`need_decision`** — Sends a formatted ask to the supervisor and blocks until it replies. The reply comes back as the tool result. Includes run metadata in the message so the supervisor knows which subagent is asking.
...
**`ask`** — Sends a message and waits for the recipient to reply. The reply is returned as the tool result. No confirmation dialog. Only one pending `ask` is allowed per session at a time. Use this when the agent needs the answer to continue working.
