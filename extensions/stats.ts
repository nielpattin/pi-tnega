...
function filterUsage(dataset: UsageDataset, filters: UsageFilters = {}): UsageScanResult {
  const filteredEntries = dataset.entries.filter((entry) => {
    if (filters.providers && !filters.providers.includes(String(entry.message?.provider)))
      return false;
...
class MultiSelectModal implements Component {
  ...
  constructor(
    ...
    private readonly theme: {
      ...
      fg: (name: "accent", text: string) => string;
    },
  ) {
...
async function pickMany(
  ...
  return await ctx.ui.custom<string[] | undefined>(
    ...
    {
      overlay: true,
      overlayOptions: { width: "60%", minWidth: 60, maxHeight: "80%", anchor: "center", margin: 2 },
    },
  );
}
...
class StatsModal implements Component {
  ...
  render(width: number): string[] {
    ...
    const visibleRows = rows
      .slice(this.scroll, this.scroll + 30)
      .map((row) =>
        this.renderRow({
          date: row.date,
  ...
  pi.registerCommand("stats", {
    ...
    handler: async (_args, ctx) => {
      ...
      const providerFilter =
        selectedProviders.length === allUsage.providers.length ? undefined : selectedProviders;
      ...
      const modelFilter =
        selectedModels.length === providerUsage.models.length ? undefined : selectedModels;
