 ...
 * - Agent profiles with [built-in] / (override) tags.
 *
 * Detail & Edit Screen:
...
type AgentPanelRuntime = unknown;

function runTool<T>(_runtime: AgentPanelRuntime, value: T | PromiseLike<T>): Promise<T> {
...
const deleteAgentFromDisk = (agent: AgentDefinition | string, cwd?: string) =>
  deleteAgentProfile(agent, { cwd });
...
/** Agent profiles do not reserve a completion tool. */
export const PROFILE_LOCKED_TOOLS = new Set<string>();
/** Child agents must not spawn nested agents or question the user. */
...
export function createAgentsPanelState(initial?: Partial<AgentsPanelState>): AgentsPanelState {
  return { selectedIndex: 0, viewMode: "list", detailFieldIndex: 0, isOpen: true, ...initial };
}

export function detailFieldsFor(_agent?: AgentDefinition): readonly DetailField[] {
  return AGENT_FIELDS;
}
...
export function reduceAgentsPanelKey(
  ...
  const fields = detailFieldsFor(selected);
  ...
  if (state.viewMode === "detail") {
    ...
    if (key === "up" || key === "k") {
      ...
      return { state: { ...state, detailFieldIndex: nextIdx }, intent: { type: "none" } };
    }
    if (key === "down" || key === "j") {
    ...
    if (key === " " || key === "space" || key === "enter") {
      if (field === "name")
        return {
          ...
          intent: { type: "edit_name" },
        };
      if (field === "enabled")
        return {
          state: { ...state, detailFieldIndex: safeFieldIndex },
          intent: { type: "toggle_enabled" },
        };
      if (field === "model")
        return {
          state: { ...state, detailFieldIndex: safeFieldIndex },
      ...
      if (field === "description")
        return {
          state: { ...state, detailFieldIndex: safeFieldIndex },
          intent: { type: "edit_description" },
        };
      if (field === "body")
        return {
          state: { ...state, detailFieldIndex: safeFieldIndex },
          intent: { type: "edit_body" },
        };
...
export class FullScreenAgentsManager implements Component, Focusable {
  ...
  constructor(
    ...
    private runtime: AgentPanelRuntime,
    ...
    this.createNameInput.onSubmit = (val) => {
      ...
      if (!clean) {
        this.statusMessage =
          "Invalid agent profile name. Use lowercase alphanumeric, dash, or underscore.";
    ...
    this.createIntentInput.onSubmit = async (val) => {
      ...
      this.statusMessage = "Generating agent profile draft with model...";
      this.tui.requestRender();
  ...
  private async generateAndOpenDraft() {
    ...
    if (this.ctx?.model) {
      try {
        const systemPrompt =
          "You are an expert agent profile designer. Return a JSON object with description, guidance, and body.";
        const userPrompt = `Create agent profile definition for: Name="${this.newName}", Intent="${this.newIntent}". Return JSON ONLY.`;
  ...
  private getAvailableTools(): AgentToolInfo[] {
    ...
    const addTool = (
      ...
      if (!existing) {
        ...
        if (!existing.description && typeof description === "string")
          existing.description = description;
        if (!existing.promptSnippet && typeof promptSnippet === "string")
          existing.promptSnippet = promptSnippet;
    ...
    // 1. Discover tools dynamically from the panel runtime, ExtensionCommandContext and registries
    ...
    const BASELINE_TOOLS: Array<{
      ...
      {
        name: "ask_user",
        description: "Ask the parent user a question. Disabled inside child agents.",
        promptSnippet: "Interactive question",
      },
      {
        name: "agent_spawn",
        description: "Spawn child agents. Disabled inside child agents.",
        promptSnippet: "Agent delegation",
      },
      {
        name: "agent_list",
        description: "List child agents. Disabled inside child agents.",
        promptSnippet: "List agents",
      },
      {
        name: "agent_cancel",
        description: "Cancel child agents. Disabled inside child agents.",
        promptSnippet: "Cancel agents",
      },
      {
        name: "web_search",
        description:
          "Search the web using multi-engine routing with domain, freshness, and category targeting.",
      ...
      try {
        ...
        for (const m of registered) {
          if (typeof m === "string") {
            ...
            if (provider && id) {
              const fullId = String(id).startsWith(`${provider}/`)
                ? String(id)
                : `${provider}/${id}`;
  ...
  private openPicker(picker: "model" | "thinking" | "tools") {
    ...
    if (picker === "model") this.viewState = "select_model";
      ...
      const baseSelected = (agent?.tools ? [...agent.tools] : []).filter(
        (t) => !DISABLED_NESTED_TOOLS.has(t),
      );
  ...
  private async commitTextEdit(val: string) {
    ...
    const agent = this.currentAgent();
    if (!agent) return;
    const fields = detailFieldsFor(agent);
  ...
  private async saveDetailChanges() {
    ...
    const updatedAgent: AgentDefinition = { ...agentToSave, filePath };
    ...
    const saveMessage = `Saved ${updatedAgent.name} to ${filePath}`;
  ...
  handleInput(data: string): void {
    ...
    const left =
      matchesKey(data, Key.left) || this.keybindings.matches(data, "tui.editor.cursorLeft");
    const right =
      matchesKey(data, Key.right) || this.keybindings.matches(data, "tui.editor.cursorRight");
    const cancel =
      matchesKey(data, Key.escape) || this.keybindings.matches(data, "tui.select.cancel");
    ...
    if (this.state.viewMode === "detail") {
      const isDirtyDetail = Boolean(
        this.initialAgentSnapshot &&
        JSON.stringify(this.currentAgent()) !== this.initialAgentSnapshot,
      );
    ...
    const fields = detailFieldsFor(this.currentAgent());
  ...
  private handlePickerInput(data: string) {
    ...
    if (this.viewState === "select_tools") {
      ...
      if (data === "a" && !q) {
        this.tempSelectedTools = filtered
          .map((t) => t.name)
          .filter((name) => !DISABLED_NESTED_TOOLS.has(name));
        for (const locked of PROFILE_LOCKED_TOOLS) {
    ...
    const options =
      this.viewState === "select_model" ? this.getAvailableModels() : REASONING_EFFORTS;
  ...
  render(width: number): string[] {
    ...
    if (this.viewState === "create_name") {
      return [
        theme.bold("Create New Agent Profile (Step 1/2)"),
        "Enter agent profile name/identifier (lowercase alphanumeric, dash, underscore):",
    ...
    if (this.viewState === "create_intent") {
      return [
        theme.bold(`Create New Agent Profile: "${this.newName}" (Step 2/2)`),
        "Describe what this agent should do:",
        ...this.createIntentInput.render(width),
      ];
    }
    if (this.viewState === "generating") {
      return [theme.fg("accent", "Generating agent profile draft with model...")];
    }
    ...
    const lines: string[] = [];
    const vm = this.viewModel;
    const headerLeft = theme.fg("accent", theme.bold("Agent Profiles"));
    ...
    const footerLines = [
      "",
      ...wrappedListFooter.map((l) => padLine(theme.fg("dim", `  ${l.trimStart()}`), width)),
    ];
  ...
  private renderPickerOverlay(width: number, targetRows: number): string[] {
    ...
    if (isTools) {
      ...
      if (totalBodyLines > maxDetailBodyHeight) {
        ...
        dividerStr =
          "─".repeat(Math.floor(padLen / 2)) +
          theme.fg("dim", pageStr) +
      ...
      const maxVis = Math.max(
        1,
        targetRows - HEADER_RESERVED - FOOTER_RESERVED - detailLines.length,
      );
      ...
      for (let idx = 0; idx < visibleItems.length; idx++) {
        ...
        const checked =
          !isDisabledNested && (isLockedProfile || this.tempSelectedTools.includes(tool.name));
      ...
      lines.push(
        theme.fg(
          "dim",
          "Space: toggle · a: select all · n: clear all · Enter: confirm · Esc: cancel",
        ),
      );
    } else {
  ...
  private renderBodyEditorOverlay(width: number, targetRows: number): string[] {
    ...
    if (this.pendingEscConfirm) {
      const escWarning = theme.bold(
        theme.fg("error", "  [Unsaved changes! Press Esc again to discard]"),
      );
    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      if (isCursorLine) {
        if (cursorColIndex < rLine.length) {
          ...
          styledLine =
            rLine.slice(0, cursorColIndex) +
            `\x1b[7m${charAt}\x1b[27m` +
            rLine.slice(cursorColIndex + 1);
        } else {
      ...
      for (let j = 0; j < lineArr.length; j++) {
        ...
        wrappedEditorLines.push({ lineNum: i + 1, text: lineArr[j] });
    ...
    if (this.pendingEscConfirm) {
      lines.push(
        padLine(
          theme.bold(theme.fg("error", "Unsaved changes! Press Esc again to exit without saving")),
          width,
        ),
      );
    } else {
      const navHint = theme.fg(
        "dim",
        "Type prompt text · Up/Down/Left/Right navigate · Enter newline · ",
      );
      const saveHint = isDirty
        ? theme.bold(theme.fg("accent", "Ctrl+S save *"))
        : theme.fg("dim", "Ctrl+S save");
  ...
  private renderListRows(width: number, height: number): string[] {
    ...
    if (list.length === 0) {
      out.push(theme.fg("dim", "  (no agent profiles available)"));
      return out;
    }
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const title = isSelected
        ...
        : theme.fg("text", agent.name);

      const tagStr = agentDisplayTags(agent)
        .map((tag) =>
          tag === "built-in" ? theme.fg("dim", "[built-in]") : theme.fg("warning", "(override)"),
        )
        .join(" ");

      const left = tagStr
        ? ` ${marker} ${statusStr} ${title} ${tagStr}`
        : ` ${marker} ${statusStr} ${title}`;
  ...
  private renderDetailScreen(width: number, targetRows: number): string[] {
    ...
    const agent = this.currentAgent();
    const isDirtyDetail = Boolean(
      this.initialAgentSnapshot && JSON.stringify(agent) !== this.initialAgentSnapshot,
    );

    const titleBase = `Editing Agent Profile: ${agent?.name ?? "?"}`;
    ...
    if (this.pendingDetailEscConfirm) {
      ...
      for (const wLine of wrapped) {
        ...
        if (idx !== -1) {
          ...
          const tokenStr = isDirtyDetail
            ? theme.bold(theme.fg("accent", token))
            : theme.fg("dim", token);
  ...
  private buildDetailFields(width: number, viewportBudget: number): string[] {
    ...
    const agent = this.currentAgent();
    if (!agent) {
      out.push(theme.fg("dim", "  (no agent profile selected)"));
      return out;
    }

    const fieldDefs: { field: AgentDetailField; label: string; val: string }[] = [
      { field: "name", label: "Name", val: agent.name },
      { field: "enabled", label: "Enabled", val: agent.enabled ? "on" : "off" },
      { field: "model", label: "Model", val: agent.model ?? "(inherit)" },
    ...
    for (let i = 0; i < fieldDefs.length; i++) {
      ...
      const labelStr = isSelected
        ? theme.fg("accent", theme.bold(labelPadded))
        : theme.fg("text", labelPadded);
    ...
    let startIdx = Math.max(
      0,
      Math.min(this.systemPromptScrollOffset, Math.max(0, wrappedLines.length - 1)),
    );
