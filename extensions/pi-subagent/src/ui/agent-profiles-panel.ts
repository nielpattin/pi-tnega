...
export type AgentDetailField = "name" | "enabled" | "model" | "thinking" | "tools" | "description" | "body";
...
export const DISABLED_NESTED_TOOLS = new Set(["ask_user", "agent_spawn", "agent_list", "agent_cancel"]);
...
export class FullScreenAgentsManager implements Component, Focusable {
   ...
   constructor(
      ...
      this.createNameInput.onSubmit = (val) => {
         const clean = val
            ...
            .replace(/[^a-z0-9_-]/g, "");
         if (!clean) {
            this.statusMessage = "Invalid agent profile name. Use lowercase alphanumeric, dash, or underscore.";
   ...
   private renderListRows(width: number, height: number): string[] {
      ...
      for (let i = 0; i < list.length; i++) {
         ...
         const left = tagStr ? ` ${marker} ${statusStr} ${title} ${tagStr}` : ` ${marker} ${statusStr} ${title}`;
