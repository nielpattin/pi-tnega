...
export function summarizeAsyncAgentStatus(tasks: ReadonlyArray<Task>): AsyncAgentStatusSummary {
  ...
  const settledTasks = visible.filter(
...
export function createAsyncAgentWidget(
  ...
  return (tui: unknown, theme: Theme) => {
    const requestRender = (tui as { requestRender?: (force?: boolean) => void } | null | undefined)
      ?.requestRender;
    let disposed = false;
    ...
    return {
      render(width: number): string[] {
        ...
        const inFlight = visibleTasks.filter(
          (task) => task.status === "running" || task.status === "pending",
        );
        const settled = visibleTasks.filter(
          (task) => task.status === "completed" || task.status === "failed",
        );
        ...
        inFlight.slice(0, flightCap).forEach((task, index) => {
          ...
          const visible =
            visibleWidth(agentLine) > budget
              ? truncateToWidth(agentLine, budget, "…", false)
              : agentLine;
        ...
        settled.slice(0, settledCap).forEach((task) => {
          ...
          const budget = Math.max(0, width - 2);
          const trimmed =
            visibleWidth(fullLine) > budget
              ? truncateToWidth(fullLine, budget, "…", false)
              : fullLine;
          if (trimmed.length > 0) lines.push(`  ${trimmed}`);
        });
        if (settledOverflow > 0)
          lines.push(theme.fg("dim", `  +${settledOverflow} more (/wr to expand)`));
