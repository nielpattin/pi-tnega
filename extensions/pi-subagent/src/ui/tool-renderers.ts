...
import type {
  AgentCancelToolParams,
  AgentListToolParams,
  AgentSpawnToolParams,
} from "../tools/agent.js";
...
export function renderAgentCall(
  args: Partial<AgentSpawnToolParams>,
  theme: Theme,
  context?: RenderContext,
): Component {
...
export function renderAgentResult(
  ...
  const agentRows =
    tasks.length > 0 ? tasks : stringValue(result.details.id) ? [result.details] : [];
  ...
  if (!allSettled) {
    if (isSpawnAcknowledgement) {
      ...
      if (options.expanded) {
        ...
        lines.push(
          theme.fg("muted", "(") +
            `${safeKeyHint("app.tools.expand", "to see schema")}${theme.fg("muted", ")")}`,
        );
      }
...
export function renderAgentCancelCall(
  ...
  return new Text(
    `${theme.fg("toolTitle", theme.bold("agent_cancel"))} ${theme.fg("accent", id)}`,
    0,
    0,
  );
}
...
export function createMarkdownTheme(theme: Theme): MarkdownTheme {
  ...
  return {
    ...
    strikethrough: (text: string) =>
      theme.strikethrough ? theme.strikethrough(text) : `\x1b[9m${text}\x1b[29m`,
    underline: (text: string) =>
      theme.underline ? theme.underline(text) : `\x1b[4m${text}\x1b[24m`,
  };
}
...
export function formatAgentStatLine(input: AgentStatLineInput, theme: Theme): string {
  const name = input.boldName
    ? theme.fg("accent", theme.bold(input.name))
    : theme.fg("accent", input.name);
...
export function renderAgentListResult(
  ...
  if (result.details.ok === false) {
    const message =
      stringValue(result.details.error) ?? (textContent(result).trim() || "agent_list failed");
    return new Text(theme.fg("error", `✗ ${message}`), 0, 0);
  }
  ...
  if (!options.expanded) {
    ...
    for (const task of visible) {
      ...
      const statLine = formatAgentStatLine(
        { status, name, profile, usage: task.usage as AgentStatLineInput["usage"] | undefined },
        theme,
      );
      lines.push(statLine);
...
export function renderAgentCancelResult(
  ...
  if (result.details.ok === false) {
    const message =
      stringValue(result.details.error) ?? (textContent(result).trim() || "agent_cancel failed");
    return new Text(theme.fg("error", `✗ ${message}`), 0, 0);
  }
