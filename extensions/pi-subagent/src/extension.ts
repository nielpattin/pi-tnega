...
import {
  ...
  ensureParentSessionReady,
  flushPendingWrites,
} from "./services/task-session.js";
...
function asTextResult(payload: unknown) {
  return { content: [{ type: "text" as const, text: fullResultText(payload) }], details: payload };
}
...
function isBatchTask(
  task: Task,
): task is Task & { readonly batchId: string; readonly batchSize: number } {
  return (
...
async function resolveAgentToolAugmentation(
  runtime: AgentsRuntime,
  cwd?: string,
): Promise<AgentToolAugmentation> {
 ...
 */
export function registerAgentsExtension(
  pi: ExtensionAPI,
  options?: AgentsExtensionOptions,
): RegistrationResult {
  ...
  const flushResults = () => {
    for (const group of resultDelivery.pendingGroups()) {
      if (isBatchTask(group[0])) {
        const deliverable = group.filter(
          (task) => task.status !== "cancelled" && !resultDelivery.isSent(task.id),
        );
        if (deliverable.length > 0) {
          try {
            ...
            void runTool(
              runtime,
              AgentManager.use((manager) =>
                manager.markResultsDelivered(deliverable.map((task) => task.id)),
              ),
            ).catch(() => {});
          } catch {
            continue;
  ...
  const queueSettledResult = (task: Task): void => {
    ...
    if (isBatchTask(task)) {
      void runTool(
          ...
          Effect.gen(function* () {
            ...
            for (const candidate of candidates) {
              if (candidate.runtimeOwned !== true) {
                yield* registry.updateStatus(candidate.id, candidate.status, {
                  runtimeOwned: true,
                });
              }
            }
        ...
        .then((tasks) => {
          if (switchingParent || !parentContext || task.ownerSessionId !== activeOwnerSessionId)
            return;
