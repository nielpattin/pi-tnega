...
const TaskIdSchema = Type.String({
  minLength: 1,
  description:
    "Task ID returned by agent_spawn (for example task-019c2f8e-7b3a-7c42-b9d1-6e8f4a2c1b90).",
});
...
export const AgentSpecSchema = Type.Object({
  ...
  profile: Type.String({
    minLength: 1,
    description:
      "Required enabled agent profile name listed in the current agent_spawn tool metadata.",
  }),
});
...
function createAgentProfileSchema(agentNames: readonly string[]) {
  ...
  return Type.Union(choices, { description: `Enabled agent profiles: ${agentNames.join(", ")}.` });
}
...
export function augmentAgentToolMetadata(
  ...
  if (enabledAgents.length === 0) {
    return { agentNames: [], descriptionAppendix: "" };
  }
...
export const handleAgentSpawn = Effect.fn("agent.handleSpawn")(function* (
  ...
  if (agents.length === 0) {
    return {
      ok: false,
      error: 'agent_spawn requires a non-empty "agents" array.',
...
export const handleAgentList = Effect.fn("agent.handleList")(function* (
  _params: AgentListToolParams,
) {
  const registry = yield* TaskRegistry;
  const tasks = yield* registry.list();

  return { ok: true, tasks: tasks.map(agentTaskView) };
});

export const handleAgentCancel = Effect.fn("agent.handleCancel")(function* (
  params: AgentCancelToolParams,
) {
  const registry = yield* TaskRegistry;
  const agentManager = yield* AgentManager;
