...
export function resolveAppendSystemPromptSource(
  cwd: string,
...
export function buildAgentLaunchScript(
  ...
  const environment: Record<string, string> = {
    ...
    PI_AGENT_ACTIVITY_FILE: resolve(
      request.activityFile ?? getAgentActivityFile(request.sessionFile),
    ),
...
function herdrAvailable(): boolean {
  return (
    process.env.HERDR_ENV === "1" && Boolean(process.env.HERDR_PANE_ID) && commandAvailable("herdr")
  );
}
...
function createHerdrPane(
  ...
  if (typeof paneId !== "string" || paneId.length === 0)
    throw new Error("Herdr did not return a agent pane id");
  try {
...
function runHerdrScript(paneId: string, scriptPath: string): void {
  execFileSync("herdr", ["pane", "run", paneId, "bash", scriptPath], {
    stdio: "ignore",
    timeout: 10_000,
  });
}
...
function currentHerdrTabPaneCount(): number | undefined {
  ...
  try {
    const paneOutput = execFileSync("herdr", ["pane", "get", parentPaneId], {
      encoding: "utf8",
      timeout: 5_000,
    });
    ...
    const tabOutput = execFileSync("herdr", ["tab", "get", tabId], {
      encoding: "utf8",
      timeout: 5_000,
    });
    const tab = parseJsonOutput(tabOutput);
    const paneCount = tab?.result?.tab?.pane_count;
    return typeof paneCount === "number" && Number.isFinite(paneCount) && paneCount >= 0
      ? paneCount
      : undefined;
  } catch {
    return undefined;
...
function moveHerdrPaneToNewTab(paneId: string, name: string): void {
  execFileSync(
    "herdr",
    ["pane", "move", paneId, "--new-tab", "--tab-label", `agents ${name}`, "--no-focus"],
...
export const defaultAgentHerdrOps: AgentHerdrOps = {
  ...
  createPane: (name, cwd, fromPaneId, direction) =>
    createHerdrPane(name, cwd, fromPaneId, direction),
...
export async function launchExternalAgent(
  ...
  const writeLaunchScript = () => {
    ...
    writeFileSync(
      scriptPath,
      buildAgentLaunchScript({ ...request, sessionFile, activityFile }, command),
      { mode: 0o700 },
    );
  };
  ...
  if (paneId === undefined) {
    if (herdrUsable) {
      try {
        paneId = ops.createPane(
          request.name,
          request.cwd,
          request.splitFromPaneId,
  ...
  const completion = waitForAgentCompletion(completionSignal.signal, {
    ...
    .then((result) =>
      outcomeFromCompletion(result, { ...request, sessionFile, activityFile }, paneId),
    )
    .catch((error) => {
  ...
  return { metadata: { ...metadata, paneId }, completion, abort, control, stopWatching };
}
