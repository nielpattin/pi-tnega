...
export type AgentActivityReadResult =
  ...
  | {
      readonly ok: false;
      readonly reason: "missing" | "invalid" | "wrong-id";
...
type MutableActivity = {
  -readonly [Key in keyof AgentActivityState]: AgentActivityState[Key];
};
...
function validateActivity(value: unknown, expectedId: string): AgentActivityReadResult {
  if (!isRecord(value))
    return { ok: false, reason: "invalid", error: "activity must be an object" };
  if (value.version !== 1)
    return { ok: false, reason: "invalid", error: "unsupported activity version" };
  if (value.runningChildId !== expectedId) return { ok: false, reason: "wrong-id" };
  if (
    typeof value.latestEvent !== "string" ||
    !EVENTS.has(value.latestEvent as AgentActivityEvent)
  ) {
  ...
  if (
    ["messageEventType", "toolCallId", "toolName"].some((key) => !validOptionalString(value[key]))
  ) {
...
export function readAgentActivityFile(
  activityFile: string,
  expectedRunningChildId: string,
): AgentActivityReadResult {
...
export function writeAgentActivityFile(activityFile: string, activity: AgentActivityState): void {
  ...
  const temporary = join(
    directory,
    `.${activity.runningChildId}.${process.pid}.${activity.sequence}.tmp`,
  );
  try {
