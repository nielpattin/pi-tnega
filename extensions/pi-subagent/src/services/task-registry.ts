...
import { AGENTS_TASK_MANIFEST_LIMITS, pruneTerminalTasksForRetention } from "./task-manifest.js";
...
export class TaskRegistry extends Context.Service<TaskRegistry, TaskRegistryShape>()(
  "agents/TaskRegistry",
) {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const prune = () => {
        const limit = AGENTS_TASK_MANIFEST_LIMITS.maxTrackedJobs;
        const retained = pruneTerminalTasksForRetention(
      ...
      const register = Effect.fn("TaskRegistry.register")(function* (taskInit) {
        ...
        const newTask: Task = { ...taskInit, status: "pending", createdAt: Date.now() };
      ...
      const onSettled = Effect.fn("TaskRegistry.onSettled")(function* (
      ...
      const onChange = Effect.fn("TaskRegistry.onChange")(function* (
        listener: (jobs: ReadonlyArray<Task>) => void,
      ) {
      ...
      const replaceAll = Effect.fn("TaskRegistry.replaceAll")(function* (
        newJobs: ReadonlyArray<Task>,
      ) {
        const limit = AGENTS_TASK_MANIFEST_LIMITS.maxTrackedJobs;
