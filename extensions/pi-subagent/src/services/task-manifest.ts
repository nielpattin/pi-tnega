...
export const AGENTS_TASK_MANIFEST_VERSION = 1;
 ...
 */
export const AGENTS_TASK_MANIFEST_LIMITS = {
...
export function resetManifestLimits(): void {
  AGENTS_TASK_MANIFEST_LIMITS.maxTrackedJobs = 64;
  AGENTS_TASK_MANIFEST_LIMITS.maxTerminalAgeMs = 30 * 24 * 60 * 60 * 1000;
  AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars = 8192;
  AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength = 64;
  AGENTS_TASK_MANIFEST_LIMITS.maxPersistedNestingDepth = 8;
  AGENTS_TASK_MANIFEST_LIMITS.maxPersistedJobBytes = 65536;
  AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes = 1024 * 1024;
  AGENTS_TASK_MANIFEST_LIMITS.persistDebounceMs = 50;
}
...
export interface AgentsTaskIndex {
...
function boundString(value: string, state: NormalizationState): string {
  return truncateString(value, AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars, state);
}

function boundStringWithoutAccounting(value: string): string {
  return truncateString(
    value,
    AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars,
    createRootNormalizationState(),
  );
}
 ...
 */
export function toPersistableValue(
  value: unknown,
  ...
  try {
    ...
    if (ctx.depth >= AGENTS_TASK_MANIFEST_LIMITS.maxPersistedNestingDepth) {
      state.truncated = true;
    ...
    if (typeof value === "object") {
      ...
      try {
        if (Array.isArray(value)) {
          const max = AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength;
          const out: JsonValue[] = [];
 ...
 * session references and the captured agent system prompt.
 */
export function normalizePersistedTask(task: Task): NormalizedTaskResult {
  ...
  const normalizedResultData =
    task.resultData !== undefined
      ? (toPersistableValue(
          task.resultData,
          { depth: 0, seen: new Set(), path: "resultData" },
          state,
        ) as unknown)
      : undefined;

  const normalizedErrorText =
    task.errorText !== undefined ? boundString(task.errorText, state) : undefined;
  ...
  const normalized: Task = {
    ...
    profile: task.profile === undefined ? undefined : boundString(task.profile, state),
    ...
    systemPrompt:
      task.systemPrompt === undefined ? undefined : boundString(task.systemPrompt, state),
    status: task.status,
    ...
    paneId: task.paneId === undefined ? undefined : boundString(task.paneId, state),
    sessionId: task.sessionId === undefined ? undefined : boundString(task.sessionId, state),
  };

  if (byteLength(normalized) > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedJobBytes) {
    state.truncated = true;
    let reduced: Task = {
      ...normalized,
      resultData: truncatedMarker("per-Task byte limit exceeded"),
      systemPrompt: undefined,
      promptOrCommand: normalized.promptOrCommand.slice(
        0,
        Math.min(normalized.promptOrCommand.length, 80),
      ),
    };
    ...
    if (byteLength(reduced) > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedJobBytes) {
...
export function pruneTerminalTasksForRetention(
  ...
  now: number,
  targetSize: number = AGENTS_TASK_MANIFEST_LIMITS.maxTrackedJobs,
): Task[] {
  const maxAge = AGENTS_TASK_MANIFEST_LIMITS.maxTerminalAgeMs;
  ...
  const removedByCapacity = removable.slice(
    0,
    Math.max(0, jobs.length - Math.min(targetSize, jobs.length)),
  );
...
export interface NormalizedIndexResult {
  readonly index: AgentsTaskIndex;
  readonly summary: ManifestSummary;
}

function indexByteSize(index: AgentsTaskIndex): number {
  // This is the exact serialization used by AgentsTaskPersistence. Keeping the
...
function compactTaskSummaries(task: Task): {
  ...
  const promptOrCommand =
    compactText(task.promptOrCommand, "promptOrCommand") ?? truncationText("promptOrCommand", 0);
  if (promptOrCommand !== task.promptOrCommand) changed = true;

  return { task: { ...task, resultData, errorText, promptOrCommand }, droppedStringChars, changed };
}
 ...
 */
function minimalTaskSummary(task: Task): {
  readonly task: Task;
  readonly droppedStringChars: number;
  readonly changed: boolean;
} {
 ...
 */
export function buildPersistedIndex(
  jobs: ReadonlyArray<Task>,
  ...
  const retainedIds = new Set(
    pruneTerminalTasksForRetention(jobs, Date.now()).map((task) => task.id),
  );
  ...
  const makeIndex = (): AgentsTaskIndex => ({
    version: AGENTS_TASK_MANIFEST_VERSION,
    parentSessionFile:
      parentSessionFile === undefined ? undefined : boundStringWithoutAccounting(parentSessionFile),
    summary,
    jobs: retainedForSize,
  });

  let index = makeIndex();
  const maxManifestBytes = AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes;
  ...
  // Measure before and after each registry reduction. Required configuration
  // fields remain exact, and jobs are dropped only after this pass.
  if (indexByteSize(index) > maxManifestBytes) applyReduction(compactTaskSummaries);
  if (indexByteSize(index) > maxManifestBytes) applyReduction(minimalTaskSummary);
  ...
  // agent IDs through the original snapshot's sequence maximum.
...
function isPersistedString(value: unknown): value is string {
  return (
    typeof value === "string" && value.length <= AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars
  );
}
...
function parseUsageStats(value: unknown): TaskUsageStats | undefined {
  if (value === undefined) return undefined;
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  const record = value as Record<string, unknown>;
  if (!hasOnlyKeys(record, new Set(["cost", "toolCalls", "contextTokens"]))) return undefined;
  const cost = parseFiniteNumber(record.cost);
  const toolCalls = parseFiniteNumber(record.toolCalls);
  const contextTokens = parseFiniteNumber(record.contextTokens);
  if (cost === undefined || toolCalls === undefined || contextTokens === undefined)
    return undefined;
  return { cost, toolCalls, contextTokens };
}

function sanitizeUsageStats(value: TaskUsageStats | undefined): TaskUsageStats | undefined {
  if (value === undefined) return undefined;
  const cost = parseFiniteNumber(value.cost);
  const toolCalls = parseFiniteNumber(value.toolCalls);
  const contextTokens = parseFiniteNumber(value.contextTokens);
  if (cost === undefined || toolCalls === undefined || contextTokens === undefined)
    return undefined;
  return { cost, toolCalls, contextTokens };
}
...
function parsePersistedJsonValue(value: unknown, depth = 0): JsonValue | undefined {
  ...
  if (Array.isArray(value)) {
    if (value.length > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength) return undefined;
    if (depth >= AGENTS_TASK_MANIFEST_LIMITS.maxPersistedNestingDepth) return undefined;
  ...
  } else if (depth >= AGENTS_TASK_MANIFEST_LIMITS.maxPersistedNestingDepth) {
    return undefined;
  }
...
function parseTranscriptContent(value: unknown): ReadonlyArray<TaskTranscriptContent> | undefined {
  if (!Array.isArray(value) || value.length > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength)
    return undefined;
...
function parseTranscript(value: unknown): ReadonlyArray<TaskTranscriptEntry> | undefined {
  ...
  for (const item of value) {
    ...
    if (
      ...
      if (
        !hasOnlyKeys(item, new Set(["type", "text", "timestamp"])) ||
        !isPersistedString(item.text)
      ) {
        return undefined;
      }
    ...
    if (item.type === "tool-call") {
      if (
        !hasOnlyKeys(
          item,
          new Set(["type", "toolCallId", "toolName", "arguments", "raw", "timestamp"]),
        ) ||
    ...
    if (item.type === "tool-result") {
      if (
        !hasOnlyKeys(
          item,
          new Set(["type", "toolCallId", "toolName", "content", "isError", "raw", "timestamp"]),
        ) ||
        !isPersistedString(item.toolCallId) ||
        !isPersistedString(item.toolName) ||
...
function parseManifestSummary(value: unknown): ManifestSummary | undefined {
  ...
  if ("droppedTranscriptEntries" in value) {
    ...
    if (legacyCount === undefined || legacyCount < 0 || !Number.isInteger(legacyCount))
      return undefined;
  }
  return {
...
export function parsePersistedTaskEntry(raw: unknown): Task | undefined {
  ...
  try {
    if (
      ...
      AGENTS_TASK_MANIFEST_LIMITS.maxPersistedJobBytes
    )
      return undefined;
  } catch {
    return undefined;
  ...
  if (
    ...
    (!TASK_STATUSES.has(record.status as TaskStatus) && record.status !== "recoverable")
  )
    return undefined;
  ...
  if (record.profile !== undefined && !isPersistedString(record.profile)) return undefined;
  ...
  if (
    ...
    (batchSize === undefined || batchSize < 1 || !Number.isInteger(batchSize))
  )
    return undefined;
  if (record.errorText !== undefined && !isPersistedString(record.errorText)) return undefined;
  if (record.systemPrompt !== undefined && !isPersistedString(record.systemPrompt))
    return undefined;
  if (record.recoveryPending !== undefined && typeof record.recoveryPending !== "boolean")
    return undefined;
  if (record.resumeWithPrompt !== undefined && typeof record.resumeWithPrompt !== "boolean")
    return undefined;
  ...
  const resultData =
    record.resultData === undefined ? undefined : parsePersistedJsonValue(record.resultData);
  if (record.resultData !== undefined && resultData === undefined) return undefined;
  const transcript =
    record.transcript === undefined ? undefined : parseTranscript(record.transcript);
  if (record.transcript !== undefined && transcript === undefined) return undefined;
  const usage = parseUsageStats(record.usage);
  if (record.usage !== undefined && usage === undefined) return undefined;

  // Read legacy recoverable records as failed because recovery is no longer supported.
  const status = record.status === "recoverable" ? "failed" : (record.status as TaskStatus);

  return {
    id: record.id,
    ownerSessionId: record.ownerSessionId,
    name,
    profile: parseOptionalString(record.profile),
 ...
 */
export function parsePersistedIndex(record: Record<string, unknown>): AgentsTaskIndex | undefined {
  ...
  if (record.version !== AGENTS_TASK_MANIFEST_VERSION) return undefined;

  if (record.parentSessionFile !== undefined && !isPersistedString(record.parentSessionFile))
    return undefined;
  if (record.writtenAt !== undefined && parseFiniteNumber(record.writtenAt) === undefined)
    return undefined;
  // Accept the former sequence field while old manifests migrate on their next write.
  const rawReservedTaskSeq = record.reservedTaskSeq;
  if (rawReservedTaskSeq !== undefined) {
    const reservedTaskSeq = parseFiniteNumber(rawReservedTaskSeq);
    if (reservedTaskSeq === undefined || reservedTaskSeq < 0 || !Number.isInteger(reservedTaskSeq))
      return undefined;
  }
  if (
    !Array.isArray(record.jobs) ||
    record.jobs.length > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength
  )
    return undefined;
