import type { Task, TaskStatus, TaskTranscriptContent, TaskTranscriptEntry, TaskUsageStats } from "../domain.js";
...
function boundStringWithoutAccounting(value: string): string {
   return truncateString(value, AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars, createRootNormalizationState());
}

function boundedMarker(message: string): string {
   return boundedMetadataText(message, AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars, "[truncated]");
}
...
function truncationText(label: string, omitted: number): string {
   ...
   return boundedMetadataText(full, AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars, "[__truncated]");
}
...
const TASK_STATUSES: ReadonlySet<TaskStatus> = new Set(["pending", "running", "completed", "failed", "cancelled"]);
...
function isPersistedString(value: unknown): value is string {
   return typeof value === "string" && value.length <= AGENTS_TASK_MANIFEST_LIMITS.maxPersistedStringChars;
}
...
function parseUsageStats(value: unknown): TaskUsageStats | undefined {
   ...
   if (cost === undefined || toolCalls === undefined || contextTokens === undefined) return undefined;
...
function parseTranscriptContent(value: unknown): ReadonlyArray<TaskTranscriptContent> | undefined {
   if (!Array.isArray(value) || value.length > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength) return undefined;
...
export function parsePersistedIndex(record: Record<string, unknown>): AgentsTaskIndex | undefined {
   ...
   if (!Array.isArray(record.jobs) || record.jobs.length > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedArrayLength)
      return undefined;
