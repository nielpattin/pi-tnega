...
import {
  formatUnknownAgentProfileError,
  resolveAgentProfile,
  type AgentProfile,
...
interface SpawnAgentOptions {
  ...
  readonly cwd: string;
  readonly agentDef: Pick<
    AgentProfile,
    "name" | "systemPrompt" | "body" | "model" | "thinking" | "tools"
  >;
...
function profileSystemPrompt(
  agentDef: Pick<AgentProfile, "systemPrompt" | "body">,
): string | undefined {
    ...
    control: (text) =>
      Effect.tryPromise({
        try: () => handle.control(text),
          ...
          new ControlError({ message: error instanceof Error ? error.message : String(error) }),
      }),
...
async function launchAgent(options: SpawnAgentOptions): Promise<ExternalAgentHandle> {
  ...
  return launchExternalAgent({
    ...
    model: options.preserveSessionModel
      ? undefined
      : modelArgument(options.agentDef, options.inheritedModel),
    thinking: options.preserveSessionModel
      ? undefined
      : (options.specThinking ?? options.agentDef.thinking),
...
export interface AgentManagerShape {
  ...
  readonly resumeInterruptedTasks: (
    ownerSessionId: string,
  ) => Effect.Effect<ReadonlyArray<Task>, any>;
}
  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const clearActiveSession = (taskId: string, ownerSessionId?: string) => {
        if (ownerSessionId !== undefined && activeSessionOwners.get(taskId) !== ownerSessionId)
          return;
      ...
      const spawnBatch = Effect.fn("AgentManager.spawnBatch")(function* (
        ...
        const herdrUsable =
          shouldManageHerdrLayout && options?.useHerdr !== false && ops.available();
        if ((incomingCount > 1 || options?.forceNewTab === true) && herdrUsable) {
          try {
            batchTab = ops.createTab(
              `agents ${options?.batchId ?? "batch"}`,
              specs[0].cwd ?? process.cwd(),
            );
        ...
        return yield* Effect.gen(function* () {
          ...
          for (const [index, spec] of specs.entries()) {
            ...
            const targetProfile = spec.profile.trim();
            let agentDef = resolveAgentProfile(
              targetProfile,
            ...
            if (
              ...
              agentDef = {
                ...
                ...(spec.thinking === undefined
                  ? {}
                  : { thinking: spec.thinking as AgentProfile["thinking"] }),
              };
            }
                ...
                const handle = await launchAgent({
                  ...
                  onActivity: (activity) => {
                    ...
                    void Effect.runPromise(
                      updateRunningIfActive(taskId, { activity }, ownerSessionId),
                    ).catch(() => {});
                  },
      ...
      const closeSettledPanes = Effect.fn("AgentManager.closeSettledPanes")(function* (
        ownerSessionId: string,
      ) {
        ...
        for (const task of tasks) {
          ...
          const current = yield* registry.get(task.id);
          if (!current || current.paneId !== task.paneId || !isTerminalStatus(current.status))
            continue;
          yield* registry
      ...
      const markResultsDelivered = Effect.fn("AgentManager.markResultsDelivered")(function* (
        ids: Iterable<string>,
      ) {
        const marked: string[] = [];
        for (const id of ids) {
          ...
          yield* registry.updateStatus(id, current.status, {
      ...
      const cancelActiveSessions = Effect.gen(function* () {
        ...
        for (const task of tasks) {
          if (
            ...
            continue;
          yield* registry
            .updateStatus(task.id, task.status, { recoveryPending: true })
            .pipe(Effect.ignore);
        }
