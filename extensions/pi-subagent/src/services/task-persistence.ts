...
import {
  AGENTS_TASK_MANIFEST_VERSION,
  AGENTS_TASK_MANIFEST_LIMITS,
  buildPersistedIndex,
  parsePersistedIndex,
  type AgentsTaskIndex,
...
/** Narrow filesystem capability used by Agents Task persistence. */
...
const nodeFileSystem: AgentsFileSystem = {
  mkdir: async (dir) => {
...
export interface AgentsTaskPersistenceShape {
  readonly configure: (
    parentSessionFile: string | undefined | null,
  ) => Effect.Effect<void, ManifestPersistenceError>;
  ...
  readonly load: () => Effect.Effect<AgentsTaskIndex, ManifestPersistenceError>;
  readonly persist: (
...
export class AgentsTaskPersistence extends Context.Service<
  AgentsTaskPersistence,
  AgentsTaskPersistenceShape
>()("agents/services/AgentsTaskPersistence") {
  static readonly layer = Layer.effect(
    AgentsTaskPersistence,
    Effect.sync(() => makeAgentsTaskPersistenceShape(nodeFileSystem)),
  );

  static layerWith(fileSystem: AgentsFileSystem): Layer.Layer<AgentsTaskPersistence> {
    return Layer.effect(
      AgentsTaskPersistence,
      Effect.sync(() => makeAgentsTaskPersistenceShape(fileSystem)),
    );
  }
}
...
function makeAgentsTaskPersistenceShape(fileSystem: AgentsFileSystem): AgentsTaskPersistenceShape {
  ...
  const resolveFinalPath = (): string | undefined => {
    ...
    return path.join(indexDir, AGENTS_TASKS_FILE);
  };
  ...
  const isTempArtifact = (entry: string): boolean => entry.startsWith(`${AGENTS_TASKS_FILE}.tmp-`);
  const isBackupArtifact = (entry: string): boolean =>
    entry.startsWith(`${AGENTS_TASKS_FILE}.bak-`);
  ...
  const backupTimestamp = (entry: string): number => {
    const prefix = `${AGENTS_TASKS_FILE}.bak-`;
  ...
  const reconcileBackupsAtConfigure = async (dir: string): Promise<void> => {
    ...
    if (finalIndex) {
      ...
      await Promise.all(
        backups.map((entry) => fileSystem.unlink(path.join(dir, entry)).catch(() => {})),
      );
    ...
    await Promise.all(
      olderBackupPaths.map((backupPath) => fileSystem.unlink(backupPath).catch(() => {})),
    );
  ...
  const readIndexFromDisk = async (filePath: string): Promise<AgentsTaskIndex | undefined> => {
    try {
      const text = await fileSystem.readFile(filePath);
      if (text === undefined) return undefined;
      if (Buffer.byteLength(text, "utf8") > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes)
        return undefined;
  ...
  const configure: AgentsTaskPersistenceShape["configure"] = Effect.fn(
    "AgentsTaskPersistence.configure",
  )((parentSessionFile) =>
    Effect.gen(function* () {
  ...
  const currentTarget: AgentsTaskPersistenceShape["currentTarget"] = Effect.fn(
    "AgentsTaskPersistence.currentTarget",
  )(() => Effect.succeed(configuredParentSessionFile));

  const takeChangeListener: AgentsTaskPersistenceShape["takeChangeListener"] = Effect.fn(
    "AgentsTaskPersistence.takeChangeListener",
  )(() =>
    Effect.sync(() => {
  ...
  const setChangeListener: AgentsTaskPersistenceShape["setChangeListener"] = Effect.fn(
    "AgentsTaskPersistence.setChangeListener",
  ...
  const takeChangeWriter: AgentsTaskPersistenceShape["takeChangeWriter"] = Effect.fn(
    "AgentsTaskPersistence.takeChangeWriter",
  )(() =>
    Effect.sync(() => {
  ...
  const setChangeWriter: AgentsTaskPersistenceShape["setChangeWriter"] = Effect.fn(
    "AgentsTaskPersistence.setChangeWriter",
  ...
  const currentDir: AgentsTaskPersistenceShape["currentDir"] = Effect.fn(
    "AgentsTaskPersistence.currentDir",
  )(() => Effect.succeed(indexDir));

  const load: AgentsTaskPersistenceShape["load"] = Effect.fn("AgentsTaskPersistence.load")(() =>
    Effect.gen(function* () {
      const finalPath = resolveFinalPath();
      if (!finalPath) {
        return { version: AGENTS_TASK_MANIFEST_VERSION, jobs: [] as Task[] };
      }
      ...
      // before returning an empty task set.
      yield* Effect.promise(() => quarantineFile(finalPath));
      const dir = path.dirname(finalPath);
      yield* Effect.promise(() => syncDirectory(dir));
      return { version: AGENTS_TASK_MANIFEST_VERSION, source: "missing", jobs: [] as Task[] };
    }),
  );

  const persist: AgentsTaskPersistenceShape["persist"] = Effect.fn("AgentsTaskPersistence.persist")(
    (jobs) =>
      Effect.gen(function* () {
        const finalPath = resolveFinalPath();
        ...
        try {
          ...
          if (
            Buffer.byteLength(data, "utf8") > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes
          ) {
            throw new Error("Serialized agents jobs manifest exceeds maxPersistedManifestBytes");
          }
        } catch (error) {
          return yield* new ManifestSerializationError({
            message:
              error instanceof Error
                ? `Failed to serialize agents jobs manifest: ${error.message}`
                : "Failed to serialize agents jobs manifest",
            cause: error,
          });
        }
        ...
        yield* Effect.tryPromise({
          try: () => next,
            ...
            new ManifestPersistenceError({
              message:
                error instanceof Error
                  ? `Failed to persist agents jobs manifest: ${error.message}`
                  : "Failed to persist agents jobs manifest",
              cause: error,
            }),
        });
  ...
  const flush: AgentsTaskPersistenceShape["flush"] = Effect.fn("AgentsTaskPersistence.flush")(() =>
    Effect.gen(function* () {
      yield* Effect.tryPromise({
        try: () => writeLock,
        catch: (error) =>
          new ManifestPersistenceError({
            message:
              error instanceof Error
                ? `Failed to flush agents jobs manifest: ${error.message}`
                : "Failed to flush agents jobs manifest",
            cause: error,
          }),
      });
    }),
  ...
  return AgentsTaskPersistence.of({
    configure,
    currentTarget,
    currentDir,
    takeChangeListener,
 ...
 */
export function createRegistryChangeWriter(
  persistence: AgentsTaskPersistenceShape,
): RegistryChangeWriter {
  ...
  const snapshotContains = (snapshot: ReadonlyArray<Task>, id: string) =>
    snapshot.some((task) => task.id === id);

  const logPersistError = (error: unknown) => {
    try {
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[agents] failed to persist Task manifest: ${message}`);
    } catch {
  ...
  const scheduleWrite = (jobs: ReadonlyArray<Task>, immediate: boolean) => {
    ...
    if (immediate) {
      ...
      // transitions in the same agent collapse into a single disk write.
      timer = setTimeout(execute, 0);
    } else {
      timer = setTimeout(execute, AGENTS_TASK_MANIFEST_LIMITS.persistDebounceMs);
    }
  };
...
export function markInterruptedTaskFailed(stored: Task): Task {
  return {
    ...stored,
    status: "failed",
