...
export class AgentsTaskPersistence extends Context.Service<AgentsTaskPersistence, AgentsTaskPersistenceShape>()(
...
function makeAgentsTaskPersistenceShape(fileSystem: AgentsFileSystem): AgentsTaskPersistenceShape {
   ...
   const isBackupArtifact = (entry: string): boolean => entry.startsWith(`${AGENTS_TASKS_FILE}.bak-`);
   ...
   const readIndexFromDisk = async (filePath: string): Promise<AgentsTaskIndex | undefined> => {
      try {
         ...
         if (Buffer.byteLength(text, "utf8") > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes) return undefined;
   ...
   const configure: AgentsTaskPersistenceShape["configure"] = Effect.fn("AgentsTaskPersistence.configure")(
      (parentSessionFile) =>
         Effect.gen(function* () {
   ...
   const currentTarget: AgentsTaskPersistenceShape["currentTarget"] = Effect.fn("AgentsTaskPersistence.currentTarget")(
   ...
   const currentDir: AgentsTaskPersistenceShape["currentDir"] = Effect.fn("AgentsTaskPersistence.currentDir")(() =>
   ...
   const persist: AgentsTaskPersistenceShape["persist"] = Effect.fn("AgentsTaskPersistence.persist")((jobs) =>
      Effect.gen(function* () {
         const finalPath = resolveFinalPath();
         ...
         try {
            ...
            if (Buffer.byteLength(data, "utf8") > AGENTS_TASK_MANIFEST_LIMITS.maxPersistedManifestBytes) {
 ...
 */
export function createRegistryChangeWriter(persistence: AgentsTaskPersistenceShape): RegistryChangeWriter {
