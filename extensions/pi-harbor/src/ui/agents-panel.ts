...
export class FullScreenAgentsManager implements Component, Focusable {
   ...
   private async generateAndOpenDraft() {
      ...
      const fallbackBody = `# ${this.newName.toUpperCase()} AGENT\n\nSpecialized agent for: ${this.newIntent || "custom tasks"}.\n\n## Role\nExecute tasks according to intent.\n\n## Constraints\n- Follow guidelines strictly.`;

      const newAgent: AgentDefinition = {
         name: this.newName,
         description: generatedFields?.description || this.newIntent || "Custom agent",
         guidance: generatedFields?.guidance || `Use for ${this.newIntent || "custom tasks"}.`,
         harness: "pi",
   ...
   private getAvailableTools(): AgentToolInfo[] {
      ...
      // 1. Discover tools dynamically from HarborRuntime, ExtensionCommandContext & Registries
