...
import {
   ...
   computeNextTaskSeq
} from "../services/HarborJobPersistence.js";
import type { Job } from "../domain.js";

/**
 * Serialized parent-session activation.
 *
 * This is the single place where Harbor moves from one parent session to
 ...
 * - reserving task IDs from the validated full index
...
export const activateParentSession = Effect.fn("activateParentSession")(function* (
   ...
   return yield* gate.runExclusive(
      Effect.gen(function* () {
         ...
         yield* taskManager.reserveTaskSeq(index.reservedTaskSeq ?? computeNextTaskSeq(restored));
...
/**
 * Safe first-task/session-file readiness refresh.
 *
...
export const flushPendingWrites = Effect.fn("flushPendingWrites")(function* () {
   ...
   yield* persistence.flush().pipe(
      Effect.catch((error) =>
         Effect.sync(() => {
            try {
               console.error(`[harbor] failed to flush pending writes: ${error.message}`);
            } catch {
