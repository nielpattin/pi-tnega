...
import {
   HARBOR_JOB_MANIFEST_VERSION,
   HARBOR_JOB_MANIFEST_LIMITS,
   buildPersistedIndex,
   parsePersistedIndex,
   type HarborJobIndex
} from "./HarborJobManifest.js";

export const HARBOR_JOBS_FILE = "harbor-jobs.json";
export { HARBOR_JOB_MANIFEST_VERSION as HARBOR_JOBS_VERSION };
export { HARBOR_JOB_MANIFEST_LIMITS };
export { computeReservedTaskSeq as computeNextTaskSeq } from "./HarborJobManifest.js";

const RESTART_INTERRUPTED_ERROR =
   "Harbor parent session restarted before this job settled. The job was not resumed to avoid duplicating work.";

const MAX_REPLACE_ATTEMPTS = 5;
const RETRY_BASE_MS = 5;
const RETRY_MAX_MS = 80;

/** Narrow filesystem capability used by Harbor job persistence. */
export interface HarborFileSystem {
...
const nodeFileSystem: HarborFileSystem = {
   mkdir: async (dir) => {
...
export interface HarborJobPersistenceShape {
   ...
   readonly load: () => Effect.Effect<HarborJobIndex, ManifestPersistenceError>;
   readonly persist: (
...
export class HarborJobPersistence extends Context.Service<HarborJobPersistence, HarborJobPersistenceShape>()(
   "harbor/services/HarborJobPersistence"
) {
   static readonly layer = Layer.effect(
      HarborJobPersistence,
      Effect.sync(() => makeHarborJobPersistenceShape(nodeFileSystem))
   );

   static layerWith(fileSystem: HarborFileSystem): Layer.Layer<HarborJobPersistence> {
      return Layer.effect(
         HarborJobPersistence,
         Effect.sync(() => makeHarborJobPersistenceShape(fileSystem))
      );
   }
}
...
function makeHarborJobPersistenceShape(fileSystem: HarborFileSystem): HarborJobPersistenceShape {
   ...
   const resolveFinalPath = (): string | undefined => {
      ...
      return path.join(indexDir, HARBOR_JOBS_FILE);
   };
   ...
   const isTempArtifact = (entry: string): boolean => entry.startsWith(`${HARBOR_JOBS_FILE}.tmp-`);
   const isBackupArtifact = (entry: string): boolean => entry.startsWith(`${HARBOR_JOBS_FILE}.bak-`);
   ...
   const backupTimestamp = (entry: string): number => {
      const prefix = `${HARBOR_JOBS_FILE}.bak-`;
   ...
   const readIndexFromDisk = async (filePath: string): Promise<HarborJobIndex | undefined> => {
      try {
         const text = await fileSystem.readFile(filePath);
         if (text === undefined) return undefined;
         if (Buffer.byteLength(text, "utf8") > HARBOR_JOB_MANIFEST_LIMITS.maxPersistedManifestBytes) return undefined;
   ...
   const currentTarget: HarborJobPersistenceShape["currentTarget"] = Effect.fn("HarborJobPersistence.currentTarget")(
   ...
   const load: HarborJobPersistenceShape["load"] = Effect.fn("HarborJobPersistence.load")(() =>
      Effect.gen(function* () {
         const finalPath = resolveFinalPath();
         if (!finalPath) {
            return { version: HARBOR_JOB_MANIFEST_VERSION, jobs: [] as Job[] };
         }
         ...
         return { version: HARBOR_JOB_MANIFEST_VERSION, source: "missing", jobs: [] as Job[] };
      })
   );
   ...
   const persist: HarborJobPersistenceShape["persist"] = Effect.fn("HarborJobPersistence.persist")((jobs) =>
      Effect.gen(function* () {
         ...
         try {
            ...
            if (Buffer.byteLength(data, "utf8") > HARBOR_JOB_MANIFEST_LIMITS.maxPersistedManifestBytes) {
               throw new Error("Serialized harbor jobs manifest exceeds maxPersistedManifestBytes");
            }
         } catch (error) {
            return yield* new ManifestSerializationError({
               message:
                  error instanceof Error
                     ? `Failed to serialize harbor jobs manifest: ${error.message}`
                     : "Failed to serialize harbor jobs manifest",
               cause: error
            });
         }
         ...
         yield* Effect.tryPromise({
            try: () => next,
               ...
               new ManifestPersistenceError({
                  message:
                     error instanceof Error
                        ? `Failed to persist harbor jobs manifest: ${error.message}`
                        : "Failed to persist harbor jobs manifest",
                  cause: error
               })
         });
   ...
   const flush: HarborJobPersistenceShape["flush"] = Effect.fn("HarborJobPersistence.flush")(() =>
      Effect.gen(function* () {
         yield* Effect.tryPromise({
            try: () => writeLock,
            catch: (error) =>
               new ManifestPersistenceError({
                  message:
                     error instanceof Error
                        ? `Failed to flush harbor jobs manifest: ${error.message}`
                        : "Failed to flush harbor jobs manifest",
                  cause: error
               })
         });
      })
 ...
 */
export function createRegistryChangeWriter(persistence: HarborJobPersistenceShape): RegistryChangeWriter {
   ...
   const logPersistError = (error: unknown) => {
      try {
         ...
         console.error(`[harbor] failed to persist job manifest: ${message}`);
      } catch {
   ...
   const scheduleWrite = (jobs: ReadonlyArray<Job>, immediate: boolean) => {
      ...
      if (immediate) {
         ...
         // transitions in the same task collapse into a single disk write.
         timer = setTimeout(execute, 0);
      } else {
         timer = setTimeout(execute, HARBOR_JOB_MANIFEST_LIMITS.persistDebounceMs);
      }
   };
