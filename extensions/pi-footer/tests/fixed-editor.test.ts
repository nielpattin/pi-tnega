...
test("terminal split consumes ctrl shift c when selection exists", () => {
   const terminal = new FakeTerminal();
   let inputListener: ((data: string) => { consume?: boolean; data?: string } | undefined) | null = null;
   const copied: string[] = [];
   const tui = {
   ...
   assert.deepEqual(inputListener?.("\x1b[99;6u"), { consume: true });
...
test("terminal split end jumps to chat bottom when chat is scrolled", () => {
   const terminal = new FakeTerminal();
   let inputListener: ((data: string) => { consume?: boolean; data?: string } | undefined) | null = null;
   const tui = {
      terminal,
   ...
   assert.deepEqual(inputListener?.("\x1b[F"), { data: "\x1b[F" });
   assert.deepEqual(tui.render(), [
      "line-20",
      "line-21",
      "line-22",
   ...
   assert.equal(inputListener?.("\x1b[F"), undefined);
