...
import type { ColorScheme, SegmentContext } from "./types.js";
import type { PowerlineConfig } from "./powerline-config.js";
import { collectHiddenExtensionStatusKeys, getNotificationExtensionStatuses } from "./powerline-config.js";
...
export interface LayoutCache {
	...
	lastLayoutResult: { topContent: string; secondaryContent: string } | null;
...
export function createRenderFunctions(deps: RenderDeps, cache: LayoutCache) {
	function buildSegmentContext(ctx: any, theme: Theme): SegmentContext {
		const presetDef = getPreset(deps.config.preset);
	...
	function getResponsiveLayout(width: number, theme: Theme): { topContent: string; secondaryContent: string } {
		...
		cache.lastLayoutResult = computeResponsiveLayout(segmentCtx, presetDef, width, deps.config.customItems);
