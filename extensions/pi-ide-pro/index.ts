...
export default function piIdePro(pi: ExtensionAPI): void {
  ...
  function registerAutocomplete(ctx: ExtensionContext): void {
    ...
    ctx.ui.addAutocompleteProvider(
      (baseProvider) => new VscodeAutocompleteProvider(baseProvider, runtime),
    );
  }
  ...
  async function connect(ctx: ExtensionContext | ExtensionCommandContext): Promise<void> {
    ...
    const connection = new IdeBridgeConnection(candidate, ctx.cwd, runtime.sessionId, {
      ...
      onDiagnosticRequest: (request) =>
        handleDiagnosticRequest(pi, runtime, request as DiagnosticRequest),
    });
...
class VscodeAutocompleteProvider implements AutocompleteProvider {
  ...
  async getSuggestions(
    ...
    const files = sortOpenFiles(this.runtime.files)
      ...
      .filter(
        ({ file, path }) => fuzzyMatch(query, path) || fuzzyMatch(query, basename(file.filePath)),
      )
      .slice(0, 10);
    ...
    handler: async (args, ctx) => {
      ...
      if (!command) {
        ...
        const filtered = diagnostics.filter(
          (diagnostic) => showAll || !active || diagnostic.filePath === active,
        );
...
function handleDiagnosticRequest(
  pi: ExtensionAPI,
  runtime: Runtime,
...
function formatDiagnosticRequest(request: DiagnosticRequest): string {
  const lines = [
    ...
    `Trigger range: L${request.triggerRange.start.line + 1}`,
  ];
  for (const [index, diagnostic] of request.diagnostics.entries()) {
    ...
    if (diagnostic.selectedText)
      lines.push("- Selected text:", "```", diagnostic.selectedText, "```");
    if (diagnostic.contextLines?.length)
      lines.push(
        "- Context lines:",
        ...diagnostic.contextLines.map((line) => `${line.line + 1}: ${line.text}`),
      );
  }
