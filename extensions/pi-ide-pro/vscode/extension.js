...
function publishClients() {
  broadcast({
    ...
    params: { clients: clientsForWorkspace() },
  });
  updateStatus();
}
...
function handleRpc(socket, value) {
  ...
  if (!client) return;
  if (value.method === "get_open_files")
    send(socket, { jsonrpc: "2.0", id: value.id, result: getOpenFiles() });
...
function startServer(context) {
  ...
  websocketServer = new WebSocketServer({
    ...
    verifyClient: ({ req }, done) =>
      done(req.headers["x-pi-ide-pro-authorization"] === token, 401, "Unauthorized"),
  });
  ...
  websocketServer.on("connection", (socket) => {
    socket.on("message", (raw) => {
      try {
        ...
        log(
          "error",
          `RPC message failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    });
