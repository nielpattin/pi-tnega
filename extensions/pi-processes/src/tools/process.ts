...
import {
  DEFAULT_PROCESS_SNAPSHOT_LINES,
...
const EnvironmentSchema = Type.Optional(
  Type.Record(Type.String(), Type.String(), {
    description: "Environment variables added to the process.",
  }),
);
...
export const ProcessStartToolParamsSchema = Type.Object(
  {
    ...
    cwd: Type.Optional(
      Type.String({ description: "Working directory. Defaults to the session directory." }),
    ),
...
export const ProcessListToolParamsSchema = Type.Object(
  {},
...
export const handleProcessStart = Effect.fn("process.handleStart")(function* (
  params: ProcessStartToolParams,
) {
...
export const handleProcessList = Effect.fn("process.handleList")(function* (
  _params: ProcessListToolParams,
) {
  const supervisor = yield* ProcessSupervisor;
...
export const handleProcessSnapshot = Effect.fn("process.handleSnapshot")(function* (
  params: ProcessSnapshotToolParams,
) {
  const supervisor = yield* ProcessSupervisor;
...
export const handleProcessRestart = Effect.fn("process.handleRestart")(function* (
  params: ProcessRestartToolParams,
) {
  const supervisor = yield* ProcessSupervisor;
  const process = findProcess(yield* supervisor.ps, params.id);
  if (!process) return processNotFound(params.id);
...
export const handleProcessStop = Effect.fn("process.handleStop")(function* (
  params: ProcessStopToolParams,
) {
  const supervisor = yield* ProcessSupervisor;
  const process = findProcess(yield* supervisor.ps, params.id);
  if (!process) return processNotFound(params.id);
