...
export function paginateLogLinesWithStreams(
  lines: LogLine[],
  options?: LogViewerOptions,
): PaginatedLogLines {
  const requestedLimit =
    options?.lines === undefined ? DEFAULT_PROCESS_SNAPSHOT_LINES : Math.floor(options.lines);
  ...
  return { lines: bounded, ...(hasEarlier && first ? { before: first.sequence } : {}) };
}
