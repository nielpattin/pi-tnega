...
export class ProcessDetailView implements Component {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      truncateToWidth(`  ${theme.fg("accent", theme.bold(`${this.logMode} output`))}`, width),
    );

    const viewport = this.viewportHeight();
    const output = this.logLines.flatMap((entry) => {
      const timestamp =
        entry.timestamp === undefined ? "" : `${formatProcessLogTime(entry.timestamp)} `;
    ...
    lines.push(
      truncateToWidth(
        theme.fg(
          "dim",
          `Tab ${this.logMode} → next · Esc back · x stop · r restart · j/k scroll · g/G top/bottom`,
        ),
        width,
      ),
    );
...
export async function openProcessDetail(
  ...
  await ctx.ui.custom<null>(
    (tui, theme, keybindings, done) =>
      new ProcessDetailView(tui, theme, keybindings, runtime, procName, done),
