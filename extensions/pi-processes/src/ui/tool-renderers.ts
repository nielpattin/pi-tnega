...
function renderRecordList(
  ...
  if (shown.length < items.length)
    lines.push(theme.fg("dim", `… ${items.length - shown.length} more`));
  ...
  if (snapshotLines.length > 0) {
    ...
    if (!options.expanded && snapshotLines.length > visible.length) {
      lines.push(
        theme.fg(
          "muted",
          `... (${snapshotLines.length - visible.length} earlier lines, ctrl+o to expand)`,
        ),
      );
    }
...
function renderProcessResultEnvelope(
  ...
  if (Array.isArray(details.processes))
    return renderRecordList(records(details.processes), options, theme);
