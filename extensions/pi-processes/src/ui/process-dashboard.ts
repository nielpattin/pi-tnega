...
export class ProcessDashboard implements Component {
    ...
    if (data === "k" || this.keybindings.matches(data, "tui.select.up")) {
      this.selected =
        this.processes.length === 0
          ? 0
          : (this.selected - 1 + this.processes.length) % this.processes.length;
      this.tui.requestRender();
      return;
    }
    if (data === "x") {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      this.theme.fg(
        "dim",
        `${this.processes.length} tracked process${this.processes.length === 1 ? "" : "es"}`,
      ),
    );
...
export async function showProcessDashboard(
