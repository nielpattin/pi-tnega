    ...
    Effect.gen(function* () {
      ...
      const start = Effect.fn("ProcessSupervisor.start")(function* (params) {
        ...
        return yield* Effect.gen(function* () {
          ...
          const stdoutLogBuffer = new TimestampedOutputBuffer(
            RETAINED_STREAM_BYTES,
            nextLogSequence,
          );
          const stderrLogBuffer = new TimestampedOutputBuffer(
            RETAINED_STREAM_BYTES,
            nextLogSequence,
          );
          ...
          if (
            params.ready?.timeoutSec !== undefined &&
            params.ready.timeoutSec > 0 &&
      ...
      return ProcessSupervisor.of({ start, stop, restart, ps, logs });
    }),
  );
