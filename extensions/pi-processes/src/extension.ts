...
function createProcessToolResult(value: unknown) {
  return { content: [{ type: "text" as const, text: stringify(value) }], details: value };
}
...
async function executeProcessEffect(
  ...
  try {
    return createProcessToolResult(
      await runProcessTool(runtime, effect, { signal, interruptMessage }),
    );
  } catch (error) {
  ...
  const registerAction = <TParams extends TSchema>(definition: {
    ...
    pi.registerTool({
      ...
      async execute(_toolCallId, params, signal) {
        return executeProcessEffect(
          runtime,
          definition.handler(params),
          signal,
          definition.interruptMessage,
        );
      },
    });
  };
