...
function getAnsiSequenceLength(text: string, startIndex: number): number {
  ...
  while (index < text.length) {
    ...
    if (text.charCodeAt(index) === ESCAPE_CODE && text[index + 1] === "\\")
...
function getStickyLeftState(treeList: TreeListLike): StickyLeftState {
  ...
  if (startIndex === endIndex) {
    return { startIndex, endIndex, stickyLeftShift: 0, stickyLeftDepth: null };
  }
  ...
  for (let index = startIndex; index < endIndex; index++) {
    ...
    minVisibleDisplayIndent = Math.min(
      minVisibleDisplayIndent,
      getDisplayIndent(treeList, flatNode),
    );
  }
...
function markCurrentLine(treeList: TreeListLike, lines: string[]): string[] {
  ...
  const { startIndex, endIndex } = getVisibleWindow(treeList);
  let currentIndex = treeList.filteredNodes.findIndex(
    (node) => node?.node.entry.id === treeList.currentLeafId,
  );
  if (currentIndex === -1) {
  ...
  lines[currentIndex - startIndex] = replaceCursorSlot(
    lines[currentIndex - startIndex] ?? "",
    marker,
  );
  return lines;
}
...
function renderWithStickyLeft(
  ...
  for (let index = startIndex; index < endIndex; index++) {
    ...
    originalNodes.push({ flatNode, indent: flatNode.indent, gutters: flatNode.gutters });
...
function patchTreeListRender(treeList: TreeListLike): void {
  if ((treeList as unknown as { __treexStickyLeftPatched?: boolean }).__treexStickyLeftPatched)
    return;
  ...
  switch (entry.type) {
    case "message": {
      ...
      if (message.role === "user") {
        return {
          kind: "USER",
          full:
            extractDetailContent(treeList, message.content, { includeToolCalls: true }) ||
            "(empty)",
        };
      }
      ...
      if (message.role === "assistant") {
        return {
          kind: "ASSISTANT",
          full:
            extractDetailContent(treeList, message.content, {
      ...
      if (message.role === "toolResult") {
        return { kind: "TOOL RESULT", toolName: (message as { toolName?: string }).toolName };
      }
    ...
    case "custom_message":
      return {
        ...
        full:
          extractDetailContent(treeList, entry.content, { includeToolCalls: true }) || "(empty)",
      };

    case "compaction": {
      const tokenCount = Math.round((entry.tokensBefore ?? 0) / 1000);
      const fallback = `[compaction: ${tokenCount}k tokens]`;
      return { kind: "COMPACTION", full: normalizeDetail(entry.summary ?? fallback) || fallback };
    }

    case "branch_summary":
      return { kind: "BRANCH SUMMARY", full: normalizeDetail(entry.summary ?? "") || "(empty)" };

    case "model_change":
      return { kind: "MODEL", full: `[model: ${entry.modelId}]` };

    case "thinking_level_change":
      return { kind: "THINKING", full: `[thinking: ${entry.thinkingLevel}]` };
      ...
      return {
        ...
        full:
          entry.data === undefined
            ...
            : formatCustomEntryData(entry.data),
      };

    case "label":
      return { kind: "LABEL", full: entry.label ?? "(cleared)" };

    case "session_info":
      return { kind: "SESSION TITLE", full: entry.name ?? "(empty)" };

    default:
      return { kind: "ENTRY", full: "[entry]" };
  }
}
...
function getDetailContextUsage(
  session: AgentSessionWithInternals,
  entry: SessionEntry,
...
function formatDetailContextUsage(
  theme: Theme,
...
function getCurrentDirection(treeList: TreeListLike, selected: FlatNode): "up" | "down" | null {
  ...
  const currentFlatIndex = treeList.flatNodes.findIndex(
    (node) => node?.node.entry.id === treeList.currentLeafId,
  );
  const selectedFlatIndex = treeList.flatNodes.findIndex(
    (node) => node?.node.entry.id === selected.node.entry.id,
  );
  return currentFlatIndex < selectedFlatIndex ? "up" : "down";
}

function getCurrentPositionPart(
  treeList: TreeListLike,
  selected: FlatNode,
  theme: Theme,
): string | null {
...
function getDetailBodyLines(lines: string[], width: number, theme: Theme): string[] {
  ...
  if (lines.length > DETAIL_BODY_LINES) {
    ...
    bodyLines[lastLineIndex] = appendTruncatedDetailHint(
...
class ExpandedDetailPane {
  ...
  render(theme: Theme, width: number, title: string, contentLines: string[]): string[] {
    ...
    const percent =
      lines.length <= bodyHeight ? 100 : Math.round((lastVisibleLine / lines.length) * 100);
    const footerParts = [
...
class DetailContentRenderer {
  ...
  constructor(
    mode: InteractiveModeWithInternals,
    treeList: TreeListLike,
  ...
  createToolExecutionComponent(
    entry: SessionMessageEntry & {
      message: { role: "toolResult"; toolName: string; toolCallId: string };
    },
  ): ToolExecutionComponent {
    const message = entry.message;
  ...
  createUserMessageComponent(
    entry: SessionMessageEntry & { message: { role: "user" } },
  ): UserMessageComponent {
  ...
  renderExpanded(entry: SessionEntry, info: EntryInfo, width: number): string[] {
    ...
    if (entry.type === "compaction") {
      return this.renderExpandableEntryLines(
        this.components
          .compactionSummaryMessageComponent as unknown as ExpandableEntryComponentConstructor,
        entry,
        width,
      );
    }
    ...
    if (entry.type === "branch_summary") {
      return this.renderExpandableEntryLines(
        this.components
          .branchSummaryMessageComponent as unknown as ExpandableEntryComponentConstructor,
        entry,
        width,
      );
    }
...
class TreeXWrapper implements Component {
  ...
  getDetailMetadata(theme: Theme, selected: FlatNode, info: EntryInfo): string {
    ...
    const metadataGroups = [
      joinMetadataParts(theme, treeParts),
      joinMetadataParts(theme, entryParts),
    ];
  ...
  renderDetailPane(theme: Theme, width: number): string[] {
    ...
    const bodyLines = getDetailBodyLines(
      this.detailContent.renderPreview(entry, info, width),
      width,
      theme,
    );
  ...
  render(width: number): string[] {
    ...
    if (stickyLeftDepth) {
      lines[TREE_STICKY_STATUS_LINE_INDEX] = this.renderStickyLeftLine(
        theme,
        renderWidth,
        stickyLeftDepth,
      );
...
export function installTreeXNativePatches(
  ...
  proto[SHOW_SELECTOR_PATCH] = { original: originalShowSelector, patched: patchedShowSelector };
