...
type StreamUpdate = {
  ...
  delta?: string;
  contentIndex?: number;
  partial?: { content?: Array<{ type?: unknown; name?: unknown }>; usage?: { output?: number } };
...
function composeMessage(
  ...
  const lastMessage =
    result.lastMessageMs === null ? "--" : `${(result.lastMessageMs / 1000).toFixed(1)}s`;
  const localTimes =
    times === undefined
      ? ""
...
function shouldReportIntermediate(
  state: LoopState,
    ...
    if (result) {
      ctx.ui.notify(
        composeMessage(result, {
          promptTimeMs: state.promptTimeMs ?? lastUserPromptTimeMs,
          loopEndTimeMs,
        }),
        "info",
      );
