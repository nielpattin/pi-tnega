...
export class FullScreenAgentsManager implements Component, Focusable {
   ...
   private async generateAndOpenDraft() {
      ...
      const fallbackBody = `# ${this.newName.toUpperCase()} AGENT\n\nSpecialized agent for: ${this.newIntent || "custom workers"}.\n\n## Role\nExecute workers according to intent.\n\n## Constraints\n- Follow guidelines strictly.`;

      const newAgent: AgentDefinition = {
         name: this.newName,
         description: generatedFields?.description || this.newIntent || "Custom agent",
         guidance: generatedFields?.guidance || `Use for ${this.newIntent || "custom workers"}.`,
         harness: "pi",
   ...
   private getAvailableTools(): AgentToolInfo[] {
      ...
      // 1. Discover tools dynamically from WorkersRuntime, ExtensionCommandContext & Registries
      ...
      const BASELINE_TOOLS: Array<{
         ...
         {
            name: "worker_spawn",
            description: "Spawn workers in batches.",
            promptSnippet: "Worker launcher"
         },
         {
            name: "worker_list",
   ...
   private startTextEdit(field: "name" | "description" | "body") {
      ...
      if (field === "body") {
         ...
         this.initialBodyValue = agent?.body || "";
         this.bodyEditor.setValue(agent?.body || "");
         this.tui.requestRender();
         return;
      }
   ...
   private async saveBodyEdit() {
      const v = this.bodyEditor.getValue();
   ...
   private async applyIntent(intent: AgentsPanelIntent) {
      ...
      if (intent.type === "toggle_enabled") {
         ...
         patch = { thinking: thinking || undefined };
      }
      if (Object.keys(patch).length === 0) return;
      const nextAgent: AgentDefinition = { ...agent, ...patch };
   ...
   handleInput(data: string): void {
      ...
      if (this.isEditingBody) {
         const isDirty = this.bodyEditor.getValue() !== this.initialBodyValue;
      ...
      if (isBody) {
         if (data === "\x1b[5~" || data === "\x15") {
            this.bodyScrollOffset = Math.max(0, this.bodyScrollOffset - 5);
            this.tui.requestRender();
            return;
         }
         if (data === "\x1b[6~" || data === "\x04") {
            this.bodyScrollOffset += 5;
            this.tui.requestRender();
            return;
         }
      }
   ...
   private handlePickerInput(data: string) {
      ...
      if (this.viewState === "select_tools") {
         ...
         if (data === " ") {
            ...
            if (tool) {
               if (WORKER_LOCKED_TOOLS.has(tool.name) || DISABLED_NESTED_TOOLS.has(tool.name)) {
                  this.tui.requestRender();
                  return;
               }
         ...
         if (data === "n" && !q) {
            this.tempSelectedTools = Array.from(WORKER_LOCKED_TOOLS);
            this.tui.requestRender();
            return;
         }
   ...
   private async commitSingleValuePicker(val: string) {
      ...
      const agent = this.currentAgent();
      if (!agent) return;
      const patch = isModel ? { model: cleanVal } : { thinking: cleanVal };
      const nextAgent: AgentDefinition = { ...agent, ...patch };
   ...
   render(width: number): string[] {
      ...
      const lines: string[] = [];
      const vm = this.viewModel;
      const headerLeft = theme.fg("accent", theme.bold("Workers Agents Manager"));
   ...
   private renderPickerOverlay(width: number, targetRows: number): string[] {
      ...
      if (isTools) {
         ...
         for (let idx = 0; idx < visibleItems.length; idx++) {
            ...
            const isLockedWorker = WORKER_LOCKED_TOOLS.has(tool.name);
            const isDisabledNested = DISABLED_NESTED_TOOLS.has(tool.name);

            const checked = !isDisabledNested && (isLockedWorker || this.tempSelectedTools.includes(tool.name));
            ...
            let lockBadge = "";
            if (isLockedWorker) {
               lockBadge = theme.fg("dim", " (worker required)");
            } else if (isDisabledNested) {
   ...
   private renderBodyEditorOverlay(width: number, targetRows: number): string[] {
      ...
      const cursorIndex = this.bodyEditor.getCursorIndex();
   ...
   private isFieldChanged(field: AgentDetailField): boolean {
      ...
      if (field === "body") return agent.body !== orig.body;
      return false;
   }
