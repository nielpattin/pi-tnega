...
import type { JobTranscriptEntry, WorkerSpec } from "../domain.js";
...
export function renderProcessListCall(
   _args: Partial<ProcessListToolParams>,
   theme: Theme,
   _context?: RenderContext
): Component {
...
export function renderProcessStopCall(
   args: Partial<ProcessStopToolParams>,
   theme: Theme,
   _context?: RenderContext
): Component {
   const id = stringValue(args.id) ?? "process";
   return new Text(`${theme.fg("toolTitle", theme.bold("process_stop"))} ${theme.fg("accent", id)}`, 0, 0);
}
...
/** Render a Workers job result without exposing raw JSON envelopes. */
