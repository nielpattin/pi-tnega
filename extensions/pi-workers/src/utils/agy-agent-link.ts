...
const LINK_MARKER = ".workers-link.json";
...
function getAgyAgentProjection(sourcePath: string, agentName: string): AgyAgentProjection {
   ...
   if (!frontmatter) {
      return {
         content: `---${newline}name: ${agentName}${newline}description: ${JSON.stringify(`Pi Workers agent ${agentName}`)}${newline}---${newline}${content}`,
         canSymlink: false
      };
   }
   ...
   const descriptionValue = description ? parseYamlScalar(description) : `Pi Workers agent ${agentName}`;
   return {
...
/**
 * Make an Agy custom-agent path follow a Workers/Pi agent file.
 *
...
export function ensureAgyAgentLink(
   ...
   if (destinationStats?.isSymbolicLink()) {
      ...
      if (!markerMatchesSource(marker, source)) {
         throw new Error(`Agy agent "${agentName}" already exists at ${destination} and is not worker-managed.`);
      }
      fs.unlinkSync(destination);
