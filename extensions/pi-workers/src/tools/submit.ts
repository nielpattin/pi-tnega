...
export function createSubmitToolParamsSchema(expectedSchema?: unknown): TSchema {
   const dataSchema =
      expectedSchema && typeof expectedSchema === "object" && !Array.isArray(expectedSchema)
         ? Type.Unsafe(
              // SAFETY: WorkerManager converts worker output schemas before creating the worker, and this preserves the validated JSON Schema verbatim for provider-side tool validation.
...
export const submitToolDefinition = {
   name: "submit",
   description: "Submit final worker execution result or error.",
