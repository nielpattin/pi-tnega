   ...
   const spawnStep = (promptText: string, isContinuation: boolean) => {
      ...
      if (options.spawnProc) {
         const proc = options.spawnProc(
            commandString,
            { cwd: options.cwd, env: { WORKERS_CHILD_SESSION: "1" } },
            (code) => {
...
export class AgyBackend extends Context.Service<AgyBackend, AgyBackendShape>()("workers/AgyBackend") {
