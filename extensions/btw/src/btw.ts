...
type BtwSession = { readonly session: AgentSession; readonly displayStartIndex: number };
 ...
 */
export function buildBtwHandoff(
...
export async function createBtwChatSession(
  ...
  const seedMessages = buildSessionContext(
...
export class BtwChatOverlay implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    lines.push(
      this.frame(this.theme.fg("dim", `${this.readSessionInfo()}${scrollInfo}`), innerWidth),
    );
    lines.push(border("├", "─", "┤"));
    ...
    lines.push(
      this.frame(
        this.theme.fg("dim", "Enter send · /btw:inject move · PgUp/PgDn · Esc"),
        innerWidth,
      ),
    );
    lines.push(border("└", "─", "┘"));
...
class BtwChatController {
  ...
  private ensureOverlay(ctx: ExtensionCommandContext): void {
    ...
    void ctx.ui
      .custom<void>(
        ...
        {
          overlay: true,
          overlayOptions: { anchor: "center", width: "78%", maxHeight: "78%", margin: 1 },
          onHandle: (handle) => {
  ...
  private async submit(prompt: string): Promise<void> {
    ...
    try {
      ...
      const last = chat.session.messages
        .toReversed()
  ...
  async inject(ctx: ExtensionCommandContext, instructions: string): Promise<void> {
    ...
    const handoff = buildBtwHandoff(
      chat.session.messages.slice(chat.displayStartIndex),
      instructions,
    );
    if (!handoff) {
