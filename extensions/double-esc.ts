...
export function handleEscape(
  ...
  if (isIdle) {
    return { nextState: { hintActive: false, lastEscTime: 0 }, action: "nothing" };
  }

  const elapsed = now - state.lastEscTime;
  if (state.lastEscTime > 0 && elapsed <= debounceMs) {
    return { nextState: { hintActive: false, lastEscTime: 0 }, action: "abort" };
  }

  return { nextState: { hintActive: true, lastEscTime: now }, action: "show_hint" };
}
