...
export default function register(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const topK = Math.min(
        50,
        Math.max(params.topK ?? config.topK, (params.page ?? 1) * PAGE_SIZE),
      );
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use code_symbol_search to find function, class, interface, or type declarations.",
    ],
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use code_outline to see what a file or directory declares before reading it.",
    ],
  ...
  pi.registerTool({
    ...
    description:
      "Find callers, callees, or calls in a file. Shows what calls a function and what it calls.",
  ...
  pi.registerTool({
    ...
    label: "code_recall",
    description:
      "Recall memories by query. Blends keyword + semantic automatically, same as code_search.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use code_forget to remove a memory that is no longer relevant. memory_id is required.",
    ],
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      setActiveCwd(
        resolveProjectPaths(params.projectPath, ctx.cwd).base,
  ...
  pi.registerCommand("cc-index", {
    description:
      "Index files in the current project. After indexing, file watcher auto-starts in background.",
    handler: async (args, ctx) => {
      ...
      try {
        result = await indexPaths(paths, config, undefined, (text) =>
          ctx.ui.setStatus("cc-index", text),
        );
      } catch (e) {
      ...
      ctx.ui.setStatus("cc-index", undefined);
      ctx.ui.notify(
        `Indexed ${result.files} files, ${result.chunks} chunks in ${fmtTime(elapsed)}.`,
        "info",
      );
    },
  });
      ...
      const msg =
        result.deleted > 0
          ? `Cortex index files deleted (${dbPath}).` +
            (result.failed > 0
              ? ` ${result.failed} file(s) could not be deleted.${failureDetails}`
              : "") +
