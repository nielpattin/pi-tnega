...
export default function register(pi: ExtensionAPI) {
    ...
    * the optional `onProgress` callback (the `ce-index` command pipes this to
   ...
   pi.registerTool({
      ...
      label: "code_index",
      description:
         "Index files in the current project (or specified paths). Builds the embedding index so code_search, code_symbol_search, code_call_graph, code_triple_query, and code_ast_grep have data to query. Idempotent: a file watcher keeps the index fresh after the first run.",
      promptSnippet: "Build the local code index so search/grep tools return results.",
      promptGuidelines: [
         "Use code_index when other cortex tools return empty or no index exists yet.",
         "Do not call code_index repeatedly — it is idempotent and the watcher keeps it fresh.",
         "Long-running on big repos; returns a summary of files and chunks indexed."
      ],
      parameters: CodeIndexParams,
      async execute(_toolCallId, params, signal, onUpdate, ctx) {
         setActiveCwd(resolveProjectPaths(params.projectPath, ctx.cwd).base);
         const config = loadConfig();
         const paths = params.paths?.trim() ? params.paths.trim().split(/\s+/) : ["."];
         const start = performance.now();
         const result = await indexPaths(paths, config, signal, (text) =>
            onUpdate?.({
         ...
         return {
            content: [
               {
                  type: "text" as const,
                  text: `Indexed ${result.files} files, ${result.chunks} chunks in ${fmtTime(elapsed)}.`
               }
            ],
            details: undefined
         };
   ...
   pi.registerCommand("ce-index", {
      ...
      handler: async (args, ctx) => {
         ...
         ctx.ui.setStatus("ce-index", "Indexing starting…");
         const start = performance.now();
         let result: { files: number; chunks: number };
         try {
            result = await indexPaths(paths, config, undefined, (text) => ctx.ui.setStatus("ce-index", text));
         } catch (e) {
            ctx.ui.setStatus("ce-index", undefined);
   ...
   pi.registerCommand("cleanup-embed", {
      description: "Delete the cortex database. Run /ce-index to rebuild.",
      handler: async (_args, ctx) => {
         ...
         const msg = deleted
            ? "Cortex database deleted. Run /ce-index to rebuild."
   ...
   pi.registerCommand("ce-forget", {
      description: "Delete a memory by ID. Usage: /ce-forget <memory_id>",
      handler: async (args, ctx) => {
         const id = parseInt(args.trim(), 10);
         if (!Number.isFinite(id)) {
            ctx.ui.notify("Usage: /ce-forget <memory_id>", "error");
            return;
         }
         await forget(id);
