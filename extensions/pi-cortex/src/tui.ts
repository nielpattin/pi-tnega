 ...
 */
export function renderSymbolCall(
  args: { symbol: string; kind?: string; page?: number },
  theme: Theme,
): Text {
  const kind = args.kind ? ` ${fg(theme, "dim", `kind=${args.kind}`)}` : "";
  const page = args.page ? ` ${fg(theme, "dim", `page=${args.page}`)}` : "";
  return new Text(
    `${toolTitle(theme, "code_symbol_search")} ${fg(theme, "accent", args.symbol)}${kind}${page}`,
    0,
    0,
  );
}
 ...
 */
export function renderCallGraphCall(
  args: { symbol: string; direction?: string; page?: number },
  theme: Theme,
): Text {
  ...
  if (options.expanded) {
    if (edges.length === 0) {
      ...
      for (let i = 0; i < edges.length; i++) {
        const e = edges[i];
        lines.push(
          `${fg(theme, "accent", e.callerSymbol ?? "?")} → ${fg(theme, "accent", e.calleeSymbol ?? "?")}`,
        );
...
export function renderTripleResult(
  ...
  c.addChild(
    new Text(
      `hits=${triples.length} elapsed=${(data.elapsed ?? 0).toFixed(3)}s${pageNote(undefined, data)}`,
    ),
  );
...
export function renderMemoryResult(
  ...
  c.addChild(
    new Text(
      `hits=${memories.length} elapsed=${(data.elapsed ?? 0).toFixed(3)}s${pageNote(undefined, data)}`,
    ),
  );
