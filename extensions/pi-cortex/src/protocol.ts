...
const pending = new Map<
  number,
  { resolve: (v: unknown) => void; reject: (e: Error) => void; kind: string }
>();
let buf = "";
let ready = false;
...
function doStartSidecar(model: string, dbPath: string): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    child = spawn(
      binaryPath,
      ["--model-repo", model, "--models-dir", modelsDir, "--db-path", dbPath],
...
export async function rustTextSearch(
  ...
  const r = await rustSend(
    { text_search: { query, top_k: topK, path_filter: pathFilter ?? null } },
    15000,
    signal,
  );
  return (r as any).results;
...
export async function rustCallGraph(
  ...
  const r = await rustSend(
    { call_graph: { symbol, direction, path: filePath ?? null } },
    15000,
    signal,
  );
  return (r as any).results ?? [];
...
export async function rustMemoryStore(
  ...
  const r = await rustSend(
    { memory_store: { content, embedding, source, importance, scope } },
    15000,
    signal,
  );
...
export async function rustAstGrep(
  ...
  const r = await rustSend(
    { ast_grep: { pattern, lang, path_filter: pathFilter, top_k: topK } },
    15000,
    signal,
  );
  return (r as any).results ?? [];
...
export async function rustAstReplace(
  ...
  Array<{ file: string; matches: number; added: number; removed: number; diff?: string }>
> {
  const r = await rustSend(
