...
export const CodeOutlineParams = Type.Object({
  path: Type.String({
    description: "File or directory to outline (lists every symbol with kind and line range)",
  }),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
  ),
  projectPath: Type.Optional(
    Type.String({ description: "Project root (defaults to current project)" }),
  ),
});
...
export const CodeSearchParams = Type.Object({
  ...
  projectPath: Type.Optional(
    Type.String({ description: "Project root to search (defaults to current project)" }),
  ),
});
...
export const CodeSymbolParams = Type.Object({
  ...
  kind: Type.Optional(
    Type.String({
      description:
        "Only symbols of this kind: function, class, variable, interface, type, struct, enum, trait",
    }),
  ),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
...
export const CodeCallGraphParams = Type.Object({
  ...
  path: Type.Optional(
    Type.String({ description: "File path filter (required when direction='file')" }),
  ),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
  ),
  projectPath: Type.Optional(
    Type.String({ description: "Project root to query (defaults to current project)" }),
  ),
});

export const CodeTripleQueryParams = Type.Object({
  subject: Type.Optional(
    Type.String({ description: "Filter on subject (function/class/module name)" }),
  ),
  predicate: Type.Optional(
    Type.String({ description: "Filter on predicate (calls, defines, imports, etc.)" }),
  ),
  object: Type.Optional(Type.String({ description: "Filter on object (target name)" })),
  limit: Type.Optional(
    Type.Number({ description: "Max triples to return", minimum: 1, maximum: 500 }),
  ),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
  ),
...
export const CodeRememberParams = Type.Object({
  content: Type.String({
    description: "The text to remember. Will be embedded and stored for later recall.",
  }),
  source: Type.Optional(
    Type.String({ description: "Where this memory came from (e.g. tool name, url, file)" }),
  ),
  importance: Type.Optional(
    Type.Number({
      description: "Importance 0-1; higher = prioritised in recall",
      minimum: 0,
      maximum: 1,
    }),
  ),
  scope: Type.Optional(
    Type.String({ description: "Memory scope", enum: ["session", "project", "global"] as const }),
  ),
});
...
export const CodeRecallParams = Type.Object({
  query: Type.String({
    description:
      "Query to recall memories for. Function names lean keyword; sentences lean semantic.",
  }),
  topK: Type.Optional(
    Type.Number({ description: "Number of memories to return", minimum: 1, maximum: 50 }),
  ),
  scope: Type.Optional(
    Type.String({
      description: "Limit to memories in this scope (session, project, global); empty = all",
    }),
  ),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
...
export const CodeAstGrepParams = Type.Object({
  ...
  lang: Type.Optional(
    Type.String({
      description: "Limit to files of this language/extension (e.g. 'ts', 'py', 'rs')",
    }),
  ),
...
export const CodeAstReplaceParams = Type.Object({
  pattern: Type.String({
    description:
      "ast-grep pattern with metavariables (e.g. 'console.log($MSG)'). Same syntax as code_ast_grep.",
    minLength: 1,
  }),
  rewrite: Type.String({
    description:
  ...
  dryRun: Type.Optional(
    Type.Boolean({
      description: "Preview changes without applying (default false). Returns unified diff.",
    }),
  ),
  page: Type.Optional(
    Type.Number({ description: "Page number (1-based); 10 results per page", minimum: 1 }),
