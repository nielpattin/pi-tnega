...
async function showAccountsMenu(
  ...
  while (true) {
    ...
    if (screen === "login-providers") {
      ...
      await loginAccount(pi, ctx, store, provider.adapter, name, syncProvider, () =>
        owner.isCurrent(),
      );
      return;
    }
    ...
    if (screen === "remove") {
      const option = removeAccountOptions(states, currentProviderId).find(
        (candidate) => candidate.label === choice,
      );
      if (!option) continue;
...
function accountMenuScreenOptions(
  ...
  if (screen === "switch-providers") {
    return providerStatesWithAccounts(states, currentProviderId).map(
...
async function loginAccount(
  ...
  try {
    ...
    await store.updateProvider(adapter.id, (current) =>
      isCurrent()
        ? { active: parsed.name, accounts: defineOwn(current.accounts, parsed.name, credential) }
        : current,
    );
    if (!isCurrent()) return;
