...
export const RUNTIME_FAIL_CLOSED_API_KEY = "pi-acks-auth-failed";
...
type PersistentCredentialStore = {
   modify(providerId: string, fn: (current: unknown) => unknown): unknown;
   delete(providerId: string): unknown;
   read?(providerId: string): unknown;
};

type PersistentSyncState = {
...
export class RuntimeAuthCoordinator {
   ...
   private readonly persistent: RuntimeCredentialSync;
   ...
   constructor(
      ...
      this.persistent = new RuntimeCredentialSync(provider.id);
   }
   ...
   async ensureActive(
      ...
      const persistence = this.persistent.begin();
      ...
      if (!active) {
         ...
         try {
            ...
            await this.persistent.clear(ctx, persistence);
      ...
      try {
         ...
         try {
            await this.persistent.sync(ctx, credential, persistence);
         } catch {
            // Best-effort mirror of the active credential into pi's auth.json for
            // next-startup model restore; runtime auth is already applied above.
         }
...
class RuntimeCredentialSync {
   private generation = 0;
   private readonly states = new WeakMap<object, PersistentSyncState>();
   ...
   async sync(ctx: ExtensionContext, credential: OAuthCredential, generation: number): Promise<void> {
      if (generation !== this.generation) return;
      const target = getPersistentCredentialStore(ctx);
      if (!target) return;
      await target.modify(this.providerId, async () => credential);
      this.getState(target).lastActiveAccountId = accountIdOf(credential);
   }

   async clear(ctx: ExtensionContext, generation: number): Promise<void> {
      if (generation !== this.generation) return;
      const target = getPersistentCredentialStore(ctx);
      if (!target) return;
      const state = this.getState(target);
      const lastActiveAccountId = state.lastActiveAccountId;
      state.lastActiveAccountId = undefined;
      if (lastActiveAccountId === undefined) return;
      let current: unknown;
      try {
         current = await target.read?.(this.providerId);
      } catch {
         return; // Cannot verify ownership; leave the stored entry untouched.
      }
      if (accountIdOf(current) !== lastActiveAccountId) return;
      await target.delete(this.providerId);
   }

   private getState(target: object): PersistentSyncState {
...
function getPersistentCredentialStore(ctx: ExtensionContext): (PersistentCredentialStore & object) | undefined {
   const registry = ctx.modelRegistry as unknown as { runtime?: unknown; authStorage?: unknown };
   for (const candidate of [registry, registry.runtime, registry.authStorage]) {
      const store = isRecord(candidate) && isRecord(candidate.credentials) ? candidate.credentials : candidate;
      if (isRecord(store) && typeof store.modify === "function" && typeof store.delete === "function") {
         return store as PersistentCredentialStore & object;
      }
   }
   return undefined;
}
...
function accountIdOf(value: unknown): string | undefined {
   return isRecord(value) && typeof value.accountId === "string" ? value.accountId : undefined;
}
...
class RuntimeProviderOverlay {
   ...
   apply(ctx: ExtensionContext, generation: number, auth: ModelAuth, availableModelIds?: readonly string[]): boolean {
      ...
      if (this.owned && !shallowConfigEqual(current, this.applied)) {
         this.reset();
         throw new Error(
            `${this.provider.displayName} provider configuration changed while pi-acks owned its auth overlay.`
         );
      }
      if (!this.owned) {
...
export function redactTokenText(text: string, exactSecrets: readonly string[] = []): string {
   const secrets = [...new Set(exactSecrets.filter(Boolean))].toSorted((a, b) => b.length - a.length);
