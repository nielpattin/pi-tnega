...
type PersistentSyncState = { lastActiveAccountId?: string };
...
type ProviderAccountState = { active?: string; accounts: Record<string, OAuthCredential> };
      ...
      try {
        current = await store.updateProviderAsync(this.provider.id, async (latest) => {
          ...
          try {
            ...
            return { ...latest, accounts: defineOwn(latest.accounts, active, refreshed) };
          } catch (error) {
            refreshError = error;
            return latest;
          }
...
function getPersistentCredentialStore(
  ctx: ExtensionContext,
): (PersistentCredentialStore & object) | undefined {
  ...
  for (const candidate of [registry, registry.runtime, registry.authStorage]) {
    const store =
      isRecord(candidate) && isRecord(candidate.credentials) ? candidate.credentials : candidate;
    if (
      isRecord(store) &&
...
export function redactTokenText(text: string, exactSecrets: readonly string[] = []): string {
  const secrets = [...new Set(exactSecrets.filter(Boolean))].toSorted(
