...
import { type PermissionVerdict, reviewBashCommand } from "./auto-guardian";
import { isAutoModeEnabled, shouldAutoApprovePermissionState } from "./yolo-mode";
...
export interface PermissionPrompterDeps {
   ...
   /** Optional custom guardian reviewer for auto mode testing / override. */
   reviewGuardian?: (details: PromptPermissionDetails, ctx: ExtensionContext) => Promise<PermissionVerdict>;
}

/**
 * Encapsulates the full permission-prompt flow:
 *   1. Yolo-mode auto-approval check.
 *   2. Auto-mode guardian review check for bash.
 *   3. Review-log "waiting" entry.
 *   4. UI-present vs. subagent-forwarding branching (via confirmPermission).
 *   5. Review-log "approved" / "denied" entry.
 */
export class PermissionPrompter implements PermissionPrompterApi {
   ...
   async prompt(ctx: ExtensionContext, details: PromptPermissionDetails): Promise<PermissionPromptDecision> {
      ...
      if (shouldAutoApprovePermissionState("ask", config)) {
         this.writeReviewEntry("permission_request.auto_approved", details);
         return { approved: true, state: "approved", autoApproved: true };
      }

      if (isAutoModeEnabled(config) && (details.toolName === "bash" || details.toolName?.endsWith(".bash"))) {
         const reviewer = this.deps.reviewGuardian ?? reviewBashCommand;
         const verdict = await reviewer(details, ctx);
         ...
         if (verdict.decision === "revise") {
            const denialReason = `Auto Permissions requested revision: ${verdict.reason}\nRevise the command and try again.`;
            this.writeReviewEntry("permission_request.denied", {
               ...details,
               resolution: "guardian_revised",
               denialReason
            });
            return {
               approved: false,
