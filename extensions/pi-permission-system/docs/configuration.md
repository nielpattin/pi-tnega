...
| Scope   | Path                                                            |
| ------- | --------------------------------------------------------------- |
| Global  | `~/.pi/agent/permission.jsonc` (respects `PI_CODING_AGENT_DIR`) |
...
```jsonc
{
    "$schema": "https://raw.githubusercontent.com/nielpattin/pi-packages/main/extensions/pi-permission-system/schemas/permissions.schema.json",
...
Permission prompt sounds are configured in Pi's agent settings file, not in `permission.jsonc`:
...
`sound` may be absolute, `~`/`$HOME`-relative, or relative to `~/.pi/agent/settings.json`. `volume` is a percentage, so `150` plays at 150%; values above `100` use an ffplay audio filter because ffplay's startup volume is capped at `100`. If the section is absent, the default sound path is `~/.pi/agent/assets/permission-request.mp3`.
