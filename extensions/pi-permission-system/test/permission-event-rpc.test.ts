...
describe("registerPermissionRpcHandlers — permissions:rpc:check", () => {
   ...
   it("replies allow for an allowed surface/value", async () => {
      ...
      const successNarrowed = reply as Extract<typeof reply, { success: true }>;
      expect(successNarrowed.data?.result).toBe("allow");
      expect(successNarrowed.data?.origin).toBe("global");
   });
   ...
   it("replies deny for a denied surface/value", async () => {
      ...
      expect(reply.success).toBe(true);
      const successNarrowed = reply as Extract<typeof reply, { success: true }>;
      expect(successNarrowed.data?.result).toBe("deny");
      expect(successNarrowed.data?.matchedPattern).toBe("rm *");
   });
   ...
   it("replies ask for an ask surface/value", async () => {
      ...
      expect(reply.success).toBe(true);
      const successNarrowed = reply as Extract<typeof reply, { success: true }>;
      expect(successNarrowed.data?.result).toBe("ask");
   });
...
describe("registerPermissionRpcHandlers — permissions:rpc:prompt", () => {
   ...
   it("replies with approval when user approves", async () => {
      ...
      expect(reply.protocolVersion).toBe(PERMISSIONS_PROTOCOL_VERSION);
      const successNarrowed = reply as Extract<typeof reply, { success: true }>;
      expect(successNarrowed.data?.approved).toBe(true);
      expect(successNarrowed.data?.state).toBe("approved");
   });
   ...
   it("replies with denied when user denies", async () => {
      ...
      expect(reply.success).toBe(true);
      const successNarrowed = reply as Extract<typeof reply, { success: true }>;
      expect(successNarrowed.data?.approved).toBe(false);
      expect(successNarrowed.data?.state).toBe("denied_with_reason");
      expect(successNarrowed.data?.denialReason).toBe("Too risky");
   });
