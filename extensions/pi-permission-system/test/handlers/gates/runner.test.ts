...
describe("runGateCheck", () => {
   ...
   describe("denialContext formatting", () => {
      ...
      it("uses denialContext to format denyReason with extension tag", async () => {
         ...
         const actionNarrowed = result as Extract<typeof result, { action: "block" }>;
         expect(actionNarrowed.reason).toContain(EXTENSION_TAG);
         expect(actionNarrowed.reason).not.toContain("Hard stop");
      });
      ...
      it("uses denialContext to format unavailableReason with extension tag", async () => {
         ...
         const actionNarrowed = result as Extract<typeof result, { action: "block" }>;
         expect(actionNarrowed.reason).toContain(EXTENSION_TAG);
         expect(actionNarrowed.reason).toContain("no interactive UI");
      });
      ...
      it("uses denialContext to format userDeniedReason with extension tag", async () => {
         ...
         const actionNarrowed = result as Extract<typeof result, { action: "block" }>;
         expect(actionNarrowed.reason).toContain(EXTENSION_TAG);
         expect(actionNarrowed.reason).toContain("too risky");
      });
   });
});
