...
describe("session_start handler consolidation", () => {
   ...
   test("session_start handler preserves lifecycle.reload debug log", async () => {
      ...
      const sessionStartHandler = registrations.find((r) => r.name === "session_start");
      ...
      await expect(sessionStartHandler!.handler({ reason: "reload" }, mockCtx)).resolves.not.toThrow();
   });
});
