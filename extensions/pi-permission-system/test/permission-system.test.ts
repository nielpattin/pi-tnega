...
import piPermissionSystemExtension from "../index.js";
...
test("Permission-system status shows current mode (default/auto/yolo)", () => {
...
test("Tool registry blocks unregistered tools and handles aliases", () => {
   ...
   const statusNarrowed = unknownCheck as Extract<typeof unknownCheck, { status: "unregistered" }>;
   expect(statusNarrowed.availableToolNames).toEqual(["bash", "mcp", "read"]);
...
function createManagerWithProject(
   ...
   writeFileSync(globalConfigPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
   const projectConfigNarrowed = options as CreateManagerWithProjectOptions & { projectConfig: ScopeConfig };
   writeFileSync(projectGlobalConfigPath, `${JSON.stringify(projectConfigNarrowed.projectConfig, null, 2)}\n`, "utf8");
