...
describe("checkRequestedToolRegistration", () => {
   ...
   test("returns registered when tool name matches a string entry", () => {
      ...
      const statusNarrowed = result as Extract<typeof result, { status: "registered" }>;
      expect(statusNarrowed.requestedToolName).toBe("read");
      expect(statusNarrowed.normalizedToolName).toBe("read");
   });
   ...
   test("returns unregistered when tool is not in the list", () => {
      ...
      const statusNarrowed = result as Extract<typeof result, { status: "unregistered" }>;
      expect(statusNarrowed.requestedToolName).toBe("ghost");
      expect(statusNarrowed.availableToolNames).toContain("read");
      expect(statusNarrowed.availableToolNames).toContain("write");
   });

   test("available tool names are sorted alphabetically", () => {
      const result = checkRequestedToolRegistration("ghost", ["write", "read", "edit"]);
      const statusNarrowed = result as Extract<typeof result, { status: "unregistered" }>;
      expect(statusNarrowed.availableToolNames).toEqual(["edit", "read", "write"]);
   });
   ...
   test("resolves alias: requested alias maps to registered canonical name", () => {
      ...
      expect(result.status).toBe("registered");
      const statusNarrowed = result as Extract<typeof result, { status: "registered" }>;
      expect(statusNarrowed.normalizedToolName).toBe("bash");
   });
   ...
   test("returns unregistered with empty availableToolNames for empty tool list", () => {
      ...
      expect(result.status).toBe("unregistered");
      const statusNarrowed = result as Extract<typeof result, { status: "unregistered" }>;
      expect(statusNarrowed.availableToolNames).toEqual([]);
   });
