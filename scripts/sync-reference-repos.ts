...
export class ReferenceRepoSelectionError extends Schema.TaggedError<ReferenceRepoSelectionError>()(
  ...
  { repoId: Schema.String, expectedRepoIds: Schema.Array(Schema.String) },
) {
...
export class ReferenceRepoPrefixCheckError extends Schema.TaggedError<ReferenceRepoPrefixCheckError>()(
  ...
  { repoId: Schema.String, prefix: Schema.String, cause: Schema.Defect() },
) {
  override get message(): string {
...
export const planReferenceRepoSync = Effect.fn("planReferenceRepoSync")(function* (
  ...
  ReferenceRepoSyncPlan,
  ReferenceRepoSyncError,
  FileSystem.FileSystem | Path.Path
> {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const action: ReferenceRepoSyncAction = (yield* fs
    .exists(path.join(rootDir, repo.prefix))
    .pipe(
      Effect.mapError(
        (cause) =>
          new ReferenceRepoPrefixCheckError({ repoId: repo.id, prefix: repo.prefix, cause }),
      ),
    ))
    ? "pull"
    : "add";
...
> {
  ...
  const child = yield* spawner
    ...
    .pipe(
      Effect.mapError(
        (cause) => new ReferenceRepoGitSubtreeError({ ...errorContext, operation: "spawn", cause }),
      ),
    );
  ...
  const [stdout, stderr, exitCode] = yield* Effect.all(
    ...
    Effect.mapError(
      (cause) =>
        new ReferenceRepoGitSubtreeError({ ...errorContext, operation: "communicate", cause }),
    ),
